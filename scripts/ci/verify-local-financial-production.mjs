...
function appendSummary(text) { if (summaryPath) appendFileSync(summaryPath, text); }
...
async function fetchProduction(path) {
  ...
  const response = await fetch(url, { redirect: 'follow', cache: 'no-store', signal: AbortSignal.timeout(30_000), headers: { 'user-agent': 'TexasDefined-CI-Local-Financial-Smoke/1.0' } });
  return { response, body: await response.text() };
}
const hasNoindex = (body) => /<meta[^>]+(?:name=["']robots["'][^>]+content=["'][^"']*noindex|content=["'][^"']*noindex[^"']*["'][^>]+name=["']robots["'])/i.test(body);
...
for (const page of affordabilityPages) {
  ...
  try {
    ...
    const required = [`${page.name} home affordability calculator`, `href="${page.tax}"`, `href="${page.relocation}"`, `${origin}${page.path}`, 'WebApplication', 'BreadcrumbList', 'FAQPage', 'This is a planning calculator'];
    const missing = required.filter((needle) => !body.includes(needle));
    if (!response.ok) fail(label, `HTTP ${response.status}`); else if (hasNoindex(body)) fail(label, 'unexpected robots noindex'); else if (missing.length) fail(label, `missing ${missing.join(', ')}`); else { console.log(`[${label}] verified (${response.status})`); appendSummary(`| ✅ pass | ${label} | 200, H1/copy, canonical, indexable, local tax + relocation links, WebApplication/BreadcrumbList/FAQPage |\n`); }
...
for (const page of costOfLivingPages) {
  ...
  try {
    const { response, body } = await fetchProduction(page.path);
    const required = [`${page.name} cost of living calculator`, `${page.name} Cost of Living Calculator | Texas Defined`, `href="${page.tax}"`, `href="${page.relocation}"`, `${origin}${page.path}`, 'WebApplication', 'BreadcrumbList', 'FAQPage', 'No citywide average or preset local index is used.', 'Planning only.'];
    const missing = required.filter((needle) => !body.includes(needle));
    if (!response.ok) fail(label, `HTTP ${response.status}`); else if (hasNoindex(body)) fail(label, 'unexpected robots noindex'); else if (missing.length) fail(label, `missing ${missing.join(', ')}`); else { console.log(`[${label}] verified (${response.status})`); appendSummary(`| ✅ pass | ${label} | 200, title/H1/copy, canonical, indexable, local tax + relocation links, WebApplication/BreadcrumbList/FAQPage |\n`); }
  } catch (error) { fail(label, error instanceof Error ? error.message : String(error)); }
}
...
for (const page of salaryNeededPages) {
  ...
  try {
    const { response, body } = await fetchProduction(page.path);
    const required = [`Salary needed to live in ${page.name} calculator`, `${page.name} Salary Needed`, `href="${page.costPath}"`, `href="${page.tax}"`, `href="${page.relocation}"`, `${origin}${page.path}`, 'WebApplication', 'BreadcrumbList', 'FAQPage', 'Monthly household budget', 'Planning only.'];
    const missing = required.filter((needle) => !body.includes(needle));
    if (!response.ok) fail(label, `HTTP ${response.status}`); else if (hasNoindex(body)) fail(label, 'unexpected robots noindex'); else if (missing.length) fail(label, `missing ${missing.join(', ')}`); else { console.log(`[${label}] verified (${response.status})`); appendSummary(`| ✅ pass | ${label} | 200, title/H1/copy, canonical, indexable, local budget + tax + relocation links, WebApplication/BreadcrumbList/FAQPage |\n`); }
  } catch (error) { fail(label, error instanceof Error ? error.message : String(error)); }
}
...
for (const page of propertyTaxSamples) {
  ...
  try {
    const { response, body } = await fetchProduction(page.path);
    const required = [page.name, 'property tax calculator', `${origin}${page.path}`, 'parcel', 'taxing'];
    const missing = required.filter((needle) => !body.toLowerCase().includes(needle.toLowerCase()));
    if (!response.ok) fail(label, `HTTP ${response.status}`); else if (hasNoindex(body)) fail(label, 'unexpected robots noindex'); else if (missing.length) fail(label, `missing ${missing.join(', ')}`); else { console.log(`[${label}] verified (${response.status})`); appendSummary(`| ✅ pass | ${label} | 200, county context, canonical, indexable, parcel/taxing-unit explanation |\n`); }
  } catch (error) { fail(label, error instanceof Error ? error.message : String(error)); }
}
...
try {
  ...
  if (!response.ok) fail('sitemap', `HTTP ${response.status}`);
    ...
    const expectedPaths = [...affordabilityPages.map((page) => page.path), ...costOfLivingPages.map((page) => page.path), ...salaryNeededPages.map((page) => page.path), ...propertyTaxSamples.map((page) => page.path)];
    const missing = expectedPaths.filter((path) => !body.includes(`${origin}${path}`));
    if (missing.length) fail('sitemap', `missing ${missing.join(', ')}`); else { console.log(`[sitemap] verified ${expectedPaths.length} local financial URLs`); appendSummary(`| ✅ pass | sitemap | all ${expectedPaths.length} checked local financial URLs present |\n`); }
  }
} catch (error) { fail('sitemap', error instanceof Error ? error.message : String(error)); }
