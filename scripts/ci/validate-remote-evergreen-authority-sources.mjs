...
for (const line of sourceText.split(/\r?\n/)) {
  ...
  let match;
  while ((match = sourcePattern.exec(listText)) !== null) {
    sources.push({ label: match[1], url: match[2], scope: match[3] });
  }

  if (!sources.length) {
    throw new Error(`Could not parse authority sources for ${slug}`);
  }
...
for (const slug of REQUIRED_SLUGS) {
  ...
  if (!sources || sources.length < 2) {
    throw new Error(`${slug} must have at least two authority sources`);
  }

  const urls = new Set(sources.map((source) => source.url));
  if (urls.size !== sources.length) {
    throw new Error(`${slug} contains duplicate authority-source URLs`);
  }

  for (const source of sources) {
    if (source.label.trim().length < 8) {
      throw new Error(`${slug} has an authority-source label shorter than 8 characters`);
    }
    if (source.scope.trim().length < 20) {
      throw new Error(`${slug} has an authority-source scope shorter than 20 characters`);
    }

    const url = new URL(source.url);
    if (url.protocol !== "https:") {
      throw new Error(`${slug} uses a non-HTTPS authority source: ${source.url}`);
    }
    if (!APPROVED_SOURCE_HOSTS.has(url.hostname)) {
      throw new Error(`${slug} uses an unapproved authority-source host: ${url.hostname}`);
    }
  }
}
