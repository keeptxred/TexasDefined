...
for (const [label, path, needle] of surfaces) {
  ...
  for (let attempt = 1; attempt <= 6; attempt += 1) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.log(`[${label}] HTTP ${response.status}, but expected content is not live yet: ${needle}`);
      }
    } catch (error) {
  ...
  if (!passed) {
    ...
    if (lastBody) {
      const snippet = lastBody.slice(0, 1200).replace(/\s+/g, ' ');
      console.error(`[${label}] response sample: ${snippet}`);
    }
