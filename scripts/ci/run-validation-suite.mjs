...
const predeployIds = new Set([
  ...
  'texas-icon-link-depth',
  'texas-weather-authority',
  'texas-food-history',
  'texas-culture-citation-index',
  'texas-flag-authority',
  'painted-churches-seo',
  'painted-church-search-intents',
  'painted-church-map',
  'painted-church-completion',
  'military-history-expansion',
]);

const suiteName = process.argv[2] ?? 'full';
const selected = suiteName === 'predeploy' ? checks.filter(([id]) => predeployIds.has(id)) : suiteName === 'full' ? checks.filter(([id]) => !['texas-flag-authority', 'painted-churches-seo', 'painted-church-map', 'painted-church-completion'].includes(id)) : null;
...
for (const [id, classification, label, command, args] of selected) {
  ...
  summaryRows.push(`${ok ? '✅' : '❌'} | ${classification} | ${label} | ${durationSeconds}s`);
...
if (failed) {
  console.error(`Validation suite '${suiteName}' failed. The failing check and failure class are recorded above and in the GitHub job summary.`);
  process.exit(1);
}
