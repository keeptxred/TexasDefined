...
for (const profile of profiles) {
  ...
  if (!sitemap.includes(expectedUrl)) throw new Error(`Wave 6 sitemap missing ${profile.name}: ${expectedUrl}`);
  ...
  if (directives.has('noindex') || !directives.has('index') || !directives.has('follow')) {
    throw new Error(`Wave 6 robots policy mismatch for ${profile.name}: ${metaContent(html, 'robots') || 'missing'}`);
  }
}

console.log(`RV Wave 6 production verification passed: ${profiles.length} profiles are sitemap-discoverable, canonical index/follow destinations with WebPage and TouristAttraction schema.`);
