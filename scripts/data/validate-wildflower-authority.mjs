...
for (const symbol of ['11 Texas wildflowers to know', 'scientific', 'Bloom', 'Texas range', 'Open species guide', 'editorialImageSrc']) {
  if (!grid.includes(symbol)) errors.push(`Wildflower visual field guide missing: ${symbol}`);
}
for (const symbol of ['Lady Bird Johnson Wildflower Center', 'SOURCE_URL', 'internalLinks', '/article/texas-wildflowers-guide', 'Echinacea purpurea', 'Solidago spp.']) {
  if (!articles.includes(symbol)) errors.push(`Wildflower guide content safeguard missing: ${symbol}`);
}
