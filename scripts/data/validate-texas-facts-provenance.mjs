...
const sourceData = fs.readFileSync('src/data/texas-essential-fact-sources.server.ts', 'utf8');
const provenanceData = fs.readFileSync('src/data/texas-essential-facts-provenance.server.ts', 'utf8');
const functionsData = fs.readFileSync('src/data/texas-facts.functions.ts', 'utf8');
...
for (const legacyClientPath of [
  'src/data/texas-essential-fact-sources.ts',
  'src/data/texas-essential-facts-provenance.ts',
]) {
  if (fs.existsSync(legacyClientPath)) failures.push(`Texas Facts provenance-heavy data must remain server-only; remove client-loadable module: ${legacyClientPath}`);
}
...
for (const token of [
  ...
  'loadTexasFactsDataServer',
]) {
  if (!provenanceData.includes(token)) failures.push(`Texas Facts server provenance join contract missing: ${token}`);
}

for (const token of [
  "import { createServerFn } from '@tanstack/react-start';",
  "await import('./texas-essential-facts-provenance.server')",
  'loadTexasFactsDataServer()',
]) {
  if (!functionsData.includes(token)) failures.push(`Texas Facts server loader boundary missing: ${token}`);
}

for (const browserSource of [routeData, lazyData]) {
  if (browserSource.includes('texas-essential-fact-sources.server') || browserSource.includes('texas-essential-facts-provenance.server')) {
    failures.push('Texas Facts route modules must not statically import provenance-heavy server modules into the browser graph.');
  }
}
...
for (const token of [
  ...
  if (!routeData.includes(token)) failures.push(`Texas Facts provenance-backed schema/loader contract missing: ${token}`);
}
...
console.log('Texas Facts provenance validation passed: 100/100 facts have fail-closed claim-level source assignments, visible source links and citation-backed ItemList schema; provenance-heavy source data remains server-only and ClaimReview remains prohibited.');
