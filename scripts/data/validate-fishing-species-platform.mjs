...
if (!failures.length) {
  ...
  for (const signal of ["COMPLETE_FISHING_SPECIES_SLUGS", '"largemouth-bass"', '"smallmouth-bass"', '"guadalupe-bass"', '"spotted-bass"', '"crappie"', '"black-crappie"', '"white-crappie"', '"catfish"', '"blue-catfish"', '"channel-catfish"', '"flathead-catfish"', '"white-bass"', '"striped-bass"', '"hybrid-striped-bass"', '"alligator-gar"', '"freshwater-drum"', '"sunfish"', '"bluegill"', '"rainbow-trout"']) {
  ...
  if (!hubUi.includes('to="/fishing/species"') || !hubUi.includes('fishingFoundationAnchor("species", fish.slug)')) failures.push("Fishing hub does not discover the species directory and canonical species targets.");
  ...
  for (const signal of ["isCompleteFishingSpeciesSlug", "isCompleteFishingLakeSlug", "lakeSpecies", "lakeTechniques", "buildFishingSpeciesProfileHead", "citation:", "Lakes with full fishing guides appear before basic lake profiles", "not a live bite report"]) {
  ...
  for (const phrase of ["Techniques listed", "Linked lake profiles", "Where to catch", "sourced lake guidance", "verified lake-to-species relationship"]) {
  ...
  if (!directoryUi.includes("Every published fish has a guide") || !directoryUi.includes("Every published fish record has a sourced species page")) failures.push("Species directory visitor-facing relationship label missing.");
...
console.log("Fishing species platform validation passed: statewide taxonomy, all 19 published species routes, verified lake relationships, server-built profiles, native lazy UI boundaries, deep largemouth guidance, sponsorship separation, search/internal links, structured data and sitemap governance are protected.");
