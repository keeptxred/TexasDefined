...
if (!siteSearchLazy.includes('createLazyFileRoute("/search")')) errors.push('Site-wide search UI must remain behind a native lazy route boundary.');
if (!siteSearchRoute.includes('name="q"')) errors.push('Site-wide search form does not accept the SearchAction query parameter.');
if (!siteSearchShell.includes('canonicalPath: "/search"')) errors.push('Site-wide search does not consolidate query variants to its canonical route.');
if (!siteSearchShell.includes('robots: "noindex, follow"')) errors.push('Site-wide search results are not protected from indexing.');
...
console.log('WebSite SearchAction, noindex search surfaces, lazy global-search UI, and resolved publication-gated destination search indexing passed validation.');
