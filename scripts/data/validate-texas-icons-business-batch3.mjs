...
if ((research.match(/remains noindex pending image-rights and internal-link certification/g) ?? []).length !== 10) failures.push("Every Business batch-3 profile must retain the noindex/image-rights/internal-link boundary.");
...
for (const token of [
  "industrial agricultural innovator",
  "one-transistor DRAM",
  "Apollo 13",
  ...
  if (!research.includes(token)) failures.push(`Business batch 3 is missing required accuracy/context token: ${token}.`);
}
...
for (const domain of ["galleryfurniture.com", "footballfoundation.org", "invent.org", "ibm.com", "nasa.gov", "historycollection.jsc.nasa.gov", "ntrs.nasa.gov", "axiomspace.com", "rice.edu", "swri.org", "tshaonline.org", "tamu.edu"]) {
  if (!research.includes(domain)) failures.push(`Business batch 3 is missing expected authority/source domain: ${domain}.`);
}
...
if (!resolver.includes("texasTalentFutureCanonicalPath") || !resolver.includes("indexableAtOwnRoute: false")) failures.push("Business batch 3 must preserve Texas Talent canonical ownership and non-indexable Icons research routes.");

if (failures.length) fail();
console.log("Texas Icons Business & Science batch-3 validation passed: ranks 171-180 contain ten verified staged profiles with corrected identity/context claims and duplicate protection.");
