...
const [route, hub, slugs, sitemap, search, publicRoutes] = await Promise.all([
...
]) assert(route.includes(marker), `Fishing lakes directory is missing quality/SEO marker: ${marker}.`);
...
for (const slug of ["lake-conroe", "lake-fork", "sam-rayburn-reservoir", "lake-livingston", "lake-texoma"]) {
  assert(slugs.includes(`"${slug}"`), `Complete fishing lake allowlist is missing ${slug}.`);
}
...
]) assert(hub.includes(marker), `Fishing hub is missing lakes-directory discovery marker: ${marker}.`);

for (const marker of [
  'FISHING_LAKES_DIRECTORY_PATH = "/fishing/lakes"',
  '{ path: FISHING_LAKES_DIRECTORY_PATH, lastmod: FISHING_LAKES_DIRECTORY_VERIFIED_AT }',
]) assert(sitemap.includes(marker), `Fishing sitemap is missing lakes-directory ownership marker: ${marker}.`);
...
for (const marker of [
  ...
  'href: "/fishing/lakes"',
  '"compare fishing lakes"',
]) assert(search.includes(marker), `Fishing site-search index is missing statewide/lakes directory marker: ${marker}.`);
...
console.log("Fishing lakes directory validated: five completed lake guides are query-backed, sitemap/search-owned, answer-first and protected from thin/incomplete lake leakage.");
