...
for (const marker of ['Link to="/fishing/lakes"', 'Open the full lake finder →', 'Browse fishing lakes →', 'A few places to start.', 'Full fishing guide', 'Lake profile']) assert(hubComponent.includes(marker), `Fishing hub is missing lakes-directory discovery marker: ${marker}.`);
...
console.log("Fishing lakes directory validated: ten completed lake guides remain query-backed, lazily rendered and sitemap/search-owned while the fishing hub now discovers the full lake finder and distinguishes full guides from lake profiles.");
