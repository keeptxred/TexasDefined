...
const sourcePath = path.join(collectionDir, 'part-1.csv');
const reconciliationPath = path.join(collectionDir, 'part-1-reconciliation.csv');
...
const routeRequiredStatuses = new Set([
  'existing-authority',
  'existing-cluster',
  'existing-destination',
]);

const routeForbiddenStatuses = new Set([
  'candidate-new-article',
  'claim-risk-hold',
  'route-review-needed',
]);

function fail(message) {
  console.error(`250 Texas Stories Part 1 reconciliation validation failed: ${message}`);
  process.exit(1);
}
...
if (sourceByNumber.size !== 50) fail('part-1.csv must expose 50 unique source_number values');

const seen = new Set();

for (const row of reconciliationRows) {
  const sourceNumber = Number(row.source_number);
  if (!Number.isInteger(sourceNumber) || sourceNumber < 1 || sourceNumber > 50) {
  ...
  if (!source) fail(`source_number ${sourceNumber} does not exist in part-1.csv`);
  ...
  if (routeRequiredStatuses.has(row.coverage_status)) {
    if (!row.canonical_route.startsWith('/')) {
...
console.log(
  `250 Texas Stories Part 1 reconciliation passed: 50 source-matched rows remain fail-closed. ` +
