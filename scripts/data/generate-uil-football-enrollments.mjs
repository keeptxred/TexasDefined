...
const OFFICIAL_ALPHA_NAME_ALIASES = new Map([
  ['Dallas Gateway Charter Acad', 'Dallas Gateway Charter Academy'],
  ['College Station A&M Consol.', 'College Station A&M Consolidated'],
  ['Woodlands College Park', 'Conroe Woodlands College Park'],
]);
...
function extractPdfFootballRows(text) {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    const [, rawSchoolName, submittedConference, enrollmentText, middle, footballType] = match;
    const schoolName = OFFICIAL_ALPHA_NAME_ALIASES.get(rawSchoolName) ?? rawSchoolName;
    if (!/\d/.test(middle)) {
