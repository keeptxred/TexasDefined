...
for (const marker of ['platform.articles.list(scope)', '.filter((article) => !isLegacyCountySeriesArticle(article.slug) && isTexasGatewayIndexReadyArticle(article))', '.map((article) => ({ path: `/article/${article.slug}`']) if (!sitemap.includes(marker)) failures.push(`Article sitemap discovery contract missing: ${marker}`);
