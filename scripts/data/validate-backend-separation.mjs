...
if(!errors.length){
  ...
  for(const slug of livingRoutes){const source=fs.readFileSync(`src/routes/${slug}.tsx`,'utf8');if(!source.includes(`createFileRoute('/${slug}')`)||!source.includes('canonicalLink'))errors.push(`TexasDefined living route is incomplete: ${slug}`);if(source.includes('keeptxred.com')||source.includes('SUPABASE_'))errors.push(`TexasDefined living route ${slug} contains cross-site dependency.`)}
  const living=fs.readFileSync('src/routes/texas-living.tsx','utf8');
  for(const token of ['/explore','/moving-to-texas','/decide/financial-tools','/find-my-dmv','/find-my-school-district'])if(!living.includes(token))errors.push(`Texas living hub missing ${token}.`);
}
...
console.log(`TexasDefined lifestyle ownership, ${livingRoutes.length} living guides, and ${calculators.length} independent calculators are valid.`);
