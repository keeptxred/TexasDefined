...
const articleCountManifest = fs.readFileSync(path.join(root, 'src/data/explore-category-article-counts.ts'), 'utf8');
...
function manifestCounts() {
  ...
  for (const match of articleCountManifest.matchAll(/^\s*"([^"]+)":\s*(\d+),?\s*$/gm)) counts.set(match[1], Number(match[2]));
  return counts;
}
...
for (const feature of [
  'EXPLORE_CATEGORY_ARTICLE_COUNTS',
  'as const satisfies Partial<Record<CategorySlug, number>>',
  'exploreCategoryArticleCount(category: CategorySlug)',
]) {
  if (!articleCountManifest.includes(feature)) errors.push(`Explore category lightweight article manifest missing: ${feature}.`);
}
...
validateMegaMenuImages('Texas Life', 12);
...
for (const feature of [
  'return "major-springs"', 'return "national-parks"', 'return "caverns"', 'return "beaches-coast"', 'return "historic-sites"',
  '"lighthouse"', '"wildlife_refuge"', '"wildlife_management_area"', 'return "outdoors"', 'explore_entity_types(key,name)', 'MAX_REMOTE_DESTINATIONS',
]) {
  if (!remote.includes(feature)) errors.push(`Migrated Explore catalog feature missing: ${feature}.`);
}

for (const feature of [
  'readingMinutes: 4', 'function estimateReadingMinutes(article: Article): number', 'Math.max(3, Math.ceil(words / 200))', 'readingMinutes: estimateReadingMinutes(article)',
]) {
