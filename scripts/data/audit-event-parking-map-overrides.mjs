...
function objectStringProperty(node, key) {
  ...
  for (const property of node.properties) {
    ...
    return literalString(property.initializer);
  }
  return undefined;
}

function readCanonicalMajorEventOccurrences() {
  const file = path.join(DATA_DIR, 'major-event-index.ts');
  ...
  const visit = (node) => {
    if (ts.isObjectLiteralExpression(node)) {
      ...
      const startDate = objectStringProperty(node, 'startDate');
      const venue = objectStringProperty(node, 'venue');
      if (slug && startDate && venue) occurrences.set(slug, { slug, startDate, venue });
    }
