...
if (!failures.length) {
  ...
  if (!profileUi.includes("If a detail isn’t listed, we don’t guess.") || !profileUi.includes("guide.startingPriceCents !== undefined")) failures.push("Guide profile optional-fact rendering is not protected.");
  for (const marker of ["Guide coverage", "Where this guide fishes.", "Guide details", ">Sources</p>", "Last reviewed {source.checkedAt}"]) if (!profileUi.includes(marker)) failures.push(`Guide profile visitor-facing source language missing: ${marker}`);
  for (const stale of ["Verified service relationships", "Where this guide is verified to fish.", "Verified listing details", "Verification sources", "Checked {source.checkedAt}"]) if (profileUi.includes(stale)) failures.push(`Guide profile must not restore internal verification UI wording: ${stale}`);
