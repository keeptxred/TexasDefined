...
const batch2 = read('src/data/sports-venue-enrichment-batch2.ts');
const remediation2 = read('src/data/sports-venue-content-remediation-wave2.ts');
...
const venues = [
  ...
  ['eagle-stadium-allen', 'Eagle Stadium'],
  ['mckinney-isd-stadium', 'McKinney ISD Stadium'],
  ['childrens-health-stadium-prosper', 'Children’s Health Stadium'],
  ['legacy-stadium-katy', 'Legacy Stadium'],
  ['ratliff-stadium', 'Ratliff Stadium'],
  ['cy-fair-fcu-stadium', 'Cy-Fair FCU Stadium'],
  ['mesquite-memorial-stadium', 'Mesquite Memorial Stadium'],
  ['ufcu-disch-falk-field', 'UFCU Disch-Falk Field'],
  ['lupton-stadium', 'Lupton Stadium'],
  ['baylor-ballpark', 'Baylor Ballpark'],
  ['reckling-park', 'Reckling Park'],
];
...
for (const [slug, name] of venues) {
  ...
  requireText(wave6Guides, `'${slug}': {`, `${name} verified wave-six guide`);
  requireText(wave6Guides, `canonicalPath: '/sports-venue/${slug}'`, `${name} canonical path`);

  const guideStart = wave6Guides.indexOf(`'${slug}': {`);
  const nextGuide = wave6Guides.indexOf("\n  '", guideStart + 1);
  const guideBlock = wave6Guides.slice(guideStart, nextGuide === -1 ? undefined : nextGuide);
...
for (const marker of [
  'getSportsVenueGuideWave6',
  'getSportsVenueGuideWave6(slug) ?? getSportsVenueGuideWave5(slug) ?? getSportsVenueGuideWave4(slug) ?? getSportsVenueGuidePilot(slug)',
...
const correctDkrUrl = 'https://texaslonghorns.com/facilities/memorial-stadium/1';
for (const [source, label] of [[wave6Guides, 'Wave 6 DKR guide'], [batch2, 'legacy DKR enrichment'], [remediation2, 'authoritative DKR remediation']]) {
  requireText(source, correctDkrUrl, label);
  if (source.includes('https://texaslonghorns.com/facilities/bobcat-stadium/1')) failures.push(`${label}: stale Bobcat Stadium facility URL must be removed.`);
  if (source.includes('https://texaslonghorns.com/facilities/dkr-texas-memorial-stadium/1')) failures.push(`${label}: stale DKR slug facility URL must be removed.`);
}

requireText(wave6Guides, "'reliant-stadium': {", 'Reliant Stadium current venue identity');
requireText(wave6Guides, "officialUrl: 'https://www.houstontexans.com/stadium/'", 'Reliant Stadium current Texans official destination');
requireText(wave6Guides, "capacity: '72,220'", 'Reliant Stadium current official capacity');

for (const filler of ["homeTeam: 'N/A'", "homeTeam: 'None'", 'Make the venue part of the weekend', 'Best trip pattern']) {
  if (wave6Guides.includes(filler)) failures.push(`Wave-six guide registry contains prohibited filler: ${filler}`);
}
...
// Galaxy Stadium is still governed by its protected static canonical/alias contract.
...
if (failures.length) {
  console.error('Sports venue Phase 3 wave-six validation failed:');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`Sports venue Phase 3 wave-six validation passed for ${venues.length} conventional migrations: modular verified guides, corrected DKR source records, current Reliant identity, governed dynamic routing, source-reviewed editorial records, event/calendar wiring, provider-neutral ticket fallbacks, fail-closed Stay Nearby behavior and licensed-photo-or-intentional-fallback image handling remain intact without generic filler.`);
