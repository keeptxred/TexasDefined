...
for (const marker of ['Monthly household budget', 'Monthly savings / reserve', 'Filing status', 'Pre-tax retirement contribution', 'Planning gross income', 'Planning only.', 'grossSalaryForTakeHome2026', 'estimatePayroll2026', 'FinancialCalculatorScaffold', 'profile.path', 'profile.propertyTaxHref', 'profile.affordabilityHref', 'profile.homeownershipHref', 'profile.insuranceHref', 'profile.relocationHref', '/texas-salary-calculator', '/texas-budget-planner']) if (!page.includes(marker)) failures.push(`Salary-needed planner UI missing ${marker}.`);
