...
for (const token of [
  ...
  "2012 NFL MVP",
  "2026 Dallas Cowboys homecoming",
  "2026 Rams trade",
  "21 PGA Tour wins by August 2026",
  "13 PGA Tour wins through 2026",
  "2,105-yard 1984 season",
  "six NFL passing titles",
  "1948 Heisman Trophy",
]) {
  if (!research.includes(token)) failures.push(`Sports batch 4 is missing required editorial context: ${token}.`);
}
for (const domain of [
  "usopm.org",
  ...
  if (!research.includes(domain)) failures.push(`Sports batch 4 is missing expected authority/source domain: ${domain}.`);
}
...
for (const [, , slug] of entries) {
  if (talentSource.includes(`slug: "${slug}"`)) failures.push(`Sports batch-4 duplicate detected: ${slug} now exists in Texas Talent and must be reconciled instead of duplicated.`);
}
for (const token of [
  "TEXAS_ICON_RESEARCH_SPORTS_BATCH_4",
  'from "@/data/texas-icons-research-sports-4.server"',
  "...TEXAS_ICON_RESEARCH_SPORTS_BATCH_4",
]) {
  if (!resolver.includes(token)) failures.push(`Sports batch-4 resolver wiring missing: ${token}.`);
}
...
if (failures.length) fail();
console.log("Texas Icons sports batch-4 validation passed: ranks 131-140 preserve ten staged research profiles, 2026 current-state context, source depth, future duplicate detection and noindex publication boundaries.");
