...
const checks = [
  ...
  [hurstWhirlyballProductionVerifier, "fetchLive('/sitemap-explore.xml'", 'Hurst/WhirlyBall production verifier must inspect the live Explore sitemap'],
  ...
  [hurstWhirlyballProductionVerifier, 'const maxAttempts = 18;', 'Hurst/WhirlyBall production verifier must allow bounded edge-propagation retries'],
  [hurstWhirlyballProductionVerifier, 'requiredMarkers.filter((marker) => !body.includes(marker))', 'Hurst/WhirlyBall production verifier must retry stale successful HTML until required revision markers are live'],
  [hurstWhirlyballProductionVerifier, "is still serving the previous revision; missing:", 'Hurst/WhirlyBall production verifier must diagnose stale successful responses'],
