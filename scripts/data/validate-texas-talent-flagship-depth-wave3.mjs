...
for (const slug of expected) {
  ...
  const works = stringArray(block, "definingWorks");
  ...
  if (works.length < 5) fail(`${slug}: needs at least five defining works or achievements`);
  if (timeline.length < 5) fail(`${slug}: needs at least five timeline milestones`);
...
console.log(`Texas Talent flagship-depth wave 3 passed: ${expected.length} additional music and film profiles meet the stronger biography, works, timeline, legacy and Texas-place launch thresholds without receiving launch approval.`);
