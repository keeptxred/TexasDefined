...
function blockFor(source, slug) {
  ...
  if (start < 0) fail(`missing wave 3 record for ${slug}`);
...
const timelineRepairs = ["george-strait", "stevie-ray-vaughan", "janis-joplin", "waylon-jennings", "roy-orbison"];
const depthCohort = ["ornette-coleman", "townes-van-zandt", "lightnin-hopkins", "lead-belly", "billy-gibbons"];
...
for (const slug of timelineRepairs) {
  const block = blockFor(source, slug);
  const timeline = objectArray(block, "timeline");
  if (timeline.length < 5) fail(`${slug}: timeline still has only ${timeline.length} milestones`);
}

for (const slug of depthCohort) {
  const block = blockFor(source, slug);
...
if (recordCount !== timelineRepairs.length + depthCohort.length) {
  fail(`wave 3 must contain exactly 10 records; found ${recordCount}`);
}

console.log("Texas Talent flagship-depth wave 3 validation passed: wave 2 timelines are repaired and Ornette Coleman, Townes Van Zandt, Lightnin' Hopkins, Lead Belly and Billy Gibbons meet launch-depth thresholds without receiving editorial launch approval.");