...
const fail = (message) => {
  console.error(`Texas Talent flagship-depth wave 4 validation failed: ${message}`);
  process.exit(1);
};
...
if (recordCount !== expected.length) fail(`wave 4 must contain exactly ${expected.length} flagship records; found ${recordCount}`);

console.log("Texas Talent flagship-depth wave 4 validation passed: T-Bone Walker, Jamie Foxx, Woody Harrelson, Tommy Lee Jones and Wes Anderson meet the stronger narrative, timeline, legacy and Texas-place launch thresholds without receiving launch approval.");
