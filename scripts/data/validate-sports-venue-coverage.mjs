...
const [major, tier2, seed, directory, guide, galaxyGuide, enrichment, enrichmentBatch2, enrichmentBatch3, enrichmentBatch4Racing, enrichmentBatch5, enrichmentBatch6, enrichmentAll, currentCorrections] = await Promise.all([
...
const totalEnrichmentCount = enrichmentCount + enrichmentBatch2Count + enrichmentBatch3Count + enrichmentBatch4RacingCount + enrichmentBatch5Count + enrichmentBatch6Count;
...
assert(totalEnrichmentCount >= 57, `Expected at least 57 deeply enriched venue profiles; found ${totalEnrichmentCount}.`);
...
for (const source of [enrichment, enrichmentBatch2, enrichmentBatch3, enrichmentBatch4Racing, enrichmentBatch5, enrichmentBatch6]) {
...
assert(enrichmentAll.includes('getSportsVenueEnrichmentBatch6(lookupSlug)'), 'Combined sports venue enrichment does not merge the sixth batch.');
...
console.log(`Sports venue coverage validated: ${majorCount} major seeds + ${tier2Count} second-tier seeds, ${totalEnrichmentCount} deep visitor profiles including ${enrichmentBatch4RacingCount} racing profiles and ${enrichmentBatch6Count} Friday Night Lights/Western profiles, current-name corrections and dedicated visitor guides present.`);
