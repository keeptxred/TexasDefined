...
for (const token of ['--publish', '--feed-id=', 'requestedFeedId', 'TEXASDEFINED_AUTO_PUBLISH_ENABLED', 'TEXASDEFINED_PUBLISH_CONFIRMATION', 'readyQueue()', 'validateDraft', 'generateAndStoreImage', 'publish_texasdefined_queue_item_v2']) {
  if (!publisher.includes(token)) errors.push(`Publisher is missing ${token}`);
}
for (const token of [
  "github.event.issue.user.login == 'keeptxred'",
  "github.event.issue.author_association == 'OWNER'",
  'flyover_publish_request_v2',
  'site=texasdefined',
  'feed_id=',
  '--feed-id="$FEED_ID"',
  'requested exact feed id',
]) {
