...
if (baseSlugs.length !== 20) failures.push(`Expected 20 protected base photo records; found ${baseSlugs.length}.`);
if (additionSlugs.length !== 39) failures.push(`Expected 39 rights-verified photo additions; found ${additionSlugs.length}.`);
...
if (overlap.length) failures.push(`Photo additions must not shadow protected base records: ${overlap.join(', ')}`);
...
for (const slug of additionSlugs) {
  ...
  const licenseMatch = block.match(/licenseUrl: '([^']+)'/);
...
console.log(`Sports venue photo additions validated: ${baseSlugs.length} protected base records + ${additionSlugs.length} rights-verified additions = ${totalLicensed}/${totalSeeded} licensed venue heroes; ${remainingFallback} venues remain on the intentional fail-closed fallback.`);
