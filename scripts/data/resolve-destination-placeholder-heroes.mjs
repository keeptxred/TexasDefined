...
const safeSlug = (value) => normalize(value).replace(/\s+/g, "-").replace(/^-+|-+$/g, "");
...
async function commonsSearch(query, limit = 40) {
  const params = new URLSearchParams({
    ...
    prop: "imageinfo", iiprop: "url|mime|size|extmetadata", iiurlwidth: "1800", format: "json", origin: "*",
  });
...
async function chooseCommonsImage(destination, usedTitles) {
  const searches = [
    `intitle:\"${destination.name}\"`,
    `\"${destination.name}\" Texas`,
    `${destination.name} ${destination.nearestTown || "Texas"}`,
    `${destination.name} Texas`,
  ];
  let best = null;
...
function aiPrompt(destination) {
  const context = [
    destination.summary,
    destination.nearestTown ? `Nearest town: ${destination.nearestTown}, Texas.` : "",
    destination.county ? `County context: ${destination.county}.` : "",
  ].filter(Boolean).join(" ");
  return [
...
function parseCandidateRecord(text, slugMatch) {
  ...
  const window = text.slice(start, Math.min(text.length, start + 7000));
...
function overrideEntry(row) {
  return [
    ...
    `    src: ${JSON.stringify(row.src)},`,
    `    alt: ${JSON.stringify(row.alt)},`,
    `    width: ${row.width},`,
    `    height: ${row.height},`,
    `    credit: ${JSON.stringify(row.credit)},`,
    "  },",
  ].join("\n");
}
...
async function main() {
  ...
  for (let index = 0; index < candidates.length; index += 1) {
    ...
    resolved.push({
      ...destination,
      src: `/images/destination-heroes/${destination.slug}.jpg`,
      alt: source === "free-use" ? `${destination.name} in Texas` : `AI-generated representative editorial image for ${destination.name} in Texas`,
      width,
      height,
      credit,
      source,
