...
function scoreCandidate(page, query, key) {
  ...
  let score = 0;
  for (const token of queryTokens) if (evidence.includes(token)) score += 9;
  for (const token of keyTokens) if (evidence.includes(token)) score += 14;
...
function targetForReference(reference, file, text, index) {
...
function collectTargets(file, text) {
  ...
  for (const regex of regexes) {
    for (const match of text.matchAll(regex)) {
      ...
      const target = targetForReference(reference, file, text, match.index);
      if (!target) continue;
      target.file = file;
      seen.add(reference);
      targets.push(target);
...
async function main() {
  ...
  if (unresolved.length || remaining.length) {
    throw new Error(`SVG hero remediation is incomplete: ${unresolved.length} unresolved target(s), ${remaining.length} direct SVG hero reference(s) remain.`);
  }
}
