...
const required = [
  `${root}/contract.ts`, `${root}/entities.ts`, `${root}/fingerprint.ts`, `${root}/promotion.ts`,
...
for (const file of ['contract.ts', 'entities.ts', 'fingerprint.ts', 'promotion.ts', 'index.ts']) if (!upstream.files.includes(file)) errors.push(`Upstream manifest does not include ${file}.`);
...
requireSymbols(integration, ["from '@/shared/platform-core'", 'diffEntitySets', 'fingerprintEntities', 'sharedQuarantineEntity', 'non-promotable-status', 'invalid-coordinates', 'validatePromotionApproval'], 'TexasDefined shared-core integration');
...
for (const capability of ['entity-contracts','entity-canonicalization','deterministic-fingerprints','entity-set-diffs','baseline-quarantine','promotion-previews']) if (!consumer.capabilities.includes(capability)) errors.push(`TexasDefined consumer capability missing: ${capability}`);
...
if (errors.length) fail();
console.log(`Shared platform core ${upstream.version} API ${upstream.apiVersion} pinned at ${upstream.commit} is compatible, observable, and boundary-safe.`);
