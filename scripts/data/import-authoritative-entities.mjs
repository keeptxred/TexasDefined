...
for (const [name, adapter] of selected) {
  ...
  try {
    ...
    if (write) {
      ...
      await fs.writeFile(output, `${JSON.stringify({ ...report, mode: 'staged-only', records }, null, 2)}\n`);
      staged += records.length;
    }
    results.push(report);
    console.log(`${name}: ${records.length} validated records${write ? ' staged for governed review' : ' (dry run)'}.`);
  } catch (error) {
...
await fs.writeFile(summaryPath, `${JSON.stringify({ generatedAt: new Date().toISOString(), mode: write ? 'staged-only' : 'dry-run', staged, promoted: 0, results }, null, 2)}\n`);
