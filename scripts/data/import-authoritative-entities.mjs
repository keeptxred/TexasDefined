...
const adapters = {
  ...
  usgs: {
    ...
    sourceId: 'usgs-water',
    transform: (body) => [{ snapshot: true, service: body.name ?? 'USGS National Map', sourceId: 'usgs-water' }],
  },
  tpwd: {
...
for (const [name, adapter] of selected) {
  ...
  try {
    ...
    const records = dedupe(adapter.transform(raw)).filter(validRecord);
    ...
    results.push(report);
    console.log(`${name}: ${records.length} validated records${write ? ' written' : ' (dry run)'}.`);
  } catch (error) {
...
await fs.writeFile(summaryPath, `${JSON.stringify({ generatedAt: new Date().toISOString(), write, results }, null, 2)}\n`);
...
function validRecord(record) { return Boolean(record && record.externalId && record.name && record.slug && record.sourceId && record.sourceConfidence); }
