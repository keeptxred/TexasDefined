...
const importer = read(required[8]);
const urlVerifier = read(required[9]);
const promotionRunner = read(required[10]);
const workflow = read(required[11]);
...
if (importer.includes("fs.writeFile(generated")) errors.push('Authoritative importer still writes directly to the generated production graph.');
...
requireSymbols(workflow, [
  ...
  'import-authoritative-entities.mjs --write', 'Verify official URLs',
  'Prepare promotion preview', 'actions/upload-artifact@v4', 'retention-days: 30',
