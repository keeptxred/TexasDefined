...
const researched = [
  [207, "Dan Rather", "dan-rather"],
  [208, "Bill Moyers", "bill-moyers"],
];
const reused = [
  [201, "Eva Longoria", "eva-longoria"],
  [202, "Carol Burnett", "carol-burnett"],
  [203, "Steve Martin", "steve-martin"],
  [204, "Larry McMurtry", "larry-mcmurtry"],
  [205, "Cormac McCarthy", "cormac-mccarthy"],
  [206, "Katherine Anne Porter", "katherine-anne-porter"],
  [209, "Robert Rodriguez", "robert-rodriguez"],
  [210, "Richard Linklater", "richard-linklater"],
];
...
if ((research.match(/editorialStatus: "researched-staged"/g) ?? []).length !== 2) failures.push("Media batch 2 must keep both new profiles researched-staged.");
if ((research.match(/publicationNote: staged/g) ?? []).length !== 2) failures.push("Every Media batch-2 profile must retain the staged publication boundary.");
if (!research.includes("remains noindex pending image-rights and internal-link certification")) failures.push("Media batch 2 must retain the noindex/image-rights/internal-link boundary.");
...
if (!resolver.includes("texasTalentFutureCanonicalPath") || !resolver.includes("indexableAtOwnRoute: false")) failures.push("Media batch 2 must preserve canonical ownership and non-indexable Icons routes.");

if (failures.length) fail();
console.log("Texas Icons Media & Arts batch-2 validation passed: ranks 201-210 are covered exactly once by eight Texas Talent reuses and two staged research profiles.");
