...
const researched = [
  [248, "Texas Sheet Cake", "texas-sheet-cake"],
  [249, "The Cowboy Boot", "the-cowboy-boot"],
];
const canonical = [
...
for (const [rank, name] of [...researched, ...canonical, [graphOwned[0], graphOwned[1]]]) {
  if (!source.includes(`${rank},${name},Symbols & Food,`)) failures.push(`Symbols & Food roster drift at rank ${rank}: expected ${name}.`);
}
...
for (const [, name, path, slug] of canonical) {
  ...
  if (research.includes(`slug: "${slug}"`)) failures.push(`Canonical subject ${slug} must not be duplicated as staged research.`);
}
...
if (research.includes('slug: "the-cotton-bowl"')) failures.push("The Cotton Bowl must not duplicate its knowledge-graph owner as staged research.");
...
if ((research.match(/editorialStatus: "researched-staged"/g) ?? []).length !== 2) failures.push("Final Symbols batch must keep both new profiles researched-staged.");
if ((research.match(/publicationNote: staged/g) ?? []).length !== 2) failures.push("Every final Symbols profile must retain the staged publication boundary.");
if (!research.includes("noindex pending image-rights and internal-link certification")) failures.push("Final Symbols batch must retain the noindex/image-rights/internal-link boundary.");
...
for (const token of [
  ...
  "2025",
  "Boot Capital",
]) if (!research.includes(token)) failures.push(`Final Symbols batch is missing accuracy/context token: ${token}.`);
...
if (canonicalPrecedence < 0 || graphPrecedence < 0 || researchPrecedence < 0 || canonicalPrecedence > graphPrecedence || graphPrecedence > researchPrecedence) failures.push("Canonical and knowledge-graph owners must continue to resolve before staged research.");
if (!resolver.includes("indexableAtOwnRoute: false")) failures.push("Final Symbols batch must preserve non-indexable Icons routes.");

if (failures.length) fail();
console.log("Texas Icons Symbols & Food final validation passed: ranks 246-250 close the 250-item registry with two staged research profiles, two explicit canonical reuses, and one knowledge-graph reuse.");
