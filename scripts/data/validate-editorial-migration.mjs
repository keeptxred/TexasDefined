...
for (const forbidden of ['KeepTXRed','Keep TX Red','/news/homestead','/tools/']) if (migrated.includes(forbidden) || lazyMigrated.includes(forbidden) || financeDepth.includes(forbidden)) errors.push(`Public migrated editorial contains legacy text or path: ${forbidden}`);
