...
for (const slug of expected) {
  if (!migrated.includes(`slug: "${slug}"`)) errors.push(`Migrated article missing: ${slug}`);
}

const slugMatches = [...migrated.matchAll(/slug: "([^"]+)"/g)].map((match) => match[1]);
if (new Set(slugMatches).size !== slugMatches.length) errors.push('Migrated editorial slugs must remain unique.');
if (slugMatches.length !== expected.length) errors.push(`Expected ${expected.length} migrated articles, found ${slugMatches.length}.`);

for (const forbidden of ['KeepTXRed', 'Keep TX Red', '/news/homestead', '/tools/']) {
  if (migrated.includes(forbidden)) errors.push(`Public migrated editorial contains legacy text or path: ${forbidden}`);
}
...
for (const category of ['real-estate', 'moving-to-texas', 'texas-history', 'food-bbq']) {
  if (!migrated.includes(`category: "${category}"`)) errors.push(`Expected migrated category is empty: ${category}`);
}
