...
const categoryRoute = fs.readFileSync('src/routes/things-unique-to-texas_.$category.tsx', 'utf8');
const lazyRoute = fs.readFileSync('src/routes/things-unique-to-texas_.$category.lazy.tsx', 'utf8');
const methodologyRoute = fs.readFileSync('src/routes/things-unique-to-texas_.methodology.tsx', 'utf8');
...
if (deepDiveIds.length < 49) failures.push(`Expected at least 49 purpose-built/editorial deep-dive mappings; found ${deepDiveIds.length}.`);
if (canonicalIds.length + deepDiveIds.length < 91) failures.push(`Expected at least 91 protected deeper-guide relationships; found ${canonicalIds.length + deepDiveIds.length}.`);
...
for (const token of ['"@type": "Article"', '"@type": "WebPage"', '"@type": "ItemList"', '"@type": "BreadcrumbList"', 'publisher:', 'articleSection: isFoodHistoryChild ? "Texas Food History" : "Things That Define Texas"', 'citation: sources.length', 'Sources', 'Where the historical claims come from']) if (!evergreenComponent.includes(token)) failures.push(`Shared evergreen guide schema/source layer must retain token: ${token}.`);
