...
const experiments = [
  ...
  { path: '/event/sweetwater-rattlesnake-roundup', title: 'Sweetwater Rattlesnake Roundup: Dates & Visitor Guide', description: 'Plan the Sweetwater Jaycees Rattlesnake Roundup with date guidance' },
  ...
  { path: '/event/charro-days-fiesta', title: 'Charro Days Fiesta: Dates, Parade & Brownsville Guide', description: 'Plan Charro Days Fiesta in Brownsville with current date guidance' },
...
function validateWave(name, wave, expectedCount) {
  for (const experiment of wave) {
    for (const required of [`\"${experiment.path}\"`, experiment.title, experiment.description]) {
      if (!seo.includes(required)) failures.push(`${name} CTR contract missing for ${experiment.path}: ${required}`);
    }
  }
  if (wave.length !== expectedCount) failures.push(`Expected exactly ${expectedCount} ${name} GSC CTR experiments, found ${wave.length}.`);
}

validateWave('Page-one', experiments, 20);
validateWave('Second-wave', secondWave, 14);
validateWave('Third-wave', thirdWave, 4);
