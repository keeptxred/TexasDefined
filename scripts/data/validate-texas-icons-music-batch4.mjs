...
for (const entry of researchEntries) {
  if (!research.includes(`slug: "${entry.slug}"`)) failures.push(`Missing music batch-4 research profile: ${entry.slug}.`);
}
...
for (const domain of [
  "tshaonline.org", "uh.edu", "rice.edu", "countrymusichalloffame.org",
  "arts.gov", "si.edu", "folkways.si.edu", "spoontheband.com",
]) {
  if (!research.includes(domain)) failures.push(`Music batch 4 is missing expected authority/source domain: ${domain}.`);
}
for (const token of [
  "Screwed Up Click", "Religion and Hip-Hop Culture", "accidental", "Fifth Ward",
  "Ray Price Beat", "2025", "Mal Hombre", "Before the Next Teardrop Falls", "Austin",
]) {
  if (!research.includes(token)) failures.push(`Music batch 4 is missing required editorial context: ${token}.`);
}
...
for (const token of [
  ...
  'from "@/data/texas-icons-research-music-4.server"',
  '...TEXAS_ICON_RESEARCH_MUSIC_BATCH_4',
]) {
  if (!resolver.includes(token)) failures.push(`Music batch-4 resolver wiring missing: ${token}.`);
}
...
if (!stagedResearchBlock.includes('reuseKind: "icon-research-staged"') || !stagedResearchBlock.includes("indexableAtOwnRoute: false")) failures.push("Music batch-4 research drafts must remain non-indexable at their own routes.");
...
if (failures.length) fail();
console.log("Texas Icons music batch-4 validation passed: ranks 81–90 preserve one Texas Talent reuse, nine substantive staged research profiles, source depth, duplicate safety and noindex publication boundaries.");
