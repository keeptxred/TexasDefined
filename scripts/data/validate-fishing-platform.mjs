...
if (!failures.length) {
  ...
  if (!fishingRoute.includes("source-backed lake facts") || !fishingRoute.includes("local guide resources")) failures.push("Fishing hub metadata must use visitor-facing source-backed language.");
  for (const stale of ["verified lake facts", "verified local guide infrastructure"]) {
    if (fishingRoute.includes(stale)) failures.push(`Fishing hub metadata still exposes verification-heavy wording: ${stale}`);
  }
