...
if (!errors.length) {
  ...
  const consumer = JSON.parse(fs.readFileSync(required[5], 'utf8'));
  const upstream = JSON.parse(fs.readFileSync(required[6], 'utf8'));
  ...
  for (const symbol of ['Governance Health','Blocked rate','Override acceptance','Privacy controls','bounded process memory']) if (!dashboard.includes(symbol)) errors.push(`Governance dashboard coverage missing ${symbol}`);
...
console.log('TexasDefined governance logging, analytics, drift detection, privacy, API, and dashboard are valid.');
