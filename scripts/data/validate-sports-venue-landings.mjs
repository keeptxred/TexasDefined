...
const [landings, route, indexComponent, directory, sports, publicRoutes, majorVenues, tier2Venues] = await Promise.all([
...
for (const slug of expectedLandings) {
  ...
  assert(publicRoutes.includes(`"/sports-venues/${slug}"`), `Public sitemap ownership is missing /sports-venues/${slug}.`);
}
...
]) assert(route.includes(marker), `Sports venue landing route is missing SEO/discovery marker: ${marker}.`);
...
console.log(`Sports venue landings validated: ${expectedLandings.length} indexable market/theme pages are sitemap-owned, internally linked, canonicalized, structured and kept off the heavy enrichment path.`);
