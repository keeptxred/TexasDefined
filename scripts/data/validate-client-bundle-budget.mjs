...
const feedRoutePaths = [path.resolve('src/routes/rss[.]xml.ts'), primarySitemapPath];
// CI measured the post-relocation-tool client bundle at 1,834,711 bytes.
// Keep the hard guardrail below the project's 1.85 MB ceiling.
...
function reportCiError(title, message) {
  if (process.env.GITHUB_ACTIONS === 'true') console.error(`::error title=${title}::${message}`);
}
...
async function main() {
  ...
  for (const feedRoutePath of feedRoutePaths) {
    ...
    if (eagerPlatformImport.test(source)) throw new Error(`${path.relative(process.cwd(), feedRoutePath)} must not eagerly import the fixture-backed platform into the route tree.`);
    if (!source.includes('const { platform, scope } = await import("@/data");')) throw new Error(`${path.relative(process.cwd(), feedRoutePath)} must load platform/scope inside its server handler.`);
  }

  const primarySitemap = await readFile(primarySitemapPath, 'utf8');
  if (/import\s*{\s*COUNTY_PROPERTY_RECORDS\s*}\s*from\s*["']@\/data\/property\/county-property-data["']/.test(primarySitemap)) throw new Error('Primary sitemap must not eagerly import the full county property catalog into the route tree.');
  if (!primarySitemap.includes('const { COUNTY_PROPERTY_RECORDS } = await import("@/data/property/county-property-data");')) throw new Error('Primary sitemap must load the county property catalog inside its server handler.');

  const assetsDir = await resolveAssetsDir();
  const entries = await readdir(assetsDir);
  const mainCandidates = entries.filter((name) => /^main-.*\.js$/.test(name));
  if (mainCandidates.length !== 1) throw new Error(`Expected exactly one main client bundle, found ${mainCandidates.length}: ${mainCandidates.join(', ')}`);
