...
requireCondition(
  iconsServer.includes('isTexasTalentPublishable')
    && iconsServer.includes('texasTalentFutureCanonicalPath')
    && iconsServer.includes('loadTexasTalentProfilesServer'),
...
requireCondition(
  metadata.includes('export const TEXAS_TALENT_FUTURE_BASE_PATH = "/texas-talent"')
    && metadata.includes('return `${TEXAS_TALENT_FUTURE_BASE_PATH}/${slug}`'),
  "future Texas Talent canonical ownership must remain centralized in launch metadata",
);
requireCondition(
  launch.includes('publishable: mechanicalReady && editorialApproved')
    && launch.includes('profile.readiness.launchStatus === "launch-ready"'),
  "Texas Talent publication must continue to require both mechanical readiness and explicit launch-ready approval",
);

// Image rights must be visible in the editorial review surface, not merely stored
// in data. This keeps attribution/license obligations attached to the image a
// future public implementation will inherit.
for (const requiredRendererToken of [
  "heroImage.credit",
  "heroImage.licenseLabel",
  "heroImage.sourceUrl",
  "heroImage.licenseUrl",
  "heroImage?.rightsNote",
]) {
  requireCondition(
    profilePreview.includes(requiredRendererToken),
    `hidden Talent profile preview must render image-rights field ${requiredRendererToken}`,
  );
}
requireCondition(
  profilePreview.includes('rel="noreferrer"'),
  "external image/source attribution links must retain safe rel handling",
);
requireCondition(
  readiness.includes('sourceUrl: string;')
    && readiness.includes('licenseLabel: string;')
    && readiness.includes('rightsNote: string;'),
...
requireCondition(
  ...
  "Texas Talent must remain outside public route classification before launch",
);
requireCondition(
  !sitemap.includes("texas-talent"),
  "Texas Talent must remain outside sitemap discovery before launch",
);

console.log(
  "Texas Talent ownership contract passed: overlapping Texas Icons records defer to Talent publication governance, duplicate biographies stay non-indexable, and image-rights attribution remains visible in the editorial review surface.",
);
