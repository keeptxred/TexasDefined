...
for (const [needle, label] of [
  ['const path = VIATOR_CITY_PATHS[citySlug]', 'city verified-direct-market gate'],
  ['if (!path) return null', 'city no-inventory fail-closed gate'],
  ['austin: "/Austin/d5021"', 'Austin verified market path'],
  ['`viator-city-${citySlug}`', 'city placement attribution'],
  ['campaign=texasdefined-city-${citySlug}', 'city campaign attribution'],
...
if (cityViator.includes('viator-destination-links') || cityViator.includes('viator-market-match') || cityViator.includes('viatorMarketsForPlace')) {
  errors.push('City Viator booking must keep its verified city path table local and must not pull broader destination or market data into the city monetization chunk.');
}

for (const [needle, label] of [
  ['if (surface === "sports-venue")', 'sports venue fail-closed branch'],
...
console.log('Experience affiliate analytics validation passed: CityPASS retains priority in its three Texas city markets; compact verified direct-market Viator city fallbacks plus destination and statewide Viator surfaces emit first-party click attribution with partner, label, placement, module and page path, preserve disclosure, and use sponsored/nofollow external-link attributes without forced redirects.');
