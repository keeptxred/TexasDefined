...
const contextualCallout = read('src/components/monetization/CityPassContextualCallout.tsx');
const component = [cityPassData, calloutWrapper, calloutContent, contextualCallout].join('\n');
...
requireText(contextualCallout, 'cityPassMarketForSportsVenueSlug', 'AT&T Stadium sports market resolution');
requireText(sportsQuickAnswers, 'CityPassContextualCallout surface="sports-venue" slug={slug}', 'AT&T Stadium contextual callout');
...
requireText(contextualCallout, 'cityPassMarketForCitySlug', 'City guide market resolution');
requireText(entityRoute, 'CityPassContextualCallout surface="city" slug={entity.slug}', 'City guide callout');
requireText(contextualCallout, 'cityPassMarketForDestinationSlug', 'Destination guide market resolution');
requireText(destinationRoute, 'CityPassContextualCallout surface="destination" slug={destination.slug} placement="rail"', 'Destination guide callout');

for (const [source, label] of [
  [destinationRoute, 'Destination route'],
  [entityRoute, 'Entity route'],
  [sportsQuickAnswers, 'Sports venue quick answers'],
]) {
  if (source.includes('from "@/components/monetization/CityPassCallout"') || source.includes("from '@/components/monetization/CityPassCallout'") || source.includes('from "@/data/citypass"') || source.includes("from '@/data/citypass'")) {
    errors.push(`${label} must not synchronously import CityPASS CTA or resolver data into a broad client surface.`);
  }
  requireText(source, 'CityPassContextualCallout', `${label} deferred CityPASS integration`);
}
requireText(destinationRoute, 'import("@/components/monetization/CityPassContextualCallout")', 'Destination route lazy CityPASS boundary');
requireText(entityRoute, "import('@/components/monetization/CityPassContextualCallout')", 'Entity route lazy CityPASS boundary');
requireText(sportsQuickAnswers, "import('@/components/monetization/CityPassContextualCallout')", 'Sports route lazy CityPASS boundary');
