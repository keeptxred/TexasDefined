...
if (fs.existsSync(authorityDataPath)) {
  ...
  for (const hub of hubs) {
    ...
    if (start < 0) errors.push(`Authority dataset is missing ${hub.slug}.`);
...
if (fs.existsSync(searchDocumentsPath)) {
  ...
  for (const pathValue of ['/texas-weather', '/texas-colleges-universities', '/texas-economy']) {
    if (!searchDocuments.includes(`href: \`/${pathValue.slice(1)}\``) && !searchDocuments.includes(`"${pathValue.slice(1)}"`)) {
      errors.push(`Site search discovery missing ${pathValue}.`);
    }
  }
}
...
for (const [label, pathValue] of [
  ...
  if (!page.includes(label) || !page.includes(pathValue)) errors.push(`Texas resources hub must expose ${label} at ${pathValue}.`);
}
