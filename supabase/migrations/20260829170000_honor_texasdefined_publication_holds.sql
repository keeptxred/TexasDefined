-- Keep automatic TexasDefined publication behind the same editorial hold signal
-- used by the shared newsroom preflight. A held story must not appear in the
-- ready queue and the privileged RPC must reject it again under row lock.
...
WHERE f.texasdefined_slug IS NULL
  ...
  AND coalesce(f.preflight_json->>'status', '') NOT IN ('HOLD', 'PUBLICATION_HOLD')
  AND coalesce(f.preflight_json->>'reason', '') NOT IN ('HOLD', 'PUBLICATION_HOLD')
  AND coalesce(trim(f.preflight_json->>'publicationHoldReason'), '') = ''
  AND NOT EXISTS (
...
BEGIN
  ...
  END IF;
  IF coalesce(v_feed.preflight_json->>'status', '') IN ('HOLD', 'PUBLICATION_HOLD')
     OR coalesce(v_feed.preflight_json->>'reason', '') IN ('HOLD', 'PUBLICATION_HOLD')
     OR coalesce(trim(v_feed.preflight_json->>'publicationHoldReason'), '') <> '' THEN
    RAISE EXCEPTION 'Feed item % is on publication hold and requires additional verification', p_feed_id;
  END IF;
