...
import { TEXAS_VS_STATE_GROUPS, texasVsStateName, texasVsStateSlug } from "@/data/texas-vs-state-index";
...
export const Route = createFileRoute("/texas-vs/$state")({
  loader: async ({ params }) => {
    ...
    const { texasVsStateProfile } = await import("@/data/texas-vs-states");
...
function TexasVsStatePage() {
  ...
  const sections = [
    ...
    { heading: "Transportation and daily life", body: `Driving distances, transit, airport access and commute patterns can materially change daily costs. Texas is large, so Dallas–Fort Worth, Houston, Austin, San Antonio, El Paso and rural Texas can produce very different living experiences when compared with ${name}.` },
  ...
  return <main>
    <section className="border-b border-border bg-muted/30 py-14 md:py-20"><Container><p className="eyebrow text-primary">Texas compared</p><h1 className="mt-3 max-w-5xl font-display text-5xl leading-none md:text-7xl">Texas vs {name}</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-muted-foreground">A practical side-by-side framework for comparing Texas with {name}, with state-specific context for the places, climate and tradeoffs that make this comparison different from the other 48.</p></Container></section>

    <section className="py-12 md:py-16"><Container><div className="max-w-4xl divide-y divide-border">
      {sections.map((section, index) => <section key={section.heading} className="py-8 first:pt-0"><p className="eyebrow text-muted-foreground">{String(index + 1).padStart(2, "0")}</p><h2 className="mt-2 font-display text-3xl md:text-4xl">{section.heading}</h2><p className="mt-4 leading-8 text-muted-foreground">{section.body}</p></section>)}
      <section className="py-8"><h2 className="font-display text-3xl md:text-4xl">Check current numbers before deciding</h2><p className="mt-4 leading-8 text-muted-foreground">Housing costs, wages, unemployment and tax rules change. Use current public data for the numbers, then use Texas Defined's city, county and calculator tools to translate statewide averages into the place you would actually live.</p><div className="mt-5 flex flex-wrap gap-x-6 gap-y-3 text-sm"><a href="https://www.census.gov/quickfacts/" target="_blank" rel="noreferrer" className="font-semibold text-primary underline decoration-primary/40 underline-offset-4">U.S. Census QuickFacts ↗</a><a href="https://www.bls.gov/" target="_blank" rel="noreferrer" className="font-semibold text-primary underline decoration-primary/40 underline-offset-4">U.S. Bureau of Labor Statistics ↗</a><Link to="/texas-cost-of-living-calculator" className="font-semibold text-primary underline decoration-primary/40 underline-offset-4">Texas cost-of-living calculator</Link><Link to="/texas-salary-comparison-by-city" className="font-semibold text-primary underline decoration-primary/40 underline-offset-4">Texas salary comparison</Link><Link to="/texas-home-affordability-calculator" className="font-semibold text-primary underline decoration-primary/40 underline-offset-4">Texas home affordability</Link></div></section>
    </div></Container></section>

    <section className="border-t border-border bg-surface py-12"><Container><div className="max-w-4xl"><p className="eyebrow text-primary">Decision questions</p><h2 className="mt-2 font-display text-3xl md:text-4xl">Texas vs {name} FAQ</h2><div className="mt-6 divide-y divide-border border-y border-border">{faq.map((item) => <details key={item.q} className="group py-5"><summary className="cursor-pointer list-none pr-8 font-display text-xl marker:hidden">{item.q}<span className="float-right text-primary group-open:rotate-45">+</span></summary><p className="mt-3 max-w-3xl leading-7 text-muted-foreground">{item.a}</p></details>)}</div></div></Container></section>

    {relatedStates.length > 0 && <section className="border-t border-border py-12"><Container><p className="eyebrow text-primary">Keep comparing</p><h2 className="mt-2 font-display text-3xl md:text-4xl">More Texas vs {relatedGroup?.region} comparisons</h2><p className="mt-3 max-w-3xl leading-7 text-muted-foreground">Compare Texas with nearby or regionally similar states using the same framework, then return to the full 49-state index when you want to widen the search.</p><div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{relatedStates.map((state) => <Link key={state} to="/texas-vs/$state" params={{ state: texasVsStateSlug(state) }} className="border border-border p-4 font-display text-xl hover:border-primary hover:text-primary">Texas vs {state} →</Link>)}</div><Link to="/texas-vs-every-state" className="mt-6 inline-block text-sm font-semibold text-primary underline decoration-primary/40 underline-offset-4">Browse all 49 Texas state comparisons</Link></Container></section>}

    <section className="border-t border-border bg-muted/30 py-10"><Container><div className="flex flex-wrap gap-x-7 gap-y-3 text-sm"><Link to="/texas-vs-every-state" className="font-semibold text-primary">All 49 state comparisons</Link><Link to="/moving-to-texas" className="font-semibold text-primary">Moving to Texas</Link><Link to="/browse/cities" className="font-semibold text-primary">Texas city guides</Link><Link to="/browse/counties" className="font-semibold text-primary">Texas county guides</Link></div></Container></section>
  </main>;
}
