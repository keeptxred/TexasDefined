...
function ArticlePage() {
  ...
  return <article>
    <section className="relative isolate overflow-hidden bg-ink text-ink-foreground"><img src={article.hero.src} alt={article.hero.alt} width={article.hero.width} height={article.hero.height} fetchPriority="high" decoding="async" className="absolute inset-0 size-full object-cover opacity-60" /><div className="absolute inset-0 bg-gradient-to-t from-ink via-ink/70 to-ink/25" /><Container className="relative flex min-h-[60vh] flex-col justify-end pb-14 pt-32"><p className="eyebrow text-ink-foreground/80">{article.category.replace(/-/g, " ")}</p><h1 className="mt-4 max-w-3xl font-display text-4xl leading-[1.08] sm:text-6xl">{article.title}</h1><p className="mt-5 max-w-2xl text-lg leading-relaxed text-ink-foreground/85">{article.dek}</p></Container></section>
