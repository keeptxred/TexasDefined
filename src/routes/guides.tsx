...
const homeNatureGuides = [
  { to: "/texas-hurricane-home-prep", label: "Texas Hurricane Home Prep", body: "Prepare the house, yard, food, documents, generator, pool and evacuation plan before a Gulf storm threatens.", action: "Prepare now" },
  { to: "/texas-pool-guide", label: "Texas Pool Guide", body: "Opening, winterizing, freeze protection, summer operation and storm preparation for pools across Texas climates.", action: "Read the guide" },
  { to: "/texas-pests-guide", label: "Texas Pests", body: "Fire ants, mosquitoes, termites, scorpions, roaches, ticks and the seasonal pest pressures Texas homeowners face.", action: "Identify the problem" },
  { to: "/texas-snakes-guide", label: "Texas Snakes", body: "Recognize venomous groups, reduce risky encounters and understand what to do around snakes in yards, trails and water.", action: "Read the guide" },
  { to: "/texas-wildlife-guide", label: "Texas Wildlife", body: "Coyotes, armadillos, alligators, feral hogs, javelinas and practical guidance for sharing space with wildlife.", action: "Know what to do" },
  { to: "/texas-birds-guide", label: "Texas Birds", body: "Backyard birds, migration, birding seasons and where to start identifying species moving through Texas.", action: "Explore the guide" },
  { to: "/texas-flowers-wildflowers-guide", label: "Texas Flowers & Wildflowers", body: "Bluebonnets, native wildflowers, bloom timing and practical ways to landscape with Texas-adapted plants.", action: "See what blooms" },
] as const;
const travelGuides = [
...
const allFeaturedGuides = [...travelGuides, ...homeNatureGuides, ...practicalGuides];
...
function GuidesPage() {
  ...
  return <>
    ...
    <Section><Container><SectionHeader eyebrow="Texas home & nature" title="Weather, pools, pests, wildlife and what grows here" description="Practical Texas-specific guides for the seasonal problems and everyday encounters that generic homeowner advice usually misses." /><ul className="mt-10 grid gap-x-8 gap-y-0 md:grid-cols-2 lg:grid-cols-4">{homeNatureGuides.map((guide, index) => <li key={guide.to} id={guideAnchor(travelGuides.length + index)} className="border-t border-border py-6"><Link to={guide.to} className="group block"><h2 className="font-display text-2xl leading-tight group-hover:text-primary">{guide.label}</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">{guide.body}</p><span className="eyebrow mt-5 inline-block text-primary">{guide.action} →</span></Link></li>)}</ul></Container></Section>
    <Section tone="surface"><Container><SectionHeader eyebrow="Living here" title="Practical guides for making Texas home" /><ul className="mt-10 grid gap-x-8 gap-y-0 md:grid-cols-2 lg:grid-cols-4">{practicalGuides.map((guide, index) => <li key={guide.to} id={guideAnchor(travelGuides.length + homeNatureGuides.length + index)} className="border-t border-border py-6"><Link to={guide.to} className="group block"><h2 className="font-display text-2xl leading-tight group-hover:text-primary">{guide.label}</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">{guide.body}</p><span className="eyebrow mt-5 inline-block text-primary">{guide.action} →</span></Link></li>)}</ul></Container></Section>
