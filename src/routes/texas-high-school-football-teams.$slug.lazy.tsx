...
function Page() {
  ...
  const associationLabel = program
    ? 'UIL'
    : featured.governingBodyHint ?? 'Association not yet verified';
        ...
        <div>
          {program ? <>
            ...
            <div className="border-y border-border py-5">
              <p className="font-display text-2xl">{featured.governingBodyHint ? `${featured.governingBodyHint} ${featured.associationClassification ?? ''}`.trim() : 'No current UIL alignment match'}</p>
      ...
      {program?.recentHistory && <section className="grid gap-8 border-b border-border py-10 lg:grid-cols-[15rem_1fr]">
        ...
        <div>
          ...
          <div className="mt-5 divide-y divide-border border-y border-border">
            {program.recentHistory.finals.map((final) => <div key={`${final.season}-${final.conference}-${final.result}`} className="grid gap-1 py-4 text-sm sm:grid-cols-[7rem_8rem_1fr]">
