...
export const Route = createFileRoute("/texas-history")({
  head: ({ loaderData }: { loaderData?: { articles: Article[]; destinations: Destination[] } }) => loaderData ? buildEditorialCollectionHead(texasDefinedBrand, {
    canonicalPath: "/texas-history", title: "Texas History", collectionName: "Texas History", description, image: enchantedRock, imageAlt,
    breadcrumbParentName: "Texas Life", breadcrumbParentPath: "/texas-living",
    items: [
...
function TexasHistoryPage() {
  ...
  return <>
    ...
    <Container className="pb-14 sm:pb-18"><div className="grid gap-8 lg:grid-cols-2">{historicSiteClusters.map((cluster) => { const sites = cluster.slugs.map((slug) => bySlug.get(slug)).filter((site): site is Destination => Boolean(site)); if (!sites.length) return null; return <section key={cluster.id} className="border-t-2 border-foreground pt-6"><p className="eyebrow text-primary">{cluster.eyebrow}</p><h2 className="mt-2 font-display text-3xl">{cluster.title}</h2><p className="mt-3 text-sm leading-7 text-muted-foreground">{cluster.description}</p><div className="mt-5 grid gap-3">{sites.map((site) => <Link key={site.slug} to="/destination/$slug" params={{ slug: site.slug }} className="group border-b border-border pb-3"><span className="font-semibold group-hover:text-primary">{site.name}</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">{site.nearestTown}{site.county ? ` · ${site.county} County` : ""}</span></Link>)}</div></section>; })}</div></Container>
    <Container className="pb-16">
      <section className="border-y border-border bg-surface p-6 sm:p-8"><p className="eyebrow text-primary">Immigration · architecture · decorative arts</p><h2 className="mt-3 font-display text-4xl">The Painted Churches are a Texas history network, not just a road trip.</h2><p className="mt-4 max-w-4xl text-sm leading-7 text-muted-foreground">Follow Czech and Moravian, German, Wendish, Polish and Mexican American histories through the statewide church collection.</p><div className="mt-6 flex flex-wrap gap-x-6 gap-y-3 text-sm font-semibold"><Link to="/explore/painted-churches" className="border-b border-primary text-primary">Painted Churches collection</Link><Link to="/explore/painted-churches/heritage" className="border-b border-primary text-primary">Cultural heritage</Link><Link to="/explore/painted-churches/timeline" className="border-b border-primary text-primary">Statewide timeline</Link></div></section>
      <section className="grid gap-8 border-b border-border py-10 lg:grid-cols-[15rem_1fr]"><div><p className="eyebrow text-primary">Living heritage</p><h2 className="mt-2 font-display text-3xl">History you can still eat, hear and visit</h2></div><div className="grid gap-px overflow-hidden border border-border bg-border md:grid-cols-2"><Link to="/german-czech-texas-towns" className="group bg-background p-6"><strong className="font-display text-2xl group-hover:text-primary">German & Czech Texas towns</strong><span className="mt-3 block text-sm leading-6 text-muted-foreground">Connect settlement history with bakeries, meat markets, churches, festivals and architecture.</span></Link><Link to="/texas-dance-halls-honky-tonks" className="group bg-background p-6"><strong className="font-display text-2xl group-hover:text-primary">Texas dance halls & honky-tonks</strong><span className="mt-3 block text-sm leading-6 text-muted-foreground">See how music venues preserve another layer of Texas social history.</span></Link></div></section>
    </Container>
  </>;
}
