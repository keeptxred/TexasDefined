...
const SAN_ANTONIO_ATTRACTIONS = [
  { name: "GO RIO River Cruises", note: "on the San Antonio River Walk", href: "/destination/san-antonio-river-walk" },
  ...
  return <>
      ...
      <Container>
        <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr]">
          ...
          <div className="max-w-2xl space-y-5 text-base leading-8 text-muted-foreground">
            <p>CityPASS® sells multi-attraction tickets in selected cities. Instead of buying every admission separately, you buy one pass and choose from a defined group of participating attractions. In Texas, the current programs are Houston CityPASS® and San Antonio CityPASS®.</p>
    ...
    <Section tone="surface">
      <Container>
        ...
        <div className="mt-10 grid gap-8 md:grid-cols-3">
          ...
          <Decision number="03" title="Can you use the pass comfortably?" body="Both Texas programs currently use a nine-day validity window beginning with first use. Check reservation rules and your travel dates before purchase." />
        </div>
      </Container>
    </Section>
    ...
    <Section tone="surface">
      <Container>
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,.55fr)]">
          <div>
            <SectionHeader eyebrow="San Antonio CityPASS®" title="Useful when downtown attractions are already the plan" description="As reviewed September 2026, San Antonio CityPASS® lets travelers choose four attractions from the current participating lineup and use the pass across a nine-day window." />
    ...
    <Section>
      <Container>
        ...
        <div className="mt-10 grid gap-x-10 gap-y-8 md:grid-cols-2">
          ...
          <Check title="Reservations constrain the itinerary" body="Some included attractions may require advance reservations or timed entry. Check the current rules before assuming every stop can be visited whenever you arrive." />
        </div>
      </Container>
    </Section>
...
function AttractionList({ items }: { items: readonly { name: string; href?: string; note?: string }[] }) {
  return <ul className="mt-8 grid gap-x-8 sm:grid-cols-2">{items.map((item) => <li key={item.name} className="border-t border-border py-4 text-sm leading-6"><span className="font-semibold text-foreground">{item.href ? <a href={item.href} className="underline decoration-primary/40 underline-offset-4 hover:text-primary">{item.name}</a> : item.name}</span>{item.note ? <span className="text-muted-foreground"> — {item.note}</span> : null}</li>)}</ul>;
}
