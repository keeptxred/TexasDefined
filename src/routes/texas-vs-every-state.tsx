  ...
  head: () => ({
    ...
    scripts: [jsonLd({
      ...
      "@graph": [
        ...
        { "@type": "ItemList", "@id": `${pageUrl}#states`, name: "Texas compared with every other U.S. state", numberOfItems: states.length, itemListElement: states.map((state, index) => ({ "@type": "ListItem", position: index + 1, name: `Texas vs ${state}`, url: `${pageUrl}#texas-vs-${slug(state)}` })) },
...
function TexasVsEveryStatePage() {
  return <main>
    ...
    <section className="border-y border-border bg-muted/30 py-12 md:py-16"><Container><h2 className="font-display text-4xl">Texas vs all 49 states</h2><p className="mt-3 max-w-3xl text-muted-foreground">Jump to any state. These entries establish the statewide comparison index; deeper individual comparisons can be expanded from the same methodology as data coverage grows.</p><div className="mt-8 grid gap-8 md:grid-cols-2 lg:grid-cols-4">{stateGroups.map((group) => <section key={group.region}><h3 className="eyebrow text-primary">{group.region}</h3><ul className="mt-4 divide-y divide-border border-y border-border">{group.states.map((state) => <li key={state}><a href={`#texas-vs-${slug(state)}`} className="block py-3 font-display text-lg hover:text-primary">Texas vs {state}</a></li>)}</ul></section>)}</div></Container></section>

    <section className="py-12 md:py-16"><Container><h2 className="font-display text-4xl">The comparison index</h2><div className="mt-8 divide-y divide-border border-y border-border">{stateGroups.flatMap((group) => group.states.map((state) => <article key={state} id={`texas-vs-${slug(state)}`} className="scroll-mt-24 py-7 md:grid md:grid-cols-[18rem_1fr] md:gap-8"><div><p className="eyebrow text-muted-foreground">State comparison</p><h3 className="mt-2 font-display text-2xl">Texas vs {state}</h3></div><div><p className="leading-7 text-muted-foreground">Compare Texas and {state} using the same core measures: housing and cost of living, income and employment, state and local taxes, climate and geography, transportation, utilities and quality-of-life considerations. Keeping the methodology consistent matters more than cherry-picking one statistic.</p><div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-sm"><Link to="/texas-data" className="text-primary hover:underline">Explore Texas data</Link><Link to="/moving-to-texas" className="text-primary hover:underline">Moving to Texas</Link><Link to="/texas-cost-of-living-calculator" className="text-primary hover:underline">Cost-of-living calculator</Link></div></div></article>))}</div></Container></section>
