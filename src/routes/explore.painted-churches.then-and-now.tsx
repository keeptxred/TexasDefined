...
const description = "Historic Texas Painted Church records paired with current rights-cleared photography, with archival provenance, image-rights status and restoration context.";
...
export const Route = createFileRoute(canonicalPath)({
  head: () => ({
    ...
    scripts: [jsonLd({
      ...
      "@graph": [
        { "@type": "CollectionPage", "@id": `${pageUrl}#page`, url: pageUrl, name: "Texas Painted Churches Then & Now", description, mainEntity: { "@id": `${pageUrl}#pairs` } },
...
function ThenAndNowIndex() {
  return <main>
    <section className="border-b border-border bg-surface"><Container className="py-16 sm:py-24">
      ...
      <p className="mt-6 max-w-4xl text-lg leading-8 text-muted-foreground">Historic records are paired only with current church-specific photographs whose reuse terms have been verified. When an archive does not grant republication rights, Texas Defined links the historic item instead of copying it.</p>
    </Container></section>

    <Container className="py-14 sm:py-18">
      <section className="grid gap-5 md:grid-cols-3">
        <Stat value={paired.length} label="Churches with archival + current pairs" />
        <Stat value={archivalOnly.length} label="Archival evidence awaiting cleared current image" />
        <Stat value={currentOnly.length} label="Current image awaiting archival counterpart" />
      </section>
