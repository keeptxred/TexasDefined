...
function TexasDefinedNews() {
  const { data = [], isLoading } = useQuery({
    queryKey: ["texasdefined-live-news"],
  ...
  return <Container className="py-12 sm:py-16">
    ...
    {isLoading ? <p className="mt-10 text-sm text-muted-foreground">Loading current stories…</p> : data.length ? <ul className="mt-12 divide-y divide-border border-y border-border">{data.map((article) => <li key={article.slug} className="py-7">
      <Link to="/news/$slug" params={{ slug: article.slug }} className="group grid gap-5 sm:grid-cols-[12rem_1fr]">
        <img src={article.hero.src} alt={article.hero.alt} width={article.hero.width} height={article.hero.height} loading="lazy" className="aspect-[16/10] w-full object-cover" />
