...
const faqs = [
  ...
  {
    question: 'Should I include HOA, MUD or PID costs in the mortgage payment?',
    answer: 'Include them in the household budget even when they are not technically part of principal and interest. HOA dues, district assessments and other recurring charges can change the amount of cash the property requires each month.',
  },
  {
    question: 'Is this the same as a lender loan estimate?',
    answer: 'No. It is a planning tool. A lender uses current loan terms, credit, fees, escrow rules and property-specific information. Use this calculator to compare scenarios before you have the final Loan Estimate or closing disclosure.',
  },
];

export const Route = createLazyFileRoute('/texas-mortgage-calculator')({
  ...
  return (
    <CalculatorPage eyebrow="Texas mortgage payment calculator" title="Texas mortgage payment calculator with taxes and insurance" description={description}>
      ...
      <section className="mt-12 border-t border-border pt-10" aria-labelledby="mortgage-scenarios-heading">
        ...
        <div className="mt-5 max-w-3xl space-y-4 text-base leading-7 text-muted-foreground">
          <p>Two $400,000 homes can produce different monthly costs because the addresses can fall into different school districts, cities, MUDs, PIDs or other taxing units. Insurance exposure, HOA dues and utility providers may differ too.</p>
          <p>For each serious property, save or note the exact assumptions you used: purchase price, down payment, interest rate, tax-rate stack, insurance estimate and recurring neighborhood costs. That makes the comparison reproducible instead of relying on a single mortgage number from a listing site.</p>
          <p>If you are deciding how much to put down, compare the effect on both principal-and-interest and your remaining cash reserves. A lower monthly payment is not automatically better if it leaves too little cash for closing costs, repairs or emergencies.</p>
        </div>
      </section>

      <section className="mt-12 border-t border-border pt-10" aria-labelledby="mortgage-official-heading">
        <p className="eyebrow text-primary">Verify before closing</p>
        <h2 id="mortgage-official-heading" className="mt-3 font-display text-3xl">Use official documents for the final numbers</h2>
        <div className="mt-5 max-w-3xl space-y-4 text-base leading-7 text-muted-foreground">
          <p>Use this calculator for planning, then replace estimates with the lender's disclosures, the appraisal district record, adopted taxing-unit rates, the insurer's quote and the title or closing documents. Those property-specific sources control the real transaction.</p>
          <div className="flex flex-wrap gap-x-6 gap-y-3 text-sm font-semibold">
            <a href="https://www.consumerfinance.gov/owning-a-home/loan-estimate/" target="_blank" rel="noreferrer" className="text-primary underline decoration-primary/40 underline-offset-4">CFPB Loan Estimate guide ↗</a>
            <a href="https://comptroller.texas.gov/taxes/property-tax/rates/" target="_blank" rel="noreferrer" className="text-primary underline decoration-primary/40 underline-offset-4">Texas Comptroller property-tax rates ↗</a>
          </div>
        </div>
      </section>
