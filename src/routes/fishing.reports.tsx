...
export const Route = createFileRoute("/fishing/reports")({
  ...
  head: ({ loaderData }) => {
    ...
    return { meta: buildMeta(texasDefinedBrand, { title: "Texas Fishing Reports — Dates & Freshness Labels", description, canonicalPath: FISHING_REPORTS_DIRECTORY_PATH, robots: reports.length ? undefined : "noindex, follow" }), links: [canonicalLink(texasDefinedBrand, FISHING_REPORTS_DIRECTORY_PATH)], scripts: [{ type: "application/ld+json", children: JSON.stringify([
      ...
      { "@context": "https://schema.org", "@type": "ItemList", name: "Texas fishing reports", numberOfItems: reports.length, itemListElement: reports.map((entry, index) => ({ "@type": "ListItem", position: index + 1, name: entry.report.title, url: `${origin}${entry.href}` })) },
