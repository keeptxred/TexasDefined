...
const texasExplainedPicks = [
  ["The Rivers That Built Texas", "/article/texas-rivers-explained"],
  ["Farm-to-Market Roads Explained", "/article/texas-farm-to-market-roads-explained"],
  ["The Cultural Regions of Texas", "/article/texas-cultural-regions-explained"],
] as const;
const sportsTravelPicks = [
  ["Statewide directory", "Texas Sports Venues", "/sports-venues"],
  ["North Texas", "Dallas–Fort Worth Sports Venues", "/sports-venues/dallas-fort-worth"],
  ["Texas football", "Football Stadiums", "/sports-venues/football"],
  ["Race weekends", "Texas Motorsports Venues", "/sports-venues/motorsports"],
] as const;
...
function HomePage() {
  ...
  return <>
    ...
    <Section><Container><SectionHeader eyebrow="Texas Explained" title="Why Texas works the way it does" description="Ten connected guides to the water, roads, towns, landscapes, wildlife, homes, land and migration patterns behind everyday Texas." actionLabel="Read all 10 guides" actionTo="/texas-explained" /><ul className="mt-10 grid gap-0 border-y border-border md:grid-cols-3 md:divide-x md:divide-border">{texasExplainedPicks.map(([title, to], index) => <li key={to} className={index > 0 ? "border-t border-border md:border-t-0" : undefined}><Link to={to} className="group block h-full px-1 py-7 md:px-6"><p className="eyebrow text-primary">Guide {String(index + 1).padStart(2, "0")}</p><h3 className="mt-3 font-display text-2xl leading-tight transition-colors group-hover:text-primary">{title}</h3><span className="eyebrow mt-5 inline-block text-primary">Read the guide →</span></Link></li>)}</ul></Container></Section>
    ...
    <Section tone="surface"><Container><SectionHeader eyebrow="Texas sports travel" title="Make game day part of the trip" description="Start with a statewide venue directory, then narrow by market or sport." actionLabel="Browse all sports venues" actionTo="/sports-venues" /><ul className="mt-10 grid gap-0 border-y border-border sm:grid-cols-2 lg:grid-cols-4 lg:divide-x lg:divide-border">{sportsTravelPicks.map(([eyebrow, title, to], index) => <li key={to} className={`${index > 0 ? "border-t border-border sm:border-t-0" : ""} ${index >= 2 ? "sm:border-t sm:border-border lg:border-t-0" : ""}`}><Link to={to} className="group block h-full px-1 py-7 sm:px-5 lg:px-6"><p className="eyebrow text-primary">{eyebrow}</p><h2 className="mt-3 font-display text-2xl leading-tight transition-colors group-hover:text-primary">{title}</h2><span className="eyebrow mt-5 inline-block text-primary">Open the guide →</span></Link></li>)}</ul></Container></Section>
