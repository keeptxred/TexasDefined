...
function HomePage() {
  ...
  return <>
    <header className="border-b border-border bg-surface px-5 py-8 sm:px-8"><div className="mx-auto max-w-6xl"><p className="eyebrow text-primary">Texas Defined</p><h1 className="mt-3 font-display text-4xl font-semibold leading-tight sm:text-5xl">The places, stories & life of Texas</h1><p className="mt-3 max-w-3xl text-base leading-7 text-muted-foreground">Texas Defined is a guide to Texas places, culture, food, history, travel and practical living for Texans, newcomers and visitors.</p><dl className="mt-6 grid gap-5 md:grid-cols-2">{homepageFaqs.map((item) => <div key={item.question}><dt><h2 className="font-display text-lg font-semibold">{item.question}</h2></dt><dd className="mt-1 text-sm leading-6 text-muted-foreground">{item.answer}</dd></div>)}</dl></div></header>
    {hero && <>
      <section className="border-b border-border bg-background">
        <Container>
          <div className="py-7 sm:py-9">
            <p className="eyebrow text-primary">Featured this month on Texas Defined</p>
            <p className="mt-3 max-w-4xl font-display text-2xl leading-tight sm:text-3xl">This month’s featured story is <span className="font-semibold">{hero.title}</span>.</p>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground sm:text-base">A closer look at one of the people, places, traditions, or experiences that helps define Texas.</p>
          </div>
        </Container>
      </section>
      <FeatureHero variant="split" eyebrow="Read the feature" title={hero.title} dek={hero.dek} image={hero.hero} to="/article/$slug" params={{ slug: hero.slug }} meta={formatReadingTime(hero.readingMinutes)} />
    </>}
    {featuredDestinations.length > 0 && <Section><Container><SectionHeader eyebrow="Editor's picks" title="A Different Side of Texas" description="Handpicked places, landscapes and stops that are worth seeing for yourself." actionLabel="Explore the full guide" actionTo="/explore" /><ul className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">{featuredDestinations.map((destination) => <li key={destination.id}><DestinationCard destination={destination} tone="overlay" regionLabel={regionName(destination.region)} /></li>)}</ul></Container></Section>}
    ...
    {worthTheDrive && <Section tone="surface"><Container><div className="grid items-center gap-10 lg:grid-cols-[1.2fr_0.8fr]"><div className="relative aspect-[16/10] overflow-hidden bg-muted"><span aria-hidden className="eyebrow absolute left-5 top-5 text-muted-foreground">Photo unavailable</span><img src={worthTheDrive.hero.src} alt={worthTheDrive.hero.alt} width={worthTheDrive.hero.width} height={worthTheDrive.hero.height} loading="lazy" decoding="async" className="absolute inset-0 size-full object-cover" onError={(event) => recoverOrHideImage(event.currentTarget)} /></div><div><p className="eyebrow text-primary">Worth the drive</p><h2 className="mt-3 font-display text-4xl leading-tight sm:text-5xl">{worthTheDrive.name}</h2><p className="mt-5 text-base leading-7 text-muted-foreground">{worthTheDrive.summary}</p><Link to="/destination/$slug" params={{ slug: worthTheDrive.slug }} className="eyebrow mt-7 inline-block border-b-2 border-primary pb-1 text-primary">Plan the trip</Link></div></div></Container></Section>}
