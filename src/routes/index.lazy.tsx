...
function HomePage() {
  ...
  return <>
    ...
    {featuredDestinations.length > 0 && <Section><Container><SectionHeader eyebrow="Editor's picks" title="Texas worth exploring" description="A considered mix of landscapes, towns and stops that show a different side of the state." actionLabel="Explore the full guide" actionTo="/explore" /><ul className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">{featuredDestinations.map((destination) => <li key={destination.id}><DestinationCard destination={destination} tone="overlay" regionLabel={regionName(destination.region)} /></li>)}</ul></Container></Section>}
