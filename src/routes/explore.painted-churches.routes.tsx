...
export const Route = createFileRoute(canonicalPath)({
  loader: async () => {
    ...
    return { itineraries: paintedChurchItineraries };
  },
  head: ({ loaderData }) => {
    const itineraries = loaderData?.itineraries ?? [];
    return {
      meta: buildMeta(texasDefinedBrand, { canonicalPath, title: "Texas Painted Churches Routes & Itineraries", description }),
      links: [canonicalLink(texasDefinedBrand, canonicalPath)],
      scripts: [jsonLd({ "@context": "https://schema.org", "@type": "ItemList", "@id": `${pageUrl}#routes`, name: "Texas Painted Churches routes", numberOfItems: itineraries.length, itemListElement: itineraries.map((item, index) => ({ "@type": "ListItem", position: index + 1, item: { "@type": "TouristTrip", name: item.name, description: item.summary, url: `${pageUrl}/${item.slug}` } })) })],
    };
  },
  component: RoutesHub,
});
