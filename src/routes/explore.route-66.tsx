...
const canonicalPath = "/explore/route-66";
const description = "Drive the complete Texas stretch of Historic Route 66 from the Oklahoma line to Glenrio with 13 linked stop guides, landmark planning, 1–3 day itineraries and official historic-road sources.";
...
export const Route = createFileRoute("/explore/route-66")({
  head: () => {
  ...
  return <>
    ...
    <Container className="py-12 sm:py-16">
      <div className="grid gap-10 lg:grid-cols-[0.9fr_1.6fr]">
        <div>
          <p className="eyebrow text-primary">Before mile one</p>
          <h2 className="mt-2 font-display text-4xl">Start at the state line, not in Amarillo</h2>
        </div>
          ...
          <li key={stop.slug} className="grid gap-4 border-b border-border py-7 sm:grid-cols-[5rem_1fr_auto] sm:items-start">
            ...
            <div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {stop.highlights.slice(0, 3).map((highlight) => <span key={highlight} className="border border-border px-3 py-1.5 text-xs text-muted-foreground">{highlight}</span>)}
              </div>
            </div>
    ...
    <Container className="py-12 sm:py-16">
      <div className="max-w-3xl">
        <p className="eyebrow text-primary">Choose your pace</p>
        <h2 className="mt-2 font-display text-4xl sm:text-5xl">One, two or three days</h2>
      </div>
      <div className="mt-8 grid gap-5 lg:grid-cols-3">
        {itineraryPlans.map((plan) => (
          <article key={plan.label} className="border border-border p-6 sm:p-7">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">{plan.label}</p>
            <h3 className="mt-3 font-display text-3xl">{plan.title}</h3>
            <p className="mt-4 text-sm leading-7 text-muted-foreground">{plan.text}</p>
          </article>
        ))}
      </div>
    </Container>
    ...
    <Container className="py-12 sm:py-16">
      <div className="grid gap-10 lg:grid-cols-[1fr_1.4fr]">
        <div>
          <p className="eyebrow text-primary">Don't miss these</p>
          <h2 className="mt-2 font-display text-4xl">The anchor experiences</h2>
          <p className="mt-4 text-sm leading-7 text-muted-foreground">If time forces you to shorten the trip, preserve the places that explain different chapters of the highway rather than choosing only roadside-photo stops.</p>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          {[
            ["Shamrock", "U-Drop Inn gives the eastern end a restored Art Deco anchor."],
            ["McLean", "Museum and commercial-district history make the highway economy tangible."],
            ["Groom", "The leaning tower is pure roadside advertising; the cross is a modern landmark of a different scale."],
            ["Amarillo", "Sixth Street, Cadillac Ranch and the city's lodging/food base reward the longest stop."],
            ["Vega", "The Magnolia Station and historic motel connect the western Panhandle to early motor travel."],
            ["Adrian + Glenrio", "The symbolic midpoint and the state-line ghost town make a memorable final act."],
          ].map(([name, text]) => (
    ...
    <Container className="py-12 sm:py-16">
      <div className="max-w-3xl">
        <p className="eyebrow text-primary">Road reality</p>
        <h2 className="mt-2 font-display text-4xl">What to know before following the old alignment</h2>
      </div>
      <ul className="mt-8 grid gap-4 md:grid-cols-2">
        {practicalNotes.map((note) => <li key={note} className="border-l-2 border-primary pl-5 text-sm leading-7 text-muted-foreground">{note}</li>)}
      </ul>
    </Container>

    <Container className="py-12 sm:py-16">
      <div className="border-y border-border py-8">
        <p className="eyebrow text-primary">Add the Panhandle</p>
        <h2 className="mt-2 font-display text-4xl">Two TexasDefined detours worth adding</h2>
        <div className="mt-6 grid gap-5 sm:grid-cols-2">
          <Link to="/destination/cadillac-ranch" className="group border border-border p-6 transition-colors hover:bg-muted">
            <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">On the route · Amarillo</p>
            <h3 className="mt-2 font-display text-3xl group-hover:text-primary">Cadillac Ranch →</h3>
            <p className="mt-3 text-sm leading-7 text-muted-foreground">Our full destination guide for Amarillo's famous roadside art installation.</p>
          </Link>
          <Link to="/destination/palo-duro-canyon" className="group border border-border p-6 transition-colors hover:bg-muted">
            <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Side trip · south of Amarillo</p>
            <h3 className="mt-2 font-display text-3xl group-hover:text-primary">Palo Duro Canyon →</h3>
            <p className="mt-3 text-sm leading-7 text-muted-foreground">Turn the Route 66 crossing into a three-day Panhandle trip with a real canyon day.</p>
          </Link>
        </div>
      </div>
    </Container>
    ...
    <Container className="py-12 sm:py-16">
      <p className="eyebrow text-primary">Primary research</p>
      <h2 className="mt-2 font-display text-4xl">Build the trip from authoritative Route 66 sources</h2>
