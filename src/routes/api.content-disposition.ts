  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (input.targetSite && input.targetSite !== 'TexasDefined') return response({ error: 'This endpoint only evaluates publication to TexasDefined.' }, 400);
        ...
        return response({ mode: 'preview-only', targetSite: 'TexasDefined', decision: decideCrossSiteContent(candidate), candidate }, 200);
      },
    },
  },
});
