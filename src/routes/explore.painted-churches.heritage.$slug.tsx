    ...
    <main>
      <section className="border-b border-border bg-surface">
        <Container className="py-16 sm:py-24">
          ...
          <p className="eyebrow mt-8 text-primary">Painted Churches heritage</p>
          <h1 className="mt-4 max-w-5xl font-display text-5xl leading-[0.98] sm:text-7xl">{heritage.name}</h1>
          <p className="mt-6 max-w-4xl text-lg leading-8 text-muted-foreground">{heritage.answer}</p>
        </Container>
      </section>
      <Container className="py-14 sm:py-18">
        <section className="border-t-2 border-foreground pt-8"><p className="eyebrow text-primary">Historical context</p><ul className="mt-5 list-disc space-y-3 pl-5 text-base leading-8 text-muted-foreground">{heritage.context.map((item) => <li key={item}>{item}</li>)}</ul></section>
        <section className="mt-14 border-t border-border pt-8">
          <p className="eyebrow text-primary">How to read heritage without overclaiming</p>
          <div className="mt-5 max-w-4xl space-y-5 text-base leading-8 text-muted-foreground">
            <p>Heritage is one layer of a Painted Church story, not a shortcut for identifying every feature in the building. A congregation&apos;s migration history, language, devotional traditions and community institutions can help explain why a church was built and sustained, but architectural style and decorative authorship still need their own evidence.</p>
            <p>Texas Defined therefore connects a heritage page only to churches supported by the cited community, parish or historical record. We do not infer a community identity from a modern town name, surname or visual motif alone. Where several cultural histories overlap, the church profile and its source trail take precedence over a broad regional label.</p>
            <p>For visitors, the useful comparison is between documented communities rather than stereotypes. Read the connected church profiles side by side, note differences in dates, artists, architecture and later preservation, and use the authority source below when a historical detail matters to research or publication.</p>
          </div>
        </section>
        ...
        <section className="mt-14 border-t border-border pt-8"><p className="eyebrow text-primary">Authority source</p><a href={heritage.sourceUrl} target="_blank" rel="noreferrer" className="mt-3 inline-block font-display text-2xl text-primary hover:underline">{heritage.sourceLabel}</a><p className="mt-3 max-w-3xl text-sm leading-7 text-muted-foreground">Texas Defined uses community, parish and heritage-organization sources to keep migration history distinct from assumptions based only on church names or modern geography.</p></section>
      </Container>
    </main>
  );
}
