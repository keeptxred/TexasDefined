...
export const Route = createLazyFileRoute("/explore/top-attractions")({
        ...
        <ol className="mt-12 grid gap-10 sm:grid-cols-2 lg:grid-cols-3">
          {attractions.map(({ rank, destination }) => {
            ...
            return <li key={destination.slug} className="relative">
              <div className="mb-3 flex items-baseline gap-3 border-b border-border pb-3"><span className="font-display text-4xl leading-none text-primary">{String(rank).padStart(2, "0")}</span><span className="eyebrow text-muted-foreground">Top Texas attraction</span></div>
              <DestinationCard destination={destination} />
              {assessment && <dl className="mt-4 grid grid-cols-2 gap-x-5 gap-y-3 border-y border-border py-4 text-xs"><div><dt className="uppercase tracking-[0.1em] text-muted-foreground">Allow</dt><dd className="mt-1 font-medium leading-5">{assessment.recommendedVisit}</dd></div><div><dt className="uppercase tracking-[0.1em] text-muted-foreground">Effort</dt><dd className="mt-1 font-medium">{assessment.physicalEffort}</dd></div><div><dt className="uppercase tracking-[0.1em] text-muted-foreground">Planning</dt><dd className="mt-1 font-medium">{assessment.planningLevel}</dd></div><div><dt className="uppercase tracking-[0.1em] text-muted-foreground">Evidence</dt><dd className="mt-1 font-medium">{sourceCount} authority {sourceCount === 1 ? "source" : "sources"}</dd></div></dl>}
            </li>;
          })}
        </ol>
      </Container>
