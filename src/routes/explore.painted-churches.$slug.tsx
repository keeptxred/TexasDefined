...
export const Route = createFileRoute("/explore/painted-churches/$slug")({
  ...
  head: ({ loaderData, params }) => {
    ...
    const heroImage = church.image ?? gallery[0];
    const churchSchema = {
      "@type": "Church",
      "@id": `${url}#church`,
      name: church.name,
    ...
    <main>
      ...
      <section className="mt-5 border-y border-border bg-ink text-ink-foreground">
        <Container className="grid gap-10 py-12 sm:py-16 lg:grid-cols-[minmax(0,1.2fr)_minmax(340px,.72fr)] lg:items-center">
          ...
          {heroImage ? (
            <figure>
              <img src={heroImage.src} alt={heroImage.alt} width={heroImage.width} height={heroImage.height} fetchPriority="high" decoding="async" className="aspect-[4/3] w-full object-cover" />
              <figcaption className="mt-3 text-xs leading-5 text-ink-foreground/60">
                {heroImage.credit} · {heroImage.license} · <a href={heroImage.sourceUrl} target="_blank" rel="noreferrer" className="border-b border-ink-foreground/40">source</a>
              </figcaption>
            </figure>
          ) : (
