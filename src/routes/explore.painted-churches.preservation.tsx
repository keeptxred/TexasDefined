...
import { paintedChurchPreservationChronologyGaps, paintedChurchPreservationEvents } from "@/data/painted-church-preservation-chronology";
...
function PreservationHub() {
  ...
  const chronologyChurchCount = new Set(paintedChurchPreservationEvents.map((event) => event.churchSlug)).size;
  const recentEvents = [...paintedChurchPreservationEvents].sort((a, b) => b.year - a.year || a.id.localeCompare(b.id)).slice(0, 18);
  return <main>
    ...
    <Container className="py-14 sm:py-18">
      ...
      <section className="mt-14 border-t border-border pt-8"><p className="eyebrow text-primary">Intervention coverage</p><h2 className="mt-3 font-display text-4xl">{chronologyChurchCount} churches currently have a sourced preservation/fabric chronology.</h2><p className="mt-4 max-w-4xl text-sm leading-7 text-muted-foreground">An intervention chronology records events such as fire, storm damage, whitewashing, repainting, rediscovery, restoration, reconstruction or stewardship change. Churches without a chronology remain explicitly in the research queue; absence of a record never means Texas Defined is claiming the interior was never altered.</p></section>
