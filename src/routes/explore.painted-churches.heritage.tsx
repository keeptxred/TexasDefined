...
export const Route = createFileRoute(canonicalPath)({
  loader: async () => {
    const { paintedChurchHeritage } = await import("@/data/painted-church-heritage");
    return paintedChurchHeritage;
  },
  head: ({ loaderData }) => {
    ...
    return {
      ...
      scripts: [jsonLd({ "@context": "https://schema.org", "@graph": [
        ...
        { "@type": "ItemList", "@id": `${pageUrl}#heritage`, numberOfItems: heritage.length, itemListElement: heritage.map((item, index) => ({ "@type": "ListItem", position: index + 1, item: { "@type": "WebPage", name: item.name, url: `${pageUrl}/${item.slug}`, description: item.answer } })) },
      ] })],
    };
  },
