...
function TexasExplainedPage() {
  ...
  return <>
    ...
    <Container className="py-12 sm:py-16">
      ...
      <section id="quick-answers" className="mt-12 scroll-mt-28" aria-labelledby="texas-explained-quick-answers"><header className="max-w-3xl"><p className="eyebrow text-primary">Quick answers</p><h2 id="texas-explained-quick-answers" className="mt-2 font-display text-3xl leading-tight sm:text-4xl">Five Texas questions, answered before you dive deeper</h2><p className="mt-4 text-base leading-7 text-muted-foreground">Each answer is the short version. The linked guide explains the history, geography and practical details behind it.</p></header><dl className="mt-8 grid border-t border-border md:grid-cols-2">{quickAnswers.map((item, index) => <div key={item.question} className={`border-b border-border py-7 md:px-6 ${index % 2 === 1 ? "md:border-l" : ""}`}><dt className="font-display text-2xl leading-tight">{item.question}</dt><dd className="mt-3 text-sm leading-7 text-muted-foreground">{item.answer}</dd><Link to={item.to} className="eyebrow mt-5 inline-block border-b border-primary py-1 text-primary">{item.label} →</Link></div>)}</dl></section>
