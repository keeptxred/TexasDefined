...
export const Route = createLazyFileRoute("/explore/$category")({ component: ExploreCategoryPage, notFoundComponent: CategoryNotFound });
...
function ExploreCategoryPage() {
  ...
  const unusualBusinessSpotlight = match.slug === "small-towns"
    ? {
        ...
        text: "Step inside a working blacksmith shop, learn how hand-forged blades are made and connect the visit to Bellville and Austin County.",
        href: "/article/phenix-knives-bellville",
        cta: "Explore Phenix Knives →",
      }
    : match.slug === "beaches-coast"
      ? {
          eyebrow: "A different way to experience the coast",
          title: "Horses on the Beach in Corpus Christi",
          text: "Ride the Padre Island shoreline on horseback, then connect the experience to North Padre Island, Nueces County and the wider Coastal Bend.",
          href: "/article/horses-on-the-beach-corpus-christi",
          cta: "Plan the beach ride →",
        }
      : null;
  ...
  return <>
    ...
    {unusualBusinessSpotlight ? <Container className="pb-10 sm:pb-14"><section className="grid gap-6 border-y border-border py-7 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div><p className="eyebrow text-primary">{unusualBusinessSpotlight.eyebrow}</p><h2 className="mt-2 font-display text-3xl">{unusualBusinessSpotlight.title}</h2><p className="mt-3 max-w-3xl text-sm leading-7 text-muted-foreground">{unusualBusinessSpotlight.text}</p><Link to="/article/unusual-texas-businesses-services" className="mt-4 inline-block text-sm font-semibold underline decoration-primary/40 underline-offset-4 hover:text-primary">More unusual Texas businesses →</Link></div><Link to={unusualBusinessSpotlight.href} className="eyebrow inline-block border-b border-primary pb-1 text-primary">{unusualBusinessSpotlight.cta}</Link></section></Container> : null}
