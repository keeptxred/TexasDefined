  ...
  return <Container className="pb-10 sm:pb-14">
    <section className="border-y border-border py-8">
      ...
      <div className="mt-7 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {FOOD_DESTINATIONS.map((destination) => <a key={destination.slug} href={`/food/${destination.slug}`} className="group border border-border p-5 transition-colors hover:border-primary/60">
          ...
          <p className="mt-4 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Known for: {destination.knownFor.slice(0, 2).join(" · ")}</p>
        </a>)}
      </div>
    </section>
  </Container>;
