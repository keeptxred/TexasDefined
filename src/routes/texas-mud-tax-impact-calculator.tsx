...
const DEFAULTS: MudState = { value: 400000, rate: 0, years: 10 };
...
function Page() {
  ...
  return <Container className="pb-16 pt-12 sm:pb-24 sm:pt-16"><article className="mx-auto max-w-6xl">
    ...
    <CalculatorSection eyebrow="Inputs" title="Set value and planning horizon"><div className="grid gap-5 md:grid-cols-3"><CurrencyInput label="Taxable value" value={state.value} onChange={(value) => update('value', value)}/><PercentageInput label="District tax rate" value={state.rate} onChange={(value) => update('rate', value)} step={.0001} help="Search an official district above or enter a verified parcel-applicable rate."/><NumberInput label="Years at same rate" value={state.years} onChange={(value) => update('years', value)} min={1} max={100} suffix="years"/></div></CalculatorSection>
