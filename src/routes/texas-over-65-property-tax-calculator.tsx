...
function Page() {
  ...
  const hasLocalRates = state.schoolRate > 0 || state.otherRate > 0;
  ...
  return <Container className="pb-16 pt-12 sm:pb-24 sm:pt-16">
    <article className="mx-auto max-w-6xl">
      ...
      <CalculatorSection eyebrow="Location" title="Load the latest finalized local taxing-unit rates">
        ...
        />
        <p className="mt-4 max-w-3xl text-sm leading-6 text-muted-foreground">The rate year shown in the results comes from the local-rate record you apply. The statewide exemption amounts and the local taxing-unit rate year are separate inputs, so do not relabel an older finalized rate as a newer rate year.</p>
      </CalculatorSection>
      ...
      <section className="border-y border-border py-10">
        <ResultGrid>
          <CalculatorResult label="Estimated school tax" value={formatMoney(result.capped)} note={hasLocalRates ? `${state.rateYear} rate scenario` : "Choose or enter local rates"} />
