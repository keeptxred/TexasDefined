...
export const Route = createFileRoute("/regions/$region")({
  loader: async ({ context, params }) => {
    ...
    return { region, presentation, subregions, metros, places, countySlugs, destinations, adjacent, allRegions: geography.CANONICAL_PRIMARY_REGIONS };
  },
  head: ({ loaderData }) => {
    ...
    return {
      ...
      scripts: [jsonLd({
        ...
        "@graph": [
          { "@type": "CollectionPage", "@id": `${pageUrl}#page`, url: pageUrl, name: title, description, isPartOf: { "@id": `${siteUrl}/#website` }, publisher: { "@id": `${siteUrl}/#organization` }, breadcrumb: { "@id": `${pageUrl}#breadcrumb` }, mainEntity: { "@id": `${pageUrl}#region` } },
          { "@type": "Place", "@id": `${pageUrl}#region`, name: loaderData.region.name, description: loaderData.presentation.identity, containedInPlace: { "@type": "State", name: "Texas" }, containsPlace: loaderData.places.slice(0, 30).map((place) => ({ "@type": "City", name: `${place.name}, Texas` })) },
          { "@type": "ItemList", "@id": `${pageUrl}#destinations`, name: `Places to explore in ${loaderData.region.name}`, numberOfItems: loaderData.destinations.length, itemListElement: loaderData.destinations.slice(0, 50).map((destination, index) => ({ "@type": "ListItem", position: index + 1, item: destinationSchema(destination) })) },
          { "@type": "BreadcrumbList", "@id": `${pageUrl}#breadcrumb`, itemListElement: [{ "@type": "ListItem", position: 1, name: "Front page", item: `${siteUrl}/` }, { "@type": "ListItem", position: 2, name: "Texas regions", item: `${siteUrl}/regions` }, { "@type": "ListItem", position: 3, name: loaderData.region.name, item: pageUrl }] },
        ],
      })],
    };
  },
...
function CanonicalRegionRoute() {
  ...
  return <Suspense fallback={<div className="min-h-[36rem]" aria-hidden="true" />}><CanonicalRegionPage {...data} /></Suspense>;
}
