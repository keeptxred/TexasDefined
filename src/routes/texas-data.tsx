        ...
        '@graph': [
          {
            ...
            dataset: TEXAS_DATASETS.map((dataset) => ({ '@type': 'Dataset', '@id': `${absoluteUrl(texasDefinedBrand, `/texas-data/${dataset.slug}`)}#dataset`, name: dataset.title, description: dataset.description, url: absoluteUrl(texasDefinedBrand, `/texas-data/${dataset.slug}`), dateModified: dataset.updated, temporalCoverage: String(dataset.year) })),
          },
...
function Page() {
  return <>
    ...
    <Container className="py-12 sm:py-16">
      <aside className="max-w-3xl border-y border-border py-5 text-sm leading-7 text-muted-foreground"><p className="eyebrow text-primary">About the data</p><p className="mt-3">Public data is most useful when it has context. Each dataset includes source notes, review dates and a path back to the original information.</p></aside>
      ...
      <aside className="border-y border-border py-5 text-sm leading-6 text-muted-foreground">Texas Defined uses public information as a starting point for understanding the state. For official decisions, deadlines or eligibility, follow the source links to the responsible agency.</aside>
    </Container>
  </>;
}
