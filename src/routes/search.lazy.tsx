...
function SearchPage() {
  ...
  const query = (q ?? "").trim();
  const searchableDocuments = [...new Map([...documents, ...paintedChurchSearchDocuments].map((document) => [document.href, document])).values()];
  const results: SearchHit[] = query ? search(searchableDocuments, { term: query, brandId: texasDefinedBrand.identity.id }) : [];
