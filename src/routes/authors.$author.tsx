...
export const Route = createFileRoute("/authors/$author")({
  ...
  head: ({ loaderData, params }) => {
    if (!loaderData) return { meta: [{ title: "Author unavailable" }, { name: "robots", content: "noindex, nofollow" }] };
    const canonicalPath = `/authors/${params.author}`;
    const url = absoluteUrl(texasDefinedBrand, canonicalPath);
    const description = `${loaderData.author.bio} Read stories by ${loaderData.author.name} in Texas Defined.`;
    return {
      ...
      scripts: [jsonLd({
        ...
        "@graph": [
          ...
          },
          {
            "@type": "ItemList",
            "@id": `${url}#articles`,
            name: `Stories by ${loaderData.author.name}`,
  ...
  notFoundComponent: () => <Container className="py-24"><p className="eyebrow text-primary">Contributors</p><h1 className="mt-3 font-display text-4xl">That author profile is unavailable</h1><Link to="/about" className="eyebrow mt-6 inline-block border-b border-primary pb-1 text-primary">About Texas Defined →</Link></Container>,
  ...
  return <>
    <section className="border-b border-border bg-surface">
      <Container className="py-16 sm:py-24">
        ...
        <p className="eyebrow mt-9 text-primary">Texas Defined contributor</p>
        ...
        <p className="mt-6 max-w-2xl text-sm leading-7 text-muted-foreground">Texas Defined contributor profiles identify the writer responsible for published work and connect each byline to that writer’s archive. Editorial and sourcing principles are described on the <Link to="/about" className="border-b border-primary text-primary">About Texas Defined</Link> page.</p>
      </Container>
    </section>

    <Section>
      <Container>
        <SectionHeader eyebrow="Author archive" title={`Stories by ${author.name}`} description={`${articles.length.toLocaleString("en-US")} published ${articles.length === 1 ? "story" : "stories"} currently carry this byline.`} />
        {articles.length > 0 ? <ul className="mt-10 grid gap-10 sm:grid-cols-2 lg:grid-cols-3">{articles.map((article) => <li key={article.id}><ArticleCard article={article} /></li>)}</ul> : <p className="mt-8 max-w-xl text-sm leading-7 text-muted-foreground">No published stories are currently attached to this profile.</p>}
      </Container>
    </Section>
  </>;
}
