...
import { INTERNAL_LINK_POLICIES, validateInternalLinkPolicies } from '@/platform/internal-link-policies';
...
function Page() {
  ...
  const errors = [...places.errors, ...sources.errors, ...entities.errors, ...audit.errors.map((issue) => issue.message), ...policyHealth.issues];
  ...
  return <Container className="py-16 sm:py-24">
    ...
    <section className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
      ...
      <Metric value={policyHealth.valid ? 'Healthy' : 'Needs review'} label="Link policies" detail={`${Object.keys(INTERNAL_LINK_POLICIES).length} governed route policies`} />
