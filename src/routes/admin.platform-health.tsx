...
function Page(){
  ...
  return <Container className="py-16 sm:py-24"><p className="eyebrow text-primary">TexasDefined Operations</p><h1 className="mt-3 font-display text-4xl sm:text-6xl">Platform Health</h1>
    ...
    {(!places.valid||!sources.valid)&&<section className="mt-8 rounded-md border border-destructive/40 p-5"><h2 className="font-display text-2xl">Validation errors</h2>{[...places.errors,...sources.errors].map(error=><p className="mt-2 text-sm" key={error}>{error}</p>)}</section>}
