...
import { TEXAS_ENTITY_REGISTRY, validateTexasEntityRegistry } from '@/data/texas-entity-registry';
...
function Page(){
  ...
  return <Container className="py-16 sm:py-24"><p className="eyebrow text-primary">TexasDefined Operations</p><h1 className="mt-3 font-display text-4xl sm:text-6xl">Platform Health</h1>
    <section className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
      <Metric value={places.valid?'Healthy':'Blocked'} label="Statewide place data" detail={`${TEXAS_COUNTIES.length} counties · ${TEXAS_CITIES.length} cities`} />
      <Metric value={entities.valid?'Healthy':'Blocked'} label="Entity registry" detail={`${TEXAS_ENTITY_REGISTRY.length} governed entities`} />
    ...
    <section className="mt-12"><h2 className="font-display text-3xl">Entity coverage</h2><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{entities.coverage.map(item=><article key={item.domain} className="rounded-md border border-border p-5"><strong className="capitalize">{item.domain.replaceAll('-',' ')}</strong><p className="mt-2 text-sm text-muted-foreground">{item.records} records · {item.active} active · {item.pending} pending verification · {item.sourceCount} sources</p></article>)}</div></section>
