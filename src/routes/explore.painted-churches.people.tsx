  ...
  head: () => ({
    ...
    scripts: [jsonLd({
      ...
      "@graph": [
        ...
        { "@type": "ItemList", "@id": `${pageUrl}#contributors`, numberOfItems: canonicalPaintedChurchContributors.length, itemListElement: canonicalPaintedChurchContributors.map((contributor, index) => ({ "@type": "ListItem", position: index + 1, item: { "@type": contributor.kind === "organization" ? "Organization" : "Person", name: contributor.name, url: `${pageUrl}/${contributor.slug}`, description: contributor.answer } })) },
      ],
    })],
  }),
  component: PeopleHub,
