...
export const Route = createFileRoute("/fishing/lakes/$slug")({
  loader: async ({ context, params }) => {
    ...
    if (params.slug === LAKE_CONROE_SLUG) {
      ...
      return { kind: "conroe" as const, lake, reports, guides, pageData };
    }
    ...
    return { kind: "showcase" as const, lake, reports, guides, businesses, placements, pageData };
  },
  head: ({ loaderData }) => {
...
function FishingLakeOverviewRoute() {
  ...
  if (data.kind === "conroe") return <LakeConroeGuide reports={data.reports} guides={data.guides} pageData={data.pageData} />;
  return <ShowcaseLakeGuide reports={data.reports} guides={data.guides} businesses={data.businesses} placements={data.placements} pageData={data.pageData} />;
}
