  ...
  return <Container className="pb-16 pt-12 sm:pb-24 sm:pt-16">
    <article className="mx-auto max-w-6xl">
      ...
      {history.length ? <section className="border-y border-border py-10">
        <div className="grid gap-6 md:grid-cols-3"><Fact label="First year" value={`${first?.year}: ${displayRate(first)}`} /><Fact label="Latest year" value={`${last?.year}: ${displayRate(last)}`} /><Fact label="Change across comparable years" value={change == null ? 'Not comparable across unavailable/variable years' : `${change >= 0 ? '+' : ''}${change.toFixed(6)}`} /></div>
      ...
      <section className="grid gap-8 border-b border-border py-12 lg:grid-cols-[15rem_1fr]">
        ...
        <div className="max-w-3xl space-y-5 text-base leading-8 text-muted-foreground"><p>Texas property taxes are imposed by local taxing units. A single property can be inside several of them at once: a county, city, school district, hospital district, municipal utility district, emergency-services district, community-college district or another special district. This explorer follows one taxing unit at a time. It does not add every unit that applies to a particular address.</p><p>A reported rate is generally expressed per $100 of taxable value. That means a rate of 0.500000 is not a 50 percent tax. It represents fifty cents per $100 of the taxable value to which that rate applies. To estimate a real bill, you still need the parcel’s taxable value after exemptions and the rates for every applicable taxing unit.</p><p>That is why rate history and bill history can move differently. A taxing unit can lower its rate while the average tax bill rises because taxable values increased. The reverse can also happen. Use the history here to study the rate itself, then use parcel-level and appraisal records to understand the bill.</p></div>
      </section>

      <section className="grid gap-10 border-b border-border py-12 lg:grid-cols-2">
        <div><p className="eyebrow text-primary">Rate components</p><h2 className="mt-2 font-display text-4xl">What makes up the tax rate</h2><div className="mt-6 space-y-5 text-sm leading-7 text-muted-foreground"><Explain title="Maintenance and operations (M&O)">M&O generally supports the ordinary operating side of a taxing unit. Depending on the type of unit, that can include personnel, services, maintenance, programs and other ongoing governmental costs. School-district rate structures are governed by additional state rules, so the label should be read in the context of that unit type.</Explain><Explain title="Interest and sinking / debt service (I&S)">Debt-service rates generally support eligible principal and interest obligations. A change can reflect debt issuance, repayment schedules, taxable-value growth, refinancing, voter-approved debt or other factors. The historical rate alone does not identify the cause.</Explain><Explain title="Why some records do not split cleanly">Statewide source files are assembled from many local units and years. Older records, variable-rate records or source conflicts do not always provide a clean M&O/I&S split. Texas Defined preserves the uncertainty rather than manufacturing a component that was not reliably reported.</Explain></div></div>
        <div><p className="eyebrow text-primary">What this explorer shows</p><h2 className="mt-2 font-display text-4xl">What you can learn here</h2><ul className="mt-6 space-y-4 text-sm leading-7 text-muted-foreground"><Check yes>How a specific county, city, school district or special district reported its rate across available years.</Check><Check yes>Whether a fixed reported rate increased, decreased or stayed relatively stable over the selected history.</Check><Check yes>Whether the source reports M&O, debt-service, levy or variable-rate information for a given year.</Check><Check>Whether your individual parcel was inside that taxing unit in every year shown.</Check><Check>What your actual tax bill was after exemptions, caps, freezes, prorations or parcel-specific changes.</Check><Check>Why a governing body adopted a particular rate in a particular year.</Check></ul></div>
      </section>
      ...
      <section className="grid gap-8 border-b border-border py-12 lg:grid-cols-[15rem_1fr]">
        ...
        <div className="max-w-3xl space-y-5 text-sm leading-7 text-muted-foreground"><p>The history is built from statewide Texas property-tax source snapshots retained by Texas Defined. Source files change format over time, and a taxing unit can appear under different names or with different component detail from one year to another. The matching layer uses the taxing-unit identity and type rather than pretending every text label is perfectly stable.</p><p>Records marked <strong className="text-foreground">not reported</strong> are not treated as zero. A missing rate is missing information, not a tax rate of 0.000000. Records marked <strong className="text-foreground">variable</strong> are likewise not collapsed into a false single number. If the source carries multiple possible rates or the rate depends on a local condition, the tool shows that uncertainty.</p><p>The comparison math only uses fixed rates that can be compared without inventing data. That is why the summary may say a net change is not comparable even when several years are displayed. This is intentional: a smaller honest dataset is more useful than a smooth chart built from assumptions.</p><p>Historical rates are statewide source snapshots. A rate history does not prove that a particular parcel was inside the taxing unit in every year shown. Annexations, deannexations, new special districts, district dissolutions and boundary changes can all alter which units apply to an address.</p></div>
      </section>

      <section className="border-b border-border py-12"><div className="grid gap-8 lg:grid-cols-[15rem_1fr]"><div><p className="eyebrow text-primary">Next step</p><h2 className="mt-2 font-display text-4xl">Estimate what the rate means for your bill</h2></div><div className="grid sm:grid-cols-2"><GuideLink href="/texas-property-tax-estimator" title="Texas property-tax estimator">Estimate a current bill using local-rate data and a taxable-value scenario.</GuideLink><GuideLink href="/texas-property-tax-bill-breakdown" title="Property-tax bill breakdown">See how multiple taxing units combine into the total bill.</GuideLink><GuideLink href="/texas-property-tax-county-comparison-calculator" title="Compare Texas locations">Compare property-tax scenarios across counties or local taxing structures.</GuideLink><GuideLink href="/learn/property-taxes" title="How Texas property taxes work">Understand appraisal, exemptions, rates, bills, protests and the annual cycle.</GuideLink><GuideLink href="/property-tax/counties" title="Property tax by county">Move from statewide data to verified county-specific resources.</GuideLink><GuideLink href="/do/property-tax-protest" title="Texas property-tax protest guide">Use the correct process when the issue is appraised value rather than the adopted tax rate.</GuideLink></div></div></section>

      <section className="py-12"><p className="eyebrow text-primary">Frequently asked questions</p><h2 className="mt-2 font-display text-4xl">Common questions about rate history</h2><div className="mt-7 grid gap-x-8 md:grid-cols-2"><Explain title="Does a lower rate mean my bill went down?">Not necessarily. Your bill depends on taxable value, exemptions and every applicable taxing unit. Rate compression can occur at the same time that taxable values rise.</Explain><Explain title="Can I use this to prove my parcel’s past taxes?">No. Use historical tax bills, appraisal records and official parcel-level records for that. This tool tracks taxing-unit rate records, not parcel jurisdiction.</Explain><Explain title="Why are six decimal places shown?">State and local source files often report rates with that precision. Keeping the source precision makes small year-to-year changes visible and avoids rounding a rate before comparison.</Explain><Explain title="Why do some units have only a few years?">A unit may be new, renamed, dissolved, unmatched in an older source file, absent from a particular statewide file or represented in a format that cannot be safely normalized. We do not fill missing years with guesses.</Explain></div></section>
    </article>
  </Container>;
}
