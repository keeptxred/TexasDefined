  ...
  return <Container className="pb-16 pt-12 sm:pb-24 sm:pt-16">
    <article className="mx-auto max-w-6xl">
      ...
      {history.length ? <section className="border-y border-border py-10">
        <div className="grid gap-6 md:grid-cols-3"><Fact label="First year" value={`${first?.year}: ${displayRate(first)}`} /><Fact label="Latest year" value={`${last?.year}: ${displayRate(last)}`} /><Fact label="Net usable fixed-rate change" value={change == null ? 'Not comparable across unavailable/variable years' : `${change >= 0 ? '+' : ''}${change.toFixed(6)}`} /></div>
