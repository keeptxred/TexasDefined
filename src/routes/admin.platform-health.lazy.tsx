...
import { INTERNAL_LINK_QUALITY_THRESHOLDS } from '@/platform/internal-link-quality';
import {
...
export const Route = createLazyFileRoute('/admin/platform-health')({ component: Page });
...
function Page() {
  ...
  return <Container className="py-16 sm:py-24">
    ...
    <InternalLinkPolicyHistory />
    <section className="mt-8 rounded-md border border-border p-5 sm:flex sm:items-center sm:justify-between sm:gap-6">
      <div><h2 className="font-display text-2xl">Rollback operations</h2><p className="mt-2 text-sm text-muted-foreground">Open the read-only release comparison to inspect exact policy differences before preparing a governed rollback release.</p></div>
      <Link to="/admin/internal-link-rollback" className="mt-4 inline-flex rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground sm:mt-0">Preview rollback</Link>
    </section>
