...
const faqs = [
  {
    ...
    answer: 'Home equity is the difference between an estimated property value and debt secured by the property. Because the home value is an estimate and loan balances change, the result should be treated as a planning number.',
  },
  {
    question: 'Is estimated equity the same as cash I can borrow?',
    answer: 'No. A lender can apply product, underwriting, appraisal, lien and legal requirements that differ from the simple equity calculation. Use the estimate to frame questions, not as a borrowing approval.',
  },
  {
    question: 'What home value should I use?',
    answer: 'Use a defensible current estimate and test more than one value. A lower-value scenario can show how sensitive the equity estimate is to an appraisal or market change.',
  },
  {
    question: 'Should I include every loan secured by the home?',
    answer: 'Yes. A useful equity estimate accounts for the balances of loans or liens that are secured by the property rather than subtracting only the first mortgage.',
  },
];

export const Route = createLazyFileRoute('/texas-home-equity-calculator')({
  component: TexasHomeEquityCalculatorPage,
});
  ...
  return (
    <CalculatorPage eyebrow="What your home may be worth to you" title="Texas home equity calculator" description={description}>
      ...
      <section className="mt-14 border-t border-border pt-10" aria-labelledby="equity-estimate-heading">
        <p className="eyebrow text-primary">Value minus secured debt</p>
        <h2 id="equity-estimate-heading" className="mt-3 font-display text-3xl">Treat home equity as an estimate, not an available credit line</h2>
        <div className="mt-5 max-w-3xl space-y-4 text-base leading-7 text-muted-foreground">
          <p>The calculation is straightforward: begin with an estimated property value and subtract debt secured by the home. The uncertainty sits in the inputs. Market estimates can differ from an appraisal, and loan balances move as payments, interest and new draws are posted.</p>
          <p>Run a conservative home-value scenario as well as the working estimate. If a planned borrowing decision only works at the highest assumed value, that sensitivity is useful to know before paying for an appraisal or applying for a loan.</p>
        </div>
      </section>

      <section className="mt-12 border-t border-border pt-10" aria-labelledby="equity-links-heading">
        <p className="eyebrow text-primary">Put the estimate in context</p>
        <h2 id="equity-links-heading" className="mt-3 font-display text-3xl">Compare equity with the loan decision you are considering</h2>
      ...
      <section className="mt-12 border-t border-border pt-10" aria-labelledby="equity-faq-heading">
        <p className="eyebrow text-primary">Common questions</p>
        <h2 id="equity-faq-heading" className="mt-3 font-display text-3xl">Texas home equity calculator FAQ</h2>
