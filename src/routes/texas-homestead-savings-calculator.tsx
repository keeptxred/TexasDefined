...
export const Route = createFileRoute('/texas-homestead-savings-calculator')({
  head: () => ({
    meta: buildMeta(texasDefinedBrand, { canonicalPath, title: 'Texas Homestead Exemption Savings Calculator', description }),
    links: [canonicalLink(texasDefinedBrand, canonicalPath)],
    scripts: [jsonLd({
      '@context': 'https://schema.org',
      '@graph': [
        { '@type': 'WebApplication', '@id': `${pageUrl}#calculator`, name: 'Texas Homestead Exemption Savings Calculator', description, url: pageUrl, applicationCategory: 'FinanceApplication', operatingSystem: 'Any', isPartOf: { '@id': `${absoluteUrl(texasDefinedBrand, '/')}#website` } },
...
function HomesteadSavingsCalculator() {
  ...
  return <Container className="pb-16 pt-12 sm:pb-24 sm:pt-16"><article className="mx-auto max-w-6xl">
    <nav aria-label="Breadcrumb" className="border-b border-border pb-4 text-xs uppercase tracking-[0.14em] text-muted-foreground"><Link to="/">Front page</Link><span className="mx-2">/</span><Link to="/property">Property</Link><span className="mx-2">/</span><Link to="/property-tax-calculators">Calculators</Link><span className="mx-2">/</span><span>Homestead savings</span></nav>
    <header className="grid gap-8 border-b border-border py-10 lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-end"><div><p className="eyebrow text-primary">Property-tax calculator</p><h1 className="mt-3 max-w-4xl font-display text-5xl leading-[0.98] sm:text-7xl">Texas homestead exemption savings calculator</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-muted-foreground sm:text-xl">Estimate the difference between taxes before and after residence-homestead exemptions using the rates that actually apply to the property.</p></div><p className="border-l border-border pl-6 text-sm leading-6 text-muted-foreground">School and other local exemptions stay separate because one exemption does not automatically reduce taxable value for every taxing unit.</p></header>
