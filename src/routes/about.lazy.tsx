...
const DATA_NOTES = [
  ...
  { title: "Browser storage and site analytics", body: "Texas Defined uses browser storage for features such as saved shop picks and a randomly generated analytics session identifier. Site analytics can record page paths, searches, resource interactions, visits to official external resources, visible reading time, trusted browser interactions and scroll-depth signals. The reader-quality dataset hashes the random session identifier before storage and does not store names, email addresses, IP addresses or full user-agent strings. These quality signals are used to distinguish raw traffic counts from stronger evidence of genuine reading behavior; they do not identify or verify a person." },
    ...
    <Section>
      <Container>
        ...
        <div className="mt-10 max-w-3xl space-y-3 border-t border-border pt-6 text-sm leading-7 text-muted-foreground">
          <p>Read the publication-wide <a href="/editorial-policy" className="border-b border-primary text-primary">Editorial Policy</a>, <a href="/sourcing-methodology" className="border-b border-primary text-primary">Sourcing Methodology</a>, and <a href="/corrections-policy" className="border-b border-primary text-primary">Corrections Policy</a> for the standards behind our bylines, research, updates and accountability.</p>
          ...
          <p>To report a factual error or request a correction, use the <Link to="/partner-with-us" className="border-b border-primary text-primary">Texas Defined contact form</Link>, identify the message as a correction or source update, and include the page URL and the fact that needs review.</p>
        </div>
        <address id="contact" className="mt-6 max-w-3xl scroll-mt-28 not-italic text-sm leading-7 text-muted-foreground"><strong className="text-foreground">Contact Texas Defined.</strong> For corrections, source updates or general questions, use the <Link to="/partner-with-us" className="border-b border-primary text-primary">contact form</Link> or an official profile: {texasDefinedBrand.identity.social.map((profile, index) => <span key={profile.href}>{index ? " · " : ""}<a href={profile.href} target="_blank" rel="noreferrer" className="border-b border-primary text-primary">{profile.label}</a></span>)}. A street address is published only when there is a verified public business location to list.</address>
      </Container>
    </Section>
