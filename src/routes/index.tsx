    ...
    const structuredData = {
      ...
      "@graph": [
        ...
        },
        {
          ...
          name: "Featured Texas stories and destinations",
...
function HomePage() {
  ...
  return (
    <>
      ...
      {brand.features.events && events.length > 0 && <Section><Container><SectionHeader eyebrow="This weekend" title="Good reasons to leave the house" description="Fairs, festivals, rodeos and the kind of local plans that make a Saturday feel longer." actionLabel="See the full weekend" actionTo="/events" /><div className="mt-8 grid gap-x-12 md:grid-cols-2">{events.map((event) => <EventCard key={event.id} event={event} regionLabel={regionName(event.region)} />)}</div></Container></Section>}
      ...
      {brand.features.guides && guides.length > 0 && <Section tone="surface"><Container><SectionHeader eyebrow="Front porch" title="The practical side of living here" description="Homes, moving, money and the everyday questions that come with putting down roots." actionLabel="Open the guides" actionTo="/guides" /><ul className="mt-10 grid gap-6 md:grid-cols-3">{guides.slice(0, 3).map((guide) => <li key={guide.id}><GuideCard guide={guide} /></li>)}</ul></Container></Section>}

      {brand.features.shop && collections.length > 0 && <Section><Container><SectionHeader eyebrow="Made here" title="Things we'd actually buy" description="A few well-made finds with a real connection to the places and stories we cover." actionLabel="Browse the shop" actionTo="/shop" /><div className="mt-10"><CollectionStrip collections={collections.slice(0, 3)} /></div></Container></Section>}
