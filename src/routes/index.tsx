...
function HomePage() {
  ...
  return <>
    ...
    {featuredDestinations.length > 0 && <Section><Container><SectionHeader eyebrow="Texas, selected" title="Four places worth knowing" description="A handpicked look at destinations that capture a different side of the state." actionLabel="Explore more of Texas" actionTo="/explore" /><ul className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">{featuredDestinations.map((destination) => <li key={destination.id}><DestinationCard destination={destination} tone="overlay" regionLabel={regionName(destination.region)} /></li>)}</ul></Container></Section>}
