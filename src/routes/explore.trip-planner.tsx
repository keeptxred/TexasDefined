...
export const Route = createFileRoute("/explore/trip-planner")({
  validateSearch: (value: Record<string, unknown>) => ({
    ...
    trip: typeof value.trip === "string" ? value.trip : undefined,
  }),
  head: () => ({
    meta: buildMeta(texasDefinedBrand, {
      canonicalPath: "/explore/trip-planner",
      title: "Texas Trip Planner",
      description:
        "Build a Texas itinerary around your region, interests, family, accessibility needs and daily driving tolerance.",
    }),
...
function buildTrip(destinations: Destination[], preferences: Preferences, seedSlug?: string): GeneratedTrip {
  const candidates = destinations
    .filter((destination) => !preferences.region || destination.region === preferences.region)
    .filter((destination) => !preferences.accessible || Boolean(destination.accessibilityNotes))
  ...
  while (chosen.length < desired && remaining.length) {
    ...
    remaining.forEach((destination, index) => {
      const distance = distanceMiles(previous, destination);
  ...
  for (const destination of chosen) {
    ...
    if (
      days[dayIndex].stops.length >= 3 ||
      (days[dayIndex].stops.length > 0 && dayDistance + leg > preferences.maxDrivingMiles)
    ) {
    ...
    days[dayIndex].stops.push({
      destination,
      durationMinutes,
      note: destination.highlights[0] ?? destination.summary,
    });
...
function encodeTrip(trip: GeneratedTrip) {
  const compact = {
    ...
    slugs: trip.days.map((day) => day.stops.map((stop) => stop.destination.slug)),
  };
...
function decodeTrip(value: string, destinations: Destination[]): GeneratedTrip | null {
  try {
    ...
    return {
      ...
      days: parsed.slugs.map((slugs, index) => ({
        day: index + 1,
        stops: slugs
...
function TripPlanner() {
  ...
  const seed = useMemo(
    () => destinations.find((destination) => destination.slug === search.destination),
  ...
  function replaceStop(dayIndex: number, stopIndex: number) {
    setTrip((current) => {
      ...
      const replacement = destinations
        .filter((destination) => !used.has(destination.slug))
        .filter((destination) => !current.preferences.region || destination.region === current.preferences.region)
        .sort((a, b) => scoreDestination(b, current.preferences) - scoreDestination(a, current.preferences))[0];
      if (!replacement) return current;
      return {
        ...current,
        days: current.days.map((day, d) => d !== dayIndex ? day : {
          ...day,
          stops: day.stops.map((stop, s) => s !== stopIndex ? stop : {
            destination: replacement,
            durationMinutes: 150,
            note: replacement.highlights[0] ?? replacement.summary,
          }),
        }),
      };
    });
      ...
      <Container className="pb-16 pt-16 sm:pt-24">
        <nav aria-label="Breadcrumb" className="text-xs text-muted-foreground">
          <ol className="flex items-center gap-2">
            ...
            <li aria-hidden="true">/</li>
            <li aria-current="page" className="text-foreground">Trip Planner</li>
          </ol>
        </nav>

        <p className="eyebrow mt-8 text-primary">Explore Texas</p>
        <h1 className="mt-3 max-w-4xl font-display text-5xl leading-tight sm:text-7xl">Build a Texas trip</h1>
        <p className="mt-5 max-w-3xl text-lg leading-8 text-muted-foreground">
          Choose what matters to you and Texas Defined will build a practical itinerary from the destination catalog, keeping daily driving and your interests in view.
        </p>
        {seed && (
          <p className="mt-5 inline-block border border-border bg-surface px-4 py-3 text-sm">
            Starting with <strong>{seed.name}</strong>
          </p>
        )}

        <div className="mt-12 grid gap-10 lg:grid-cols-[360px_1fr]">
          <form onSubmit={submit} className="space-y-5 border border-border bg-surface p-6 print:hidden">
            <Field label="Trip title" name="title" defaultValue="My Texas trip" required />
            <label className="block">
              <span className="text-sm font-medium">Region</span>
              <select name="region" defaultValue={seed?.region ?? ""} className="mt-2 w-full border border-border bg-background px-3 py-2 text-sm">
                <option value="">Anywhere in Texas</option>
                {regions.map((region) => <option key={region.id} value={region.id}>{region.name}</option>)}
              </select>
            </label>
            <div className="grid grid-cols-3 gap-3">
              <Field label="Days" name="days" type="number" defaultValue="2" min="1" max="14" required />
              <Field label="Adults" name="adults" type="number" defaultValue="2" min="1" max="20" required />
              <Field label="Children" name="children" type="number" defaultValue="0" min="0" max="20" required />
            </div>
            <Field label="Max driving per day (miles)" name="maxDrivingMiles" type="number" defaultValue="150" min="25" max="500" required />
            <fieldset>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2">
                {interests.map((interest) => (
                  <label key={interest} className="flex items-center gap-2 text-sm capitalize">
                    <input type="checkbox" name="interests" value={interest} defaultChecked={["scenic", "history"].includes(interest)} />
                    {interest}
                  </label>
                ))}
              </div>
          ...
          <section aria-live="polite">
            {!trip ? (
              <div className="border border-dashed border-border p-12 text-center text-muted-foreground">
                Pick a region, trip length and interests to build your itinerary.
              </div>
            ) : (
                ...
                <div className="flex flex-wrap items-start justify-between gap-5">
                  <div>
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">
                      {trip.preferences.days} days · {trip.preferences.adults} adults{trip.preferences.children ? ` · ${trip.preferences.children} children` : ""}
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2 print:hidden">
                    <Button variant="outline" onClick={saveTrip}><Save />{saved ? "Saved" : "Save"}</Button>
                    <Button variant="outline" onClick={shareTrip}><Share2 />Share</Button>
                    <Button variant="outline" onClick={reorderRoute}><RefreshCw />Reorder route</Button>
                    <Button variant="outline" onClick={() => window.print()}><Printer />Print</Button>
                    <Button variant="outline" onClick={() => { setTrip(null); setSaved(false); setMessage(""); window.localStorage.removeItem(STORAGE_KEY); }}><Trash2 />Clear</Button>
                  </div>
                </div>
                {message && <p className="mt-4 text-sm text-primary">{message}</p>}
                <p className="mt-6 border border-border bg-muted/40 p-4 text-sm leading-6">
                  Hours, fees, reservations, road conditions and seasonal closures can change. Verify time-sensitive details with the official source before leaving.
                </p>

                <div className="mt-10 space-y-10">
                  {trip.days.map((day, dayIndex) => (
                    <section key={day.day} className="break-inside-avoid">
                      <h3 className="border-b border-border pb-3 font-display text-3xl">Day {day.day}</h3>
                      {day.stops.length === 0 ? (
                        <p className="mt-4 text-muted-foreground">No matching destinations were available for this day.</p>
                      ) : (
                        <ol className="mt-5 space-y-4">
                          {day.stops.map((stop, stopIndex) => (
                            <li key={`${stop.destination.slug}-${stopIndex}`} className="border border-border bg-surface p-5">
                              <div className="grid gap-5 sm:grid-cols-[150px_1fr_auto]">
                                <img src={stop.destination.hero.src} alt={stop.destination.hero.alt} width={stop.destination.hero.width} height={stop.destination.hero.height} className="aspect-[4/3] w-full object-cover" />
                                <div>
                                  <p className="eyebrow text-primary">{stop.destination.nearestTown} · {regions.find((region) => region.id === stop.destination.region)?.name}</p>
                                  <h4 className="mt-1 font-display text-2xl">
                                    <Link to="/destination/$slug" params={{ slug: stop.destination.slug }} className="hover:text-primary">{stop.destination.name}</Link>
                                  </h4>
                                  <p className="mt-2 text-sm leading-6 text-muted-foreground">{stop.destination.summary}</p>
                                  <p className="mt-3 text-sm"><strong>Plan for:</strong> {Math.round(stop.durationMinutes / 60 * 10) / 10} hours</p>
                                  <p className="mt-1 text-sm"><strong>Why it fits:</strong> {stop.note}</p>
                                  {stop.destination.officialUrl && <a href={stop.destination.officialUrl} target="_blank" rel="noreferrer" className="mt-3 inline-block text-sm text-primary underline underline-offset-4">Official information</a>}
                                </div>
                                <div className="flex gap-1 print:hidden sm:flex-col">
                                  <Button size="icon" variant="ghost" onClick={() => moveStop(dayIndex, stopIndex, -1)} disabled={stopIndex === 0} aria-label="Move stop up"><ArrowUp /></Button>
                                  <Button size="icon" variant="ghost" onClick={() => moveStop(dayIndex, stopIndex, 1)} disabled={stopIndex === day.stops.length - 1} aria-label="Move stop down"><ArrowDown /></Button>
                                  <Button size="sm" variant="outline" onClick={() => replaceStop(dayIndex, stopIndex)}>Swap</Button>
                                </div>
                              </div>
                            </li>
                          ))}
...
function Field({ label, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { label: string }) {
  return (
    <label className="block">
      ...
      <input {...props} className="mt-2 w-full border border-border bg-background px-3 py-2 text-sm" />
    </label>
  );
}
