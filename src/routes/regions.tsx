...
import { getCanonicalRegionsIndexData } from "@/data/canonical-regions-page.functions";
...
export const Route = createFileRoute("/regions")({
  loader: () => getCanonicalRegionsIndexData(),
        ...
        "@graph": [
          ...
          },
          {
            ...
            itemListElement: (regionCards ?? []).map(({ region }, index) => ({
              ...
              item: { "@type": "Place", name: region.name, containedInPlace: { "@type": "State", name: "Texas" } },
            })),
          },
          {
            "@type": "BreadcrumbList",
  ...
  return <>
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {regionCards.map(({ region, presentation, subregionCount, metroCount, placeCount }) => <Link key={region.id} to="/regions/$region" params={{ region: region.id }} className="group flex min-h-[22rem] flex-col border border-border bg-background p-7 transition hover:-translate-y-0.5 hover:border-primary/45 hover:shadow-sm">
            ...
            <dl className="mt-auto grid grid-cols-3 gap-3 border-t border-border pt-6 text-center">
              ...
              <div><dt className="text-[0.68rem] uppercase tracking-[0.12em] text-muted-foreground">Places</dt><dd className="mt-1 font-display text-2xl">{placeCount}</dd></div>
            </dl>
    ...
    <Container className="py-16 sm:py-20">
      <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr]">
        <div><p className="eyebrow text-primary">One graph, different lenses</p><h2 className="mt-3 font-display text-4xl sm:text-5xl">Canonical geography underneath. Travel and relocation on top.</h2><p className="mt-5 max-w-2xl text-base leading-8 text-muted-foreground">Hill Country, Piney Woods, Big Bend, Coastal Bend, Rio Grande Valley, Texoma and other familiar names remain important subregions or travel identities. Existing Explore URLs stay live and are cross-walked to this graph rather than becoming a competing map.</p></div>
        <div className="border-l-2 border-primary/30 pl-7"><p className="font-semibold">Boundary rule</p><p className="mt-3 text-sm leading-7 text-muted-foreground">Regional boundaries are editorial and approximate. Transition cities can have a primary canonical region plus gateway or adjacency relationships. Austin is Central Texas with a Hill Country gateway; San Antonio is South Texas with a Hill Country gateway and Central Texas adjacency.</p><Link to="/browse/cities" className="eyebrow mt-6 inline-block border-b border-primary pb-1 text-primary">Browse Texas cities →</Link></div>
      </div>
    </Container>
  </>;
}
