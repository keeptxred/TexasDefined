  ...
  head: ({ loaderData: regionCards }) => {
    ...
    return {
      ...
      scripts: [jsonLd({
        ...
        "@graph": [
          { "@type": "CollectionPage", "@id": `${pageUrl}#page`, url: pageUrl, name: "The 7 Texas Regions", description, isPartOf: { "@id": `${siteUrl}#website` }, publisher: { "@id": `${siteUrl}#organization` }, mainEntity: { "@id": `${pageUrl}#regions` }, breadcrumb: { "@id": `${pageUrl}#breadcrumb` } },
          { "@type": "ItemList", "@id": `${pageUrl}#regions`, name: "TexasDefined canonical Texas regions", numberOfItems: regionCards?.length ?? 0, itemListElement: (regionCards ?? []).map(({ region }, index) => ({ "@type": "ListItem", position: index + 1, url: absoluteUrl(texasDefinedBrand, `/regions/${region.id}`), item: { "@type": "Place", name: region.name, containedInPlace: { "@type": "State", name: "Texas" } } })) },
          { "@type": "BreadcrumbList", "@id": `${pageUrl}#breadcrumb`, itemListElement: [{ "@type": "ListItem", position: 1, name: "Front page", item: siteUrl }, { "@type": "ListItem", position: 2, name: "Texas regions", item: pageUrl }] },
        ],
      })],
    };
  },
...
function TexasRegionsRoute() {
  ...
  return <Suspense fallback={<div className="min-h-[36rem]" aria-hidden="true" />}><TexasRegionsPage regionCards={regionCards} /></Suspense>;
}
