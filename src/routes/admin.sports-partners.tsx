...
export const Route = createFileRoute('/admin/sports-partners')({
  ...
  component: SportsPartnerLeadsPage,
});
  ...
  return <Container className="py-12 sm:py-16">
    <main className="mx-auto max-w-6xl">
      ...
      {!dashboard ? <section className="mt-10 max-w-xl border-y border-border py-8" aria-labelledby="unlock-heading">
        <p className="eyebrow text-primary">Protected lead data</p>
        <h2 id="unlock-heading" className="mt-2 font-display text-3xl">Unlock sports partner leads</h2>
        <p className="mt-3 text-sm leading-7 text-muted-foreground">Lead names, emails and messages are not loaded with this page. Enter the sports-partner admin key to request them from the server. The key is kept only for this browser session.</p>
...
function LeadDashboard({ dashboard, updatingLeadId, onStatusChange, error }: {
  ...
  return <>
    <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-6">
      ...
      <Metric label="Reviewing" value={dashboard.statusCounts.reviewing} />
      <Metric label="Contacted" value={dashboard.statusCounts.contacted} />
      <Metric label="Closed" value={dashboard.statusCounts.closed} />
      <Metric label="Venue-attributed" value={venueAttributed} />
    </section>
    ...
    <section className="mt-10 grid gap-8 lg:grid-cols-[17rem_1fr]">
      <aside>
        ...
        <div className="mt-5 border-t border-border">
          {dashboard.sourceCounts.length ? dashboard.sourceCounts.slice(0, 20).map((source) => <div key={source.sourcePath} className="border-b border-border py-3">
          ...
          </div>) : <p className="py-4 text-sm text-muted-foreground">No sports-travel inquiries yet.</p>}
        </div>
        <p className="mt-4 text-xs leading-5 text-muted-foreground">Generated {formatDateTime(dashboard.generatedAt)}. Attribution comes from the internal source path recorded when the inquiry form is submitted.</p>
      </aside>

      <div>
        <div className="flex items-end justify-between gap-4 border-b border-border pb-4">
          <div><p className="eyebrow text-primary">Inquiry queue</p><h2 className="mt-2 font-display text-3xl">Sports-travel opportunities</h2></div>
        </div>
          ...
          <div className="grid gap-5 md:grid-cols-[1fr_auto] md:items-start">
            <div>
              <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
                ...
                <span className="border border-border px-2 py-1 text-[0.68rem] font-semibold uppercase tracking-[0.12em]">{lead.status}</span>
              </div>
              <p className="mt-2 text-sm text-muted-foreground">{lead.contactName} · <a className="font-semibold text-primary hover:underline" href={`mailto:${lead.email}`}>{lead.email}</a></p>
...
function sourceHref(sourcePath: string) {
  return sourcePath === '/sports-venues' || sourcePath.startsWith('/sports-venue/') ? sourcePath : null;
}
...
function statusLabel(status: SportsPartnerLeadStatus) {
