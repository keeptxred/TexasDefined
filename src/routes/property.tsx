...
function PropertyHub() {
  return <>
    <Container className="pb-12 pt-16 sm:pb-16 sm:pt-24"><p className="eyebrow text-primary">Home & property</p><h1 className="mt-3 max-w-5xl font-display text-5xl leading-[0.98] tracking-tight sm:text-7xl">Own a home in Texas with fewer surprises</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-muted-foreground">From an official-rate local tax estimate to exemptions, appraisal notices, county offices and long-term ownership costs, this is the starting point for the practical side of Texas property.</p><div className="mt-8 flex flex-wrap gap-3"><Link to="/texas-property-tax-estimator" className="rounded-sm bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground">Estimate local property taxes</Link><Link to="/learn/property-taxes" className="rounded-sm border border-border px-5 py-3 text-sm font-semibold">Understand the system</Link></div></Container>

    <section className="border-y border-border bg-muted/35"><Container className="py-12 sm:py-16"><p className="eyebrow text-primary">Start here</p><h2 className="mt-2 font-display text-3xl sm:text-4xl">Three ways into the property section</h2><div className="mt-8 grid gap-5 md:grid-cols-3">{startHere.map(item=><Link key={item.to} to={item.to} className="rounded-md border border-border bg-background p-6 transition-colors hover:border-primary/50"><h3 className="font-display text-2xl">{item.label}</h3><p className="mt-3 text-sm leading-6 text-muted-foreground">{item.body}</p><span className="mt-5 inline-block text-sm font-semibold text-primary">Open →</span></Link>)}</div></Container></section>
