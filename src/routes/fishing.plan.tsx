...
export const Route = createFileRoute("/fishing/plan")({
  ...
  head: ({ loaderData }) => {
    ...
    const jsonLd = {
      ...
      "@graph": [
        { "@type": "WebPage", url: `${siteUrl}${FISHING_TRIP_PLANNER_PATH}`, name: "Texas Fishing Trip Planner", description },
...
function FishingTripPlannerPage() {
  ...
  return <>
    <Container className="pt-8 sm:pt-10"><nav aria-label="Breadcrumb" className="text-[0.72rem] uppercase tracking-[0.14em] text-muted-foreground"><a href="/">Front page</a> · <a href="/fishing">Fishing</a> · Trip planner</nav></Container>
    <header className="mt-5 border-y border-border bg-ink text-ink-foreground"><Container className="py-14 sm:py-20"><p className="eyebrow text-ink-foreground/65">Texas Defined Fishing</p><h1 className="mt-4 max-w-5xl font-display text-5xl leading-[0.96] sm:text-7xl">Build a Texas fishing trip around the fish, not the hype.</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-ink-foreground/80">Filter complete, source-backed lake guides by target species and region.</p><div className="mt-8 flex gap-5 text-sm"><a href={FISHING_LAKE_COMPARE_PATH} className="border-b border-ink-foreground pb-1 font-semibold">Compare lakes →</a><a href="/fishing/reports" className="border-b border-ink-foreground/50 pb-1">Fishing reports →</a></div></Container></header>
    ...
    <Container className="py-12 sm:py-16">
      ...
      <section className="py-10"><div className="flex flex-wrap items-end justify-between gap-5"><div><p className="eyebrow text-primary">Planner results</p><h2 className="mt-2 font-display text-4xl">{filtered.length} matching lake{filtered.length === 1 ? "" : "s"}</h2></div><p className="max-w-xl text-xs text-muted-foreground">{data.policy.ranking}</p></div>
        ...
        {!filtered.length ? <div className="border-y border-border py-12"><h3 className="font-display text-3xl">No complete lake matches both filters.</h3><p className="mt-3 text-sm text-muted-foreground">The complete-guide catalog may not yet cover this combination.</p></div> : null}
      </section>

      <section className="border-y border-border py-10"><p className="eyebrow text-primary">Freshness rules</p><h2 className="mt-2 font-display text-3xl">Fishery fit is not today's conditions.</h2><p className="mt-5 text-sm leading-7 text-muted-foreground">{data.policy.conditions} {data.policy.local} Before travel, verify regulations, lake levels, access, weather and closures with official sources.</p></section>
    </Container>
  </>;
}
