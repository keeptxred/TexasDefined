...
export function loadLocalHomeAffordabilityPageServer(slug: string) {
  const profile = LOCAL_HOME_AFFORDABILITY_PROFILE_BY_SLUG.get(slug);
  if (!profile) return null;

  const pageUrl = absoluteUrl(texasDefinedBrand, profile.path);
  const siteUrl = absoluteUrl(texasDefinedBrand, '/');
  const calculatorUrl = absoluteUrl(texasDefinedBrand, '/texas-home-affordability-calculator');
      ...
      '@graph': [
        ...
        },
        {
          ...
          itemListElement: [
            ...
            { '@type': 'ListItem', position: 2, name: 'Texas home affordability calculator', item: calculatorUrl },
