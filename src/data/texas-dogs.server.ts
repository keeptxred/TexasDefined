...
const dogDesignCollections: DogDesignCollection[] = [
  ...
  { slug: "distinguished-dogs", name: "Distinguished Dogs", tagline: "Glasses on. Standards high.", description: "Dogs in glasses, ties and serious-looking poses paired with office titles and absurdly important responsibilities.", examples: ["Senior Treat Analyst", "Chief Barketing Officer", "Director of Human Resources", "Head of Neighborhood Intelligence"] },
  { slug: "dogs-with-jobs", name: "Dogs With Jobs", tagline: "Every household has management", description: "Breed-specific workplace humor for the dog who has appointed itself lifeguard, ranch manager, security department or quality-control inspector.", examples: ["Labrador Lifeguard", "Border Collie Ranch Manager", "German Shepherd Security Department", "Beagle Quality Control"] },
  { slug: "dogs-with-attitude", name: "Dogs With Attitude", tagline: "Small patience. Big personality.", description: "Deadpan expressions, side-eye and the kind of breed humor dog owners recognize immediately.", examples: ["Professional Side-Eye", "I Heard You. I Don’t Care.", "Treat Inspector", "My Human Works For Me"] },
  { slug: "texas-dogs", name: "Texas Dogs", tagline: "Dogs, Texas style", description: "Porch dogs, ranch dogs, lake dogs, road-trip dogs and breed graphics with a distinctly Texas setting rather than a generic state-outline treatment.", examples: ["Hill Country Hound", "West Texas Heeler", "Gulf Coast Golden", "Texas Ranch Dog"] },
  { slug: "lake-dogs", name: "Lake Dogs", tagline: "Wet dog. No regrets.", description: "Water-loving breeds, dock life, boats, towels, tennis balls and the particular chaos of taking a dog to the lake.", examples: ["Lake Dog", "Dock Supervisor", "Certified Boat Dog", "Swim. Shake. Repeat."] },
...
function buildHubScripts(breeds: DogBreedSummary[]) {
  const pageUrl = `${siteUrl}/dogs`;
  return [{ type: "application/ld+json", children: JSON.stringify({ "@context": "https://schema.org", "@graph": [
...
export function loadDogHubDataServer() {
  ...
  return { breeds, collections: dogDesignCollections, head: { ...buildHubHead(), scripts: buildHubScripts(breeds) } };
}
