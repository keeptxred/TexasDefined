...
export function loadMajorEventPageServer(slug: string) {
  const event = getMajorEventAuthorityServer(slug) ?? getExpandedMajorEventAuthorityServer(slug);
  ...
  const related = event.relatedLinks.map((item) => `<li><a class="font-semibold text-primary underline" href="${esc(item.href)}">${esc(item.label)}</a><span class="text-muted-foreground"> — ${esc(item.description)}</span></li>`).join("");
  ...
  const jsonLd = JSON.stringify({ "@context": "https://schema.org", "@type": "Event", name: event.name, url: canonicalUrl, startDate: event.startDate, endDate: event.endDate, eventStatus: "https://schema.org/EventScheduled", eventAttendanceMode: "https://schema.org/OfflineEventAttendanceMode", sameAs: event.officialUrl, location: { "@type": "Place", name: event.venue || event.city, address: { "@type": "PostalAddress", addressLocality: event.city, addressRegion: "TX", addressCountry: "US" } } });
  return {
    slug: event.slug,
    name: event.name,
    city: event.city,
