...
export function loadMajorEventPageServer(slug: string) {
  ...
  const eventYear = new Date(occurrenceWindows[0]?.startDate ?? event.startDate).getFullYear();
  ...
  return {
    ...
    description: `${event.name} ${eventYear} in ${event.city}, Texas: confirmed dates, location, official sources and practical trip-planning information.`,
    html,
    jsonLd,
  };
}
