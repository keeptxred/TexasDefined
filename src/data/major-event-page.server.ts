...
export function loadMajorEventPageServer(slug: string) {
  ...
  const occurrenceSchedule = occurrenceWindows.length > 1
    ? `<section class="mt-10 border-t border-border pt-8"><h2 class="font-display text-3xl">Scheduled event windows</h2><p class="mt-3 text-sm leading-6 text-muted-foreground">This event is not scheduled continuously across its outer date range. Use these organizer-supported occurrence windows.</p><ul class="mt-4 space-y-2">${occurrenceWindows.map((window) => `<li><strong>${esc(window.label || "Event window")}</strong><span class="text-muted-foreground"> — ${esc(formatDateRange(window.startDate, window.endDate, "en-US"))}</span></li>`).join("")}</ul></section>`
    : "";
  const html = `<nav class="mb-8 text-sm text-muted-foreground"><a href="/">Front page</a> / <a href="/events">Texas Events</a> / ${esc(event.name)}</nav><header class="border-b border-border pb-8"><p class="eyebrow text-primary">Major Texas event</p><h1 class="mt-3 font-display text-5xl sm:text-6xl">${esc(event.name)}</h1><p class="mt-5 text-lg text-muted-foreground">${esc(dateLabel)} · ${esc(placeLine)}</p>${event.dateNote ? `<p class="mt-4 text-sm text-muted-foreground">${esc(event.dateNote)}</p>` : ""}<p class="mt-5"><a class="font-semibold text-primary underline" href="${esc(event.officialUrl)}" target="_blank" rel="noreferrer noopener">Official event information ↗</a></p></header>${occurrenceSchedule}<section class="mt-12"><h2 class="font-display text-3xl">Why plan around ${esc(event.name)}?</h2><p class="mt-4 leading-7 text-muted-foreground">${esc(event.whyItMatters)}</p></section><section class="mt-12 border-t border-border pt-8"><h2 class="font-display text-3xl">Plan the visit</h2>${planning}</section><section class="mt-12 border-t border-border pt-8"><h2 class="font-display text-3xl">Keep exploring</h2><ul class="mt-4 space-y-3">${related}</ul></section><section class="mt-10 border-t border-border pt-8"><h2 class="font-display text-3xl">Official sources</h2><ul class="mt-4 space-y-3">${sources}</ul></section>`;
  const location = { "@type": "Place", name: event.venue || event.city, address: { "@type": "PostalAddress", addressLocality: event.city, addressRegion: "TX", addressCountry: "US" } };
  const eventNode = (window: MajorEventOccurrenceWindow, index?: number) => ({
    "@type": "Event",
    ...(typeof index === "number" ? { "@id": `${canonicalUrl}#occurrence-${index + 1}` } : {}),
  ...
  const jsonLd = JSON.stringify(occurrenceWindows.length > 1
    ? { "@context": "https://schema.org", "@graph": occurrenceWindows.map((window, index) => eventNode(window, index)) }
    : { "@context": "https://schema.org", ...eventNode(occurrenceWindows[0]) });
  return {
    slug: event.slug,
    name: event.name,
    city: event.city,
