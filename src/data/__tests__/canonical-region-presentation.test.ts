...
const hubLazyRoute = readFileSync(new URL("../../routes/regions.lazy.tsx", import.meta.url), "utf8");
const detailRoute = readFileSync(new URL("../../routes/regions.$region.tsx", import.meta.url), "utf8");
const detailLazyRoute = readFileSync(new URL("../../routes/regions.$region.lazy.tsx", import.meta.url), "utf8");
...
test("canonical landing routes reuse the geography graph and keep rendered UI lazy", () => {
  assert.match(hubRoute, /createFileRoute\("\/regions"\)/);
  assert.match(detailRoute, /createFileRoute\("\/regions\/\$region"\)/);
  assert.match(hubLazyRoute, /createLazyFileRoute\("\/regions"\)/);
  assert.match(detailLazyRoute, /createLazyFileRoute\("\/regions\/\$region"\)/);
  ...
  assert.match(detailLazyRoute, /region\.travelRegionIds/);
  assert.match(detailLazyRoute, /Statewide region navigation/);
  assert.doesNotMatch(presentationSource, /export const CANONICAL_PRIMARY_REGIONS\s*=/);
});

test("canonical region discovery cross-links without retiring travel routes", () => {
  assert.match(exploreRoute, /to="\/regions"/);
  assert.match(exploreRoute, /Travel regions/);
