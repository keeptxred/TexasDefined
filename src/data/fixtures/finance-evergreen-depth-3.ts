...
const rentingVsBuying = base({
  ...
  body: [
    ...
    h("Protect liquidity in both scenarios"),
    p("A rent-versus-buy worksheet should show what happens to cash after the move, not only what happens to the monthly housing line. Buying can require a down payment, closing costs, prepaid taxes or insurance, moving expenses and immediate repairs. Renting can require deposits, application or move-in charges, moving expenses and cash for the next transition. Keep an emergency reserve outside either transaction and do not treat home equity as if it were the same as cash in a savings account. If buying leaves the household unable to absorb an insurance deductible, vehicle repair or income interruption, the apparent monthly advantage is not the complete financial picture."),
    h("Make the Texas comparison address-specific"),
    p("Property taxes, homeowners insurance, HOA dues, special-district obligations, utilities and commuting costs can change materially between nearby Texas addresses. Build the buying case from the actual taxing units, an address-specific insurance quote, known association or district charges and realistic utility and transportation assumptions. Build the renting case from the actual lease terms, required fees, utilities and likely renewal exposure. Comparing a statewide average homeowner with a specific apartment—or vice versa—can create a result that looks precise but does not describe either real option."),
    h("Compare more than one realistic exit date"),
    p("Run the same two choices across several plausible holding periods rather than choosing one distant date that makes the preferred result look better. Include buying and selling transaction costs in each ownership case and moving costs in each rental case. If a job change, military move, family transition or uncertain commute could shorten the stay, make that shorter horizon visible. The purpose is not to forecast the housing market; it is to see how sensitive the decision is to the amount of time the household actually remains in the home."),
