...
export const texasHighSchoolFootballScoresSchedulesArticle: Article = {
  ...
  body: [
    ...
    p("Treat that scoreboard as a current-season source, not as a replacement for the school itself. If a game has been moved, canceled, delayed or assigned to a different stadium, the participating schools and districts remain the most practical place to verify the final game-day details."),
    ...
    p("That means a blank result, incomplete schedule or delayed update should be read as a data-availability problem first. Missing scoreboard data should not be converted into a loss, cancellation or forfeit unless a controlling source says so."),
    h("Why the scoreboard is not an official standings table"),
    ...
    p("That is why the district pages focus on the official 2026–28 membership and classification structure. We will add standings only when they can be maintained from a source that represents the certified district picture rather than a partial reconstruction."),
    h("A practical Friday-night workflow"),
    list(
      "Find the school in the statewide football directory.",
      ...
      "Use the stadium guide for parking and arrival when a verified venue guide exists.",
