...
import {
  loadTexasGatewayRegionalBatch7Article,
  texasGatewayRegionalBatch7Articles,
} from "./texas-gateway-regional-batch7";
...
export const texasCoreArticleStubs: Article[] = [...baseTexasCoreArticleStubs, ...texasGatewayRegionalBatch7Articles];
...
export async function loadTexasCoreArticle(brandId: string, slug: string): Promise<Article | null> {
  ...
  const gatewayArticle = await loadTexasGatewayRegionalBatch7Article(brandId, slug);
