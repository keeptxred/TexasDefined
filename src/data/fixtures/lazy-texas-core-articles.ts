...
import { texasGatewayLifestyleBatch4Articles, loadTexasGatewayLifestyleBatch4Article } from "./texas-gateway-lifestyle-batch4";
...
export const texasCoreArticleStubs: Article[] = [...baseTexasCoreArticleStubs, ...texasGatewayLifestyleBatch4Articles];
...
export async function loadTexasCoreArticle(brandId: string, slug: string): Promise<Article | null> {
  ...
  const gatewayLifestyleArticle = await loadTexasGatewayLifestyleBatch4Article(brandId, slug);
  if (gatewayLifestyleArticle) return gatewayLifestyleArticle;
