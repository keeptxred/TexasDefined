...
import { texasGatewayMonthlyBatch15Articles, loadTexasGatewayMonthlyBatch15Article } from "./texas-gateway-monthly-batch15";
...
export const texasCoreArticleStubs: Article[] = [...baseTexasCoreArticleStubs, ...texasGatewayMonthlyBatch15Articles];
...
export async function loadTexasCoreArticle(brandId: string, slug: string): Promise<Article | null> {
  ...
  const monthlyArticle = loadTexasGatewayMonthlyBatch15Article(slug);
  if (monthlyArticle) return monthlyArticle;
