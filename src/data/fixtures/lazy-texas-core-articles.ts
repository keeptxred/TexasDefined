...
import { texasGatewayItineraryBatch10Articles, loadTexasGatewayItineraryBatch10Article } from "./texas-gateway-itinerary-batch10";
...
export const texasCoreArticleStubs: Article[] = [...baseTexasCoreArticleStubs, ...texasGatewayItineraryBatch10Articles];
...
export async function loadTexasCoreArticle(brandId: string, slug: string): Promise<Article | null> {
  ...
  const itineraryArticle = await loadTexasGatewayItineraryBatch10Article(brandId, slug);
  if (itineraryArticle) return itineraryArticle;
