...
import { texasGatewayDecisionBatch11Articles, loadTexasGatewayDecisionBatch11Article } from "./texas-gateway-decision-batch11";
...
export const texasCoreArticleStubs: Article[] = [...baseTexasCoreArticleStubs, ...texasGatewayDecisionBatch11Articles];
...
export async function loadTexasCoreArticle(brandId: string, slug: string): Promise<Article | null> {
  ...
  const decisionArticle = await loadTexasGatewayDecisionBatch11Article(brandId, slug);
  if (decisionArticle) return decisionArticle;
