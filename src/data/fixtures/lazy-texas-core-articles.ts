...
import { texasGatewayBestOfBatch9Articles, loadTexasGatewayBestOfBatch9Article } from "./texas-gateway-bestof-batch9";
...
export const texasCoreArticleStubs: Article[] = [...baseTexasCoreArticleStubs, ...texasGatewayBestOfBatch9Articles];
...
export async function loadTexasCoreArticle(brandId: string, slug: string): Promise<Article | null> {
  ...
  const batch9Article = await loadTexasGatewayBestOfBatch9Article(brandId, slug);
  if (batch9Article) return batch9Article;
