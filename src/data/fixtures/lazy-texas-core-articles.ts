...
import { texasGatewayLifestyleBatch3, loadTexasGatewayLifestyleArticle } from "./texas-gateway-lifestyle-batch3";
...
export const texasCoreArticleStubs: Article[] = [...baseTexasCoreArticleStubs, ...texasGatewayLifestyleBatch3];
...
export async function loadTexasCoreArticle(brandId: string, slug: string): Promise<Article | null> {
  ...
  const lifestyleGateway = await loadTexasGatewayLifestyleArticle(brandId, slug);
  if (lifestyleGateway) return lifestyleGateway;
