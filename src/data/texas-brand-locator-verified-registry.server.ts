...
const locationPromises = new Map<TexasBrandLocatorBrand, Promise<VerifiedBrandLocation[]>>();
const coordinatePromises = new Map<TexasBrandLocatorBrand, Promise<Map<string, Point>>>();
...
function assertVerifiedRegistryBrand(brand: TexasBrandLocatorBrand) {
...
async function loadLocationsFromSupabase(brand: TexasBrandLocatorBrand) {
  assertVerifiedRegistryBrand(brand);
  ...
  const pending = (async () => {
    ...
    if (error) throw new Error(`Brand location registry query failed for ${brand}: ${error.message}`);
    ...
    if (!locations.length) throw new Error(`${texasBrandLocatorLabel(brand)} registry returned no active public Texas locations`);
    return locations;
  })().catch((error) => {
  ...
  locationPromises.set(brand, pending);
  return pending;
}

async function loadCoordinatesFromCensus(brand: TexasBrandLocatorBrand, locations: VerifiedBrandLocation[]) {
...
async function loadCoordinatesFallback(brand: TexasBrandLocatorBrand, locations: VerifiedBrandLocation[]) {
  const coordinates = new Map<string, Point>();
  const batchSize = 6;
...
async function persistCoordinates(coordinates: Map<string, Point>) {
...
async function coordinatesForBrand(brand: TexasBrandLocatorBrand, locations: VerifiedBrandLocation[]) {
  ...
  const pending = (async () => {
    ...
    await persistCoordinates(resolved).catch((error) => {
  ...
  coordinatePromises.set(brand, pending);
  return pending;
}

function normalizeLocation(location: VerifiedBrandLocation, point: Point, origin: Point): TexasBrandLocatorLocation {
  const address = locationAddress(location);
  const label = texasBrandLocatorLabel(location.brand);
  return {
    id: location.id,
    ...
    sourceUrl: location.sourceUrl || texasBrandLocatorOfficialUrl(location.brand),
  };
}

export async function findVerifiedBrandLocationsNearPointServer(
  brand: TexasBrandLocatorBrand,
  origin: Point,
  ...
  try {
    ...
    return locations
      .map((location) => {
        ...
        return point ? normalizeLocation(location, point, origin) : null;
      })
    ...
    try {
      const { findVerifiedBrandLocationsViaPublicRpcServer } = await import("./texas-brand-locator-rpc.server");
      return await findVerifiedBrandLocationsViaPublicRpcServer(brand, origin);
    } catch (rpcError) {
      const primaryMessage = primaryError instanceof Error ? primaryError.message : String(primaryError);
      const rpcMessage = rpcError instanceof Error ? rpcError.message : String(rpcError);
      throw new Error(`${texasBrandLocatorLabel(brand)} registry unavailable through both server paths: ${primaryMessage}; fallback: ${rpcMessage}`);
    }
  }
}
