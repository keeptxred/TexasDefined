...
type ExploreAuthoritySection = {
  heading: string;
  paragraphs: string[];
  bullets?: string[];
};

type ExploreAuthorityGuide = {
  title: string;
  dek: string;
...
const exploreCategoryAuthorityServer: Partial<Record<CategorySlug, ExploreAuthorityGuide>> = {
  outdoors: {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "The same seasonal thinking applies beyond birds. Bat emergence, wildflower bloom, heat, river flow and nighttime temperatures all change the value and difficulty of an outing. A useful Texas itinerary should explain when a place is most rewarding and what conditions could change the plan. Before a long drive, check the managing agency's current alerts, hours, reservation rules, burn restrictions and trail conditions.",
        ],
      },
      {
...
function renderGuide(category: CategorySlug, guide: ExploreAuthorityGuide): string {
  const headingId = `texas-field-guide-${category}`;
  const sectionHtml = guide.sections.map((section) => {
    const paragraphs = section.paragraphs.map((paragraph) => `<p class="mt-4 text-base leading-8 text-muted-foreground">${escapeHtml(paragraph)}</p>`).join("");
    const bullets = section.bullets?.length
      ? `<ul class="mt-5 list-disc space-y-2 pl-5 text-sm leading-7 text-muted-foreground">${section.bullets.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>`
      : "";
    return `<section class="border-t border-border pt-7"><h3 class="font-display text-2xl text-foreground">${escapeHtml(section.heading)}</h3>${paragraphs}${bullets}</section>`;
  }).join("");

  const sourceHtml = guide.sources.map((source) => {
    const url = safeExternalUrl(source.url);
    return url ? `<li><a class="underline decoration-border underline-offset-4 hover:text-foreground" href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(source.name)}</a></li>` : "";
  }).join("");

  const relatedHtml = guide.relatedLinks.map((item) => {
    const href = safeInternalHref(item.href);
    return href ? `<li><a class="underline decoration-border underline-offset-4 hover:text-foreground" href="${escapeHtml(href)}">${escapeHtml(item.label)}</a></li>` : "";
  }).join("");

  return `<section class="border-y border-border bg-surface/40" aria-labelledby="${escapeHtml(headingId)}"><div class="mx-auto w-full max-w-[1240px] px-5 py-12 sm:px-8 sm:py-16 lg:px-10"><p class="eyebrow text-primary">Texas field guide</p><h2 id="${escapeHtml(headingId)}" class="mt-3 max-w-4xl font-display text-4xl leading-tight sm:text-5xl">${escapeHtml(guide.title)}</h2><p class="mt-5 max-w-3xl text-base leading-8 text-muted-foreground">${escapeHtml(guide.dek)}</p><div class="mt-10 grid gap-8">${sectionHtml}</div><div class="mt-10 grid gap-8 border-t border-border pt-8 md:grid-cols-2"><section><h3 class="font-display text-xl">Official sources</h3><ul class="mt-4 space-y-2 text-sm leading-7 text-muted-foreground">${sourceHtml}</ul></section><section><h3 class="font-display text-xl">Keep exploring</h3><ul class="mt-4 space-y-2 text-sm leading-7 text-muted-foreground">${relatedHtml}</ul></section></div></div></section>`;
}

export function getExploreCategoryAuthorityHtmlServer(category: CategorySlug): string | null {
  const guide = exploreCategoryAuthorityServer[category];
  return guide ? renderGuide(category, guide) : null;
}
