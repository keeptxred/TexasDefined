...
export async function buildFishingSearchDocuments(): Promise<SearchDocument[]> {
  ...
  const [lakes, species, techniques, guides, reports, accessPointsRaw, tackleShopsRaw, businessesRaw, lakeSpecies, lakeTechniques] = await Promise.all([
  ...
  const completeLakeCount = lakes.filter((lake) => isCompleteFishingLakeSlug(lake.slug)).length;
  ...
  for (const relation of lakeTechniques) {
    const lake = lakeById.get(relation.lakeId);
    if (!lake || !isCompleteFishingLakeSlug(lake.slug) || !relation.sources.length || !relation.verifiedAt) continue;
    const values = techniqueKeywords.get(relation.techniqueId) ?? [];
    values.push(lake.name, ...relation.speciesIds.map((speciesId) => speciesById.get(speciesId)?.commonName).filter((value): value is string => Boolean(value)), ...relation.seasons);
    techniqueKeywords.set(relation.techniqueId, values);
  }
  ...
  const documents: SearchDocument[] = [
    ...
    { id: "fishing-directory:texas-fishing-lakes", brandId: "texasdefined", kind: "guide", title: "Texas Fishing Lakes", summary: `Compare ${completeLakeCount} complete TexasDefined fishing-lake guides by region, size, location and verified fishery strengths before opening the full lake guide.`, keywords: ["Texas fishing lakes", "Texas lake fishing", "Lake Conroe", "Lake Fork", "Sam Rayburn Reservoir", "Lake Livingston", "Lake Texoma", "compare fishing lakes"], href: "/fishing/lakes" },
    ...
    { id: "fishing-directory:texas-fishing-techniques", brandId: "texasdefined", kind: "guide", title: "Texas Fishing Techniques", summary: "Browse source-backed fishing techniques connected to complete Texas lake guides, target species and seasonal relationships without live-bite or product-ranking claims.", keywords: ["Texas fishing techniques", "fishing methods", "soft plastics", "crankbaits", "spinnerbaits", "topwater", "trolling", "vertical jigging", "jigs and minnows", "live bait", "cut bait"], href: FISHING_TECHNIQUES_DIRECTORY_PATH },
    ...
    ...publicTechniques.filter((technique) => (techniqueKeywords.get(technique.id)?.length ?? 0) > 0).map((technique): SearchDocument => ({ id: `fishing-technique:${technique.id}`, brandId: technique.brandId, kind: "guide", title: `${technique.name} fishing in Texas`, summary: technique.summary, keywords: [technique.name, technique.category, "Texas fishing technique", ...(techniqueKeywords.get(technique.id) ?? [])], href: fishingTechniqueCanonicalPath(technique.slug) })),
