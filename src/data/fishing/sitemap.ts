...
import { EXPANDED_SHOWCASE_LAKE_SLUGS, SHOWCASE_LAKE_SECTION_SLUGS, SHOWCASE_LAKE_SLUGS, SHOWCASE_LAKE_VERIFIED_AT, WAVE2_SHOWCASE_LAKE_SLUGS, WAVE2_SHOWCASE_LAKE_VERIFIED_AT, showcaseLakeCanonicalPath } from "./showcase-lake-routing";
...
export const FISHING_LAKES_DIRECTORY_VERIFIED_AT = WAVE2_SHOWCASE_LAKE_VERIFIED_AT;
const ALL_SHOWCASE_LAKE_SLUGS = [...SHOWCASE_LAKE_SLUGS, ...EXPANDED_SHOWCASE_LAKE_SLUGS, ...WAVE2_SHOWCASE_LAKE_SLUGS] as const;
const WAVE2_SET = new Set<string>(WAVE2_SHOWCASE_LAKE_SLUGS);
...
export const FISHING_SITEMAP_ENTRIES = [
  ...
  ...ALL_SHOWCASE_LAKE_SLUGS.flatMap((slug) => [
    { path: showcaseLakeCanonicalPath(slug), lastmod: WAVE2_SET.has(slug) ? WAVE2_SHOWCASE_LAKE_VERIFIED_AT : SHOWCASE_LAKE_VERIFIED_AT },
    ...SHOWCASE_LAKE_SECTION_SLUGS.map((section) => ({ path: showcaseLakeCanonicalPath(slug, section), lastmod: WAVE2_SET.has(slug) ? WAVE2_SHOWCASE_LAKE_VERIFIED_AT : SHOWCASE_LAKE_VERIFIED_AT })),
  ]),
