...
export const CANONICAL_PRIMARY_REGIONS: readonly CanonicalPrimaryRegionDefinition[] = [
  {
    ...
    subregionIds: ["dallas-fort-worth-metroplex", "north-texas-prairies"],
  ...
  },
  {
    ...
    subregionIds: ["austin-area", "texas-hill-country", "central-texas-prairies"],
  ...
  },
  {
    ...
    subregionIds: ["piney-woods", "upper-east-texas"],
  ...
  },
  {
    ...
    subregionIds: ["houston-area", "upper-gulf-coast", "coastal-bend"],
...
/**
 * Explicit boundary rules. Gateway relationships provide editorial/travel
 * context without assigning a second primary region.
 */
export const BOUNDARY_CITY_GEOGRAPHY: readonly BoundaryCityGeography[] = [
  ...
  },
  {
    ...
    subregionIds: ["san-antonio-area"],
    gatewaySubregionIds: ["texas-hill-country"],
    metroId: "san-antonio",
    countySlugs: ["bexar"],
    adjacentRegionIds: ["central-texas"],
    aliases: ["San Antonio, Texas", "San Antonio, TX"],
    travelRegionIds: ["hill-country", "south-texas"],
    relocationPresentationLabels: ["San Antonio & Hill Country"],
  },
] as const;

const canonicalPrimaryRegionIdSet = new Set<string>(CANONICAL_PRIMARY_REGION_IDS);
const legacyTravelRegionIdSet = new Set<TexasRegion>([
  "big-bend",
  "gulf-coast",
  "hill-country",
  "panhandle",
...
export function assertCanonicalPrimaryRegionId(value: string): asserts value is CanonicalPrimaryRegionId {
  if (!isCanonicalPrimaryRegionId(value)) {
    throw new Error(`Unknown canonical primary region: ${value}`);
  }
}
...
export function validateCanonicalGeography(): readonly string[] {
  ...
  const subregionsById = new Map<TexasSubregionId, TexasSubregionDefinition>(
    TEXAS_SUBREGIONS.map((subregion) => [subregion.id, subregion] as const),
  );
  const metrosById = new Map<TexasMetroId, TexasMetroDefinition>(
    TEXAS_METROS.map((metro) => [metro.id, metro] as const),
  );

  if (CANONICAL_PRIMARY_REGIONS.length !== CANONICAL_PRIMARY_REGION_IDS.length) {
    errors.push(`Expected ${CANONICAL_PRIMARY_REGION_IDS.length} canonical primary regions; found ${CANONICAL_PRIMARY_REGIONS.length}.`);
  }
  ...
  for (const id of CANONICAL_PRIMARY_REGION_IDS) {
    if (!regionIds.has(id)) errors.push(`Missing canonical primary region definition for ${id}.`);
  }
  ...
  for (const region of CANONICAL_PRIMARY_REGIONS) {
    ...
    for (const adjacentId of region.adjacentRegionIds) {
      ...
      if (reciprocal && !reciprocal.adjacentRegionIds.includes(region.id)) {
        errors.push(`Adjacency must be reciprocal: ${region.id} -> ${adjacentId}.`);
      }
    }
    ...
    for (const travelRegionId of region.travelRegionIds) {
      if (!legacyTravelRegionIdSet.has(travelRegionId)) errors.push(`${region.id} references unknown travel region ${travelRegionId}.`);
    }
  }

  for (const subregion of TEXAS_SUBREGIONS) {
    if (!regionIds.has(subregion.primaryRegionId)) {
      errors.push(`${subregion.id} references unknown primary region ${subregion.primaryRegionId}.`);
    } else {
      const owner = CANONICAL_PRIMARY_REGIONS.find((region) => region.id === subregion.primaryRegionId);
      if (owner && !owner.subregionIds.includes(subregion.id)) {
        errors.push(`${subregion.id} is not listed by its owning primary region ${subregion.primaryRegionId}.`);
      }
    }
  ...
  for (const city of BOUNDARY_CITY_GEOGRAPHY) {
    ...
    for (const gatewaySubregionId of city.gatewaySubregionIds ?? []) {
      if (!subregionsById.has(gatewaySubregionId)) errors.push(`${city.citySlug} references unknown gateway subregion ${gatewaySubregionId}.`);
    }
    ...
    for (const travelRegionId of city.travelRegionIds ?? []) {
      if (!legacyTravelRegionIdSet.has(travelRegionId)) errors.push(`${city.citySlug} references unknown travel region ${travelRegionId}.`);
    }
  }
