...
export function fingerprintContentDecision(candidate: ContentCandidate, decision: ContentDecision): string {
  return fnv1aFingerprint({ candidate: {
    id: candidate.id, domain: candidate.domain, sourceSite: candidate.sourceSite,
    targetSite: candidate.targetSite, sourceCanonicalUrl: candidate.sourceCanonicalUrl,
    proposedUrl: candidate.proposedUrl ?? null, contentFingerprint: candidate.contentFingerprint ?? null,
    sourceFingerprint: candidate.sourceFingerprint ?? null, derivativePurpose: candidate.derivativePurpose ?? null,
  }, decision });
}
...
export function validatePublicationOverride(override: PublicationOverride | undefined, candidate: ContentCandidate, decision: ContentDecision, now = new Date()) {
  ...
  const expected = createPublicationOverride({ candidateId: override.candidateId, targetSite: override.targetSite,
    decisionFingerprint: override.decisionFingerprint, reviewer: override.reviewer, reason: override.reason,
    reviewedAt: override.reviewedAt, expiresAt: override.expiresAt });
...
export function enforcePublicationDecision(candidate: ContentCandidate, decision: ContentDecision, override?: PublicationOverride, now = new Date()): PublicationGateResult {
  ...
  if (decision.disposition === 'publish-original' || decision.disposition === 'publish-derivative-with-canonical-reference')
    return { status: 'allowed', publishable: true, decisionFingerprint, overrideValid: false, reasons };
  if (decision.disposition === 'reject-duplicate' || decision.disposition === 'cross-link-only') {
    reasons.push('This disposition cannot be overridden for publication.');
    return { status: 'blocked', publishable: false, decisionFingerprint, overrideValid: false, reasons };
  }
  ...
  return { status: 'allowed', publishable: true, decisionFingerprint, overrideValid: true,
    reasons: [...reasons, 'A valid reviewed override permits this manual-review candidate to proceed.'] };
}
