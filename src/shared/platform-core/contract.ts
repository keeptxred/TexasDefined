export const PLATFORM_CORE_CONTRACT = {
  ...
  capabilities: [
    'entity-contracts', 'entity-canonicalization', 'deterministic-fingerprints',
    'entity-set-diffs', 'baseline-quarantine', 'promotion-previews',
    'content-ownership', 'duplicate-content-prevention', 'cross-site-disposition',
    'publication-gates', 'reviewed-overrides', 'governance-events',
    'governance-analytics', 'ownership-drift-detection',
  ],
...
export type PlatformCoreConsumerManifest = {
  consumer: 'TexasDefined' | 'KeepTXRed'; repository: string; coreRepository: 'keeptxred/texas-common-core';
  coreCommit: string; packageVersion: string; apiVersion: string; capabilities: PlatformCoreCapability[];
  localExtensions: string[]; excludedDomains: string[]; reviewedAt: string;
};
