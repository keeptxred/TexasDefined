...
const CITY_RESOURCE_LINKS = [
  ...
  { href: '/property-tax-calculators', label: 'Property-tax calculators', copy: 'Compare the property-tax side of a move or home purchase with the Texas property-tax calculators.' },
  ...
  { href: '/explore/food-bbq', label: 'Texas food & barbecue', copy: 'Connect city planning with Texas restaurant history, regional food traditions and worthwhile road-trip stops.' },
  { href: '/explore/trip-planner', label: 'Texas trip planner', copy: 'Turn the city into a travel base and find nearby destinations, routes and weekend ideas.' },
] as const;
...
export function EntityDepthSections({ entity, related }: { entity: TexasEntityRecord; related: RankedRelatedEntity[] }) {
  ...
  return <>
    ...
    {cityProfile ? <section className="border-b border-border py-12" aria-labelledby="city-systems-heading">
      <div className="grid gap-8 lg:grid-cols-[14rem_1fr]">
        <div>
          ...
          <h2 id="city-systems-heading" className="mt-2 font-display text-4xl">{entity.name} essentials</h2>
        </div>
        <div>
          <div className="border-y border-border py-5">
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">The completed 2020 Census count provides a stable baseline; newer population estimates can move from year to year.</p>
          </div>
    ...
    <section className="border-b border-border py-12" aria-labelledby="entity-practical-heading">
      <div className="grid gap-8 lg:grid-cols-[14rem_1fr]">
        <div>
          <p className="eyebrow text-primary">Double-check first</p>
          <h2 id="entity-practical-heading" className="mt-2 font-display text-4xl">Details that can change</h2>
        </div>
        <div>
          <p className="max-w-3xl text-base leading-7 text-muted-foreground">Some details change faster than the basics on this page. Use the checklist below to confirm what matters for your visit, transaction or request.</p>
    ...
    {entity.kind === 'city' ? <section className="border-b border-border py-12" aria-labelledby="city-resource-heading">
      <div className="grid gap-8 lg:grid-cols-[14rem_1fr]">
        <div>
          ...
          <h2 id="city-resource-heading" className="mt-2 font-display text-4xl">Useful guides for {entity.name}</h2>
        </div>
        <div>
          <p className="max-w-3xl text-base leading-7 text-muted-foreground">Use these links to move from {entity.name} into relocation, property, schools, driving and trip-planning guides without repeating the same information here.</p>
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            {CITY_RESOURCE_LINKS.map((resource) => <a key={resource.href} href={resource.href} className="border border-border p-5 hover:border-primary/60"><strong className="font-display text-xl leading-tight">{resource.label}</strong><span className="mt-2 block text-sm leading-6 text-muted-foreground">{resource.copy}</span><span className="mt-3 block text-sm font-semibold text-primary">Read guide →</span></a>)}
          </div>
        </div>
      </div>
    </section> : null}
    ...
    {relatedItems.length ? <section className="border-b border-border py-12" aria-labelledby="entity-connections-heading">
      <div className="grid gap-8 lg:grid-cols-[14rem_1fr]">
        ...
        <div>
          <p className="max-w-3xl text-base leading-7 text-muted-foreground">Use these related guides to connect {entity.name} with its county, region, nearby places and other useful context.</p>
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {relatedItems.map(({ entity: candidate }) => <a key={candidate.id} href={canonicalEntityPath(candidate)} className="border border-border p-5 hover:border-primary/60"><span className="eyebrow text-primary">{title(candidate.kind)}</span><strong className="mt-2 block font-display text-xl leading-tight">{candidate.name}</strong><span className="mt-2 block text-sm leading-6 text-muted-foreground">Read the related guide →</span></a>)}
          </div>
        </div>
      </div>
    </section> : null}
...
function buildContextItems(entity: TexasEntityRecord, countyName: string | null, regionName: string | null) {
  ...
  if (governmentKinds.has(entity.kind)) {
    return [
      `${entity.name} is a public-service office or agency. This guide helps explain what it handles, points to the official source and makes it easier to reach the correct government website for a transaction or rule.`,
      entity.kind === 'appraisal-district'
        ...
        : entity.kind === 'tax-office'
          ...
          : entity.kind === 'agency'
            ...
            : `Local-government responsibilities vary by office. Use the official source on this page for records, filings, forms and services controlled by that office.`,
  ...
  if (sportsKinds.has(entity.kind) || entity.kind === 'sports-venue') {
    return [
      `${entity.name} is a sports venue guide built around the practical parts of the trip: where the venue is, what surrounds it and where to verify current event-day details before buying tickets or traveling.`,
  ...
  if (outdoorKinds.has(entity.kind)) {
    return [
      `${entity.name} is an outdoor destination where access, weather, water conditions, reservations and seasonal restrictions can all change the experience. Check those details before making the drive.`,
  ...
  if (historyKinds.has(entity.kind)) {
    return [
      `This guide places ${entity.name} in the larger geography and history around it instead of treating the site or institution as an isolated stop.`,
  ...
  if (eventKinds.has(entity.kind)) {
    return [
      `${entity.name} is an event guide with the place context needed to plan around it. Dates, tickets, gates, parking and programming remain live details controlled by the organizer.`,
  ...
  if (fishingKinds.has(entity.kind) || entity.kind.includes('fishing')) {
    return [
      `${entity.name} is a fishing guide that connects species or water-body information with current regulations, access conditions and the larger lake or river context.`,
  ...
  if (entity.kind === 'city') {
    return [
      `${entity.name} is connected here with its county, region, nearby destinations and practical guides for living in or visiting the area.`,
      ...
      `Use the related guides to move from the city overview into nearby places, property information, outdoor destinations and other useful local context.`,
    ];
  }

  return [
    `${entity.name} connects to a specific place, activity, institution or Texas story. This page adds geographic and related-guide context so the name is useful rather than isolated.`,
    ...
    entity.officialUrl ? `Where an official source exists, it is linked directly so changing hours, fees, rules or access details can be checked at the source.` : `Operational details can change, so verify current hours, access, fees or rules with the responsible organization before acting on them.`,
  ];
}
...
function practicalChecklist(entity: TexasEntityRecord) {
  ...
  return [
    ...
    { title: 'Nearby context', copy: 'Use the related guides to understand the county, region and other places connected to this reference.' },
  ];
}
...
function quickAnswers(entity: TexasEntityRecord, countyName: string | null, regionName: string | null) {
  ...
  if (entity.officialUrl) answers.push({
    ...
    answer: `Use the official website linked on this page for current schedules, fees, forms, rules, hours or operational notices. This is an independent guide and does not replace the responsible agency, venue, park, office or organizer.`, 
  });

  if (entity.kind === 'city') answers.push({
    question: `Where should I start if I am considering a move to ${entity.name}?`,
    answer: `Start with the Moving to Texas guide and checklist linked above, then verify the exact county, school district, utility service area, property-tax jurisdictions and commute pattern for the address you are considering. The city name alone does not determine every local system that applies.`, 
  });

  if (governmentKinds.has(entity.kind)) answers.push({
    question: `Is this the official website for ${entity.name}?`, 
    answer: `No. This is an independent guide. The official government source is linked on this page when it has been verified.`, 
  });
