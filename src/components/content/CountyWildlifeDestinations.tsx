...
const federalRefugeLinks: CountyWildlifeLink[] = [
  ...
  { counties: ['Cameron', 'Hidalgo', 'Starr', 'Willacy'], name: 'Lower Rio Grande Valley National Wildlife Refuge', slug: 'lower-rio-grande-valley-national-wildlife-refuge' },
...
export function CountyWildlifeDestinations({ countyName }: { countyName: string }) {
  ...
  return (
    <section className="mt-8 border-t border-border pt-6" aria-labelledby={`county-wildlife-${countySlug}`}>
      ...
      <h3 id={`county-wildlife-${countySlug}`} className="mt-2 font-display text-3xl">National wildlife refuges connected to {countyName}</h3>
      <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">These TexasDefined refuge records are tied to {countyName} through verified federal location data and point to the U.S. Fish and Wildlife Service for current visitor conditions.</p>
