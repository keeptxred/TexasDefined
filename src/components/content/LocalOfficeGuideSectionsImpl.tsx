...
export function LocalOfficeGuideSectionsImpl({ entity, profile }: { entity: TexasEntityRecord; profile: LocalGovernmentProfile | null }) {
  ...
  return <>
    <section className="grid gap-8 border-b border-border py-12 lg:grid-cols-[14rem_1fr]">
      <div><p className="eyebrow text-primary">Verified local office</p><h2 className="mt-2 font-display text-4xl">{appraisal ? 'Property appraisal contacts' : 'Tax and vehicle contacts'}</h2></div>
      <div>
        <OfficeFacts office={office} fallbackUrl={entity.officialUrl} />
        <p className="mt-6 max-w-3xl text-sm leading-7 text-muted-foreground">{appraisal
    ...
    <section className="grid gap-10 border-b border-border py-12 lg:grid-cols-2">
      ...
      <div>
        <p className="eyebrow text-primary">Before you contact the office</p><h2 className="mt-2 font-display text-4xl">Have the record in front of you</h2>
        <ol className="mt-6 space-y-4 text-sm leading-7 text-muted-foreground">{(appraisal ? [
        ...
        ]).map((item, index) => <li key={item} className="grid grid-cols-[2rem_1fr] gap-3 border-t border-border pt-3"><span className="font-display text-xl text-primary">{index + 1}</span><span>{item}</span></li>)}</ol>
      </div>
    </section>

    <section className="border-b border-border py-12"><div className="grid gap-8 lg:grid-cols-[14rem_1fr]"><div><p className="eyebrow text-primary">Property-tax workflow</p><h2 className="mt-2 font-display text-4xl">Where this office fits</h2></div><div className="grid gap-x-8 md:grid-cols-2">
    ...
    <section className="border-b border-border py-12"><div className="grid gap-8 lg:grid-cols-[14rem_1fr]"><div><p className="eyebrow text-primary">Next steps</p><h2 className="mt-2 font-display text-4xl">{countyName} tools and guides</h2></div><div className="grid sm:grid-cols-2">
      ...
      {appraisal ? <><GuideLink href="/learn/appraisal-districts" title="Texas appraisal districts explained">Understand appraisal records, values, notices, exemptions, and the role of the appraisal review board.</GuideLink><GuideLink href="/do/property-tax-protest" title="Texas property-tax protest guide">Prepare for the protest process, deadlines, evidence, and next steps.</GuideLink><GuideLink href="/do/homestead-exemption" title="Texas homestead exemption guide">Learn what the exemption does and how the local application process fits into the tax system.</GuideLink></> : <><GuideLink href="/learn/property-tax-payments" title="Paying Texas property taxes">Understand bills, collectors, due dates, payment records, and delinquency basics.</GuideLink><GuideLink href="/texas-vehicle-registration" title="Texas vehicle registration guide">Renewals, title and registration responsibilities, TxDMV, and county offices.</GuideLink><GuideLink href="/find-my-dmv" title="Find the right vehicle office">Separate county tax-office transactions from DPS driver-license services.</GuideLink></>}
    </div></div></section>

    <section className="grid gap-8 border-b border-border py-10 lg:grid-cols-[14rem_1fr]"><div><p className="eyebrow text-primary">Source notes</p><h2 className="mt-2 font-display text-3xl">Verify the live local record</h2></div><div className="max-w-3xl space-y-3 text-sm leading-7 text-muted-foreground">
...
function OfficeFacts({ office, fallbackUrl }: { office?: LocalOfficeProfile; fallbackUrl?: string }) {
  ...
  const facts = [office?.name ? ['Officeholder', office.name] : undefined, office?.phone ? ['Phone', office.phone] : undefined, office?.email ? ['Email', office.email] : undefined, office?.address ? ['Street address', office.address] : undefined, office?.lastUpdated ? ['Directory updated', formatDate(office.lastUpdated)] : undefined].filter((item): item is string[] => Boolean(item));
  return <div className="grid gap-4 sm:grid-cols-2">{facts.map(([label, value]) => <div key={label} className="border-t border-border pt-3"><span className="text-xs uppercase tracking-[0.12em] text-muted-foreground">{label}</span><strong className="mt-1 block text-sm leading-6">{value}</strong></div>)}{website ? <div className="border-t border-border pt-3"><span className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Official website</span><a href={website} target="_blank" rel="noreferrer" className="mt-1 block text-sm font-semibold underline decoration-primary/50 underline-offset-4 hover:text-primary">Open office website ↗</a></div> : null}</div>;
}
