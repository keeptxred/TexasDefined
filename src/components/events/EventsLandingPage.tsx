...
const routeDescription = "Search verified Texas events, ticket availability and commission-safe partner offers by place, date and event type.";
const EVENT_LABELS: Record<TexasEvent["category"], string> = { music: "Live Music", food: "Food & Drink", rodeo: "Rodeo", seasonal: "Seasonal", sport: "Sports", culture: "Arts & Culture" };

export type EventSearch = { featured: string; q: string; location: string; start: string; end: string; category: string; venue: string; offer: string };
type CalendarView = {
  monthLabel: string; totalCount: number; dateSummary: string; resultsHeading: string; truncated: boolean; showClearDates: boolean;
  previousHref: string; todayHref: string; nextHref: string; clearDatesHref: string; nextEventHref: string;
  categories: TexasEvent["category"][]; cityOptions: string[]; countyOptions: Array<[string, string]>; venueOptions: Array<[string, string]>;
  ...
  results: Array<{ id: string; category: TexasEvent["category"]; guidePath: string; title: string; city: string; countyName?: string; venueName?: string; venuePath?: string; summary: string; officialEventUrl: string; dateLabel: string; lastVerifiedLabel: string; ticketCta: ResolvedEventTicketCta | null }>;
};
...
export interface EventsLandingData {
  events: TexasEvent[]; regions: Region[]; majorEventGuides: MajorEventGuide[]; eventTimingLinks: DiscoveryLink[]; eventTopicLinks: DiscoveryLink[]; eventRegionLinks: DiscoveryLink[];
  featuredVenueGuide: FeaturedVenueGuide; featuredDateLabel: string; upcomingEventRecords: CarouselEvents; calendarView: CalendarView;
}

export function EventsLandingPage({ data, search, breadcrumb }: { data: EventsLandingData; search: EventSearch; breadcrumb: ReactNode }) {
  ...
  return <>
    <section className="ev-hero">
      ...
      <Container className="ev-hero-inner">
        {breadcrumb}
        <p className="eyebrow ev-kicker">Texas Events & Offers</p>
        <h1 className="ev-h1">What are you searching for today?</h1>
    ...
    <Container><TexasEventCarousel events={upcomingEventRecords} title="Upcoming events across Texas" eyebrow="Verified statewide calendar" viewAllHref="/events#calendar" /></Container>

    <Section><Container><SectionHeader eyebrow="Search Texas events & offers" title="Find the right event for the right trip" description="Search by keyword, location, dates, event type and ticket or deal availability. Every filtered view has a shareable URL." /><GlobalEventCalendar calendarView={calendarView} regions={regions} search={search} /></Container></Section>
...
function GlobalEventCalendar({ calendarView, regions, search }: { calendarView: CalendarView; regions: Region[]; search: EventSearch }) {
  return <div id="calendar" className="ev-cal">
    <form action="/events" className="ev-filter">
      <label><span className="eyebrow ev-label">Search</span><input name="q" defaultValue={search.q} className="ev-select" placeholder="Concert, rodeo, festival, team…" /></label>
      ...
      <label><span className="eyebrow ev-label">From</span><input type="date" name="start" defaultValue={search.start} className="ev-select" /></label>
      <label><span className="eyebrow ev-label">Through</span><input type="date" name="end" defaultValue={search.end} className="ev-select" /></label>
      ...
      <label><span className="eyebrow ev-label">Ticket / offer</span><select name="offer" defaultValue={search.offer} className="ev-select"><option value="">All events</option><option value="tickets">Tickets available</option><option value="affiliate">Commissionable affiliate links</option><option value="deals">Verified Texas Defined Deals</option></select></label>
      ...
      <div className="ev-filter-actions"><button type="submit" className="eyebrow ev-filter-action">Search / apply →</button>{(search.q || search.location || search.category || search.venue || search.offer || search.start || search.end) && <a href="/events#calendar" className="eyebrow ev-filter-action ev-filter-clear">Clear all</a>}</div>
    </form>
    <div className="ev-cal-layout"><div>
    ...
    </div><aside className="ev-status"><p className="eyebrow ev-e">Calendar status</p><p className="ev-status-count">{calendarView.totalCount.toLocaleString("en-US")} {calendarView.totalCount === 1 ? "event" : "events"}</p><p className="ev-status-copy">{calendarView.dateSummary}</p>{calendarView.truncated && <p className="ev-status-small">Showing the first {calendarView.results.length}; narrow the date or filters for a shorter list.</p>}</aside></div>
    <div id="calendar-results" className="ev-results"><div className="ev-results-head"><div><p className="eyebrow ev-e">Results</p><h2 className="ev-cal-month">{calendarView.resultsHeading}</h2></div><p className="ev-results-count" role="status" aria-live="polite">Showing {calendarView.results.length} of {calendarView.totalCount}</p></div>{calendarView.results.length ? <ul className="ev-results-list">{calendarView.results.map((event) => <li key={event.id} id={`calendar-event-${event.id}`}><article className="ev-result">{event.ticketCta?.isVerifiedDeal && <p className="eyebrow ev-deal">Texas Defined Deal</p>}<p className="eyebrow ev-e">{EVENT_LABELS[event.category]} · {event.dateLabel}</p><h3 className="ev-result-title"><a href={event.guidePath}>{event.title}</a></h3><p className="ev-result-meta">{event.city}{event.countyName ? ` · ${event.countyName}` : ""}{event.venueName ? ` · ${event.venueName}` : ""}</p><p className="ev-result-copy">{event.summary}</p>{event.ticketCta?.isVerifiedDeal && event.ticketCta.promotion && <p className="ev-result-offer"><strong>{event.ticketCta.promotion.label}</strong>{event.ticketCta.promotion.promoCode ? ` · Code ${event.ticketCta.promotion.promoCode}` : ""}</p>}<div className="ev-result-links"><a href={event.guidePath} className="ev-link">{event.guidePath.startsWith("/events?") ? "View date" : "Event guide"}</a><EventTicketCta cta={event.ticketCta} linkClassName="ev-link" wrapperClassName="ev-ticket" disclosureClassName="ev-ticket-disclosure" /><a href={event.officialEventUrl} target="_blank" rel="noreferrer noopener" className="ev-link">Official event site ↗</a>{event.venuePath && <a href={event.venuePath} className="ev-link">Venue guide</a>}</div><p className="ev-result-note">Last verified {event.lastVerifiedLabel}. Confirm event-day details with the official source before traveling.</p></article></li>)}</ul> : <p className="ev-empty">No source-qualified events match this view. Try another keyword, date, location, event type or offer filter.</p>}</div>
  </div>;
}
