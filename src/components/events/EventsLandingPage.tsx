...
type CalendarView = {
  ...
  results: Array<{ id: string; category: TexasEvent["category"]; guidePath: string; title: string; city: string; countyName?: string; venueName?: string; venuePath?: string; summary: string; officialEventUrl: string; dateLabel: string; lastVerifiedLabel: string }>;
};
...
function GlobalEventCalendar({ calendarView, regions, search }: { calendarView: CalendarView; regions: Region[]; search: EventSearch }) {
  return <div id="calendar" className="ev-cal">
    ...
    </div><aside className="ev-status"><p className="eyebrow ev-e">Calendar status</p><p className="ev-status-count">{calendarView.totalCount} {calendarView.totalCount === 1 ? "event" : "events"}</p><p className="ev-status-copy">{calendarView.dateSummary}</p>{calendarView.truncated && <p className="ev-status-small">Showing the first {calendarView.results.length}; narrow the date or filters for a shorter list.</p>}</aside></div>
