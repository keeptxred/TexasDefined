  ...
  return (
    <section className="border-y border-border bg-surface py-14 sm:py-18" aria-labelledby="hunting-public-land-heading">
      <Container>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-4 text-sm text-muted-foreground"><p aria-live="polite">{filtered.length.toLocaleString("en-US")} {filtered.length === 1 ? "area matches" : "areas match"} these choices.</p><Link to="/explore/outdoors" className="eyebrow border-b border-primary pb-1 text-primary">Browse all Texas outdoors →</Link></div>

        {visible.length ? <ul className="mt-8 grid gap-x-8 sm:grid-cols-2 lg:grid-cols-3">{visible.map((destination) => <li key={destination.id} className="border-t border-border py-6"><p className="eyebrow text-primary">{title(destination.region)} · Near {destination.nearestTown}</p><h3 className="mt-2 font-display text-2xl leading-tight"><Link to="/destination/$slug" params={{ slug: destination.slug }} className="hover:text-primary">{destination.name}</Link></h3><p className="mt-3 text-sm leading-6 text-muted-foreground">{destination.summary}</p><p className="mt-4 text-xs leading-5 text-muted-foreground"><strong className="font-semibold text-foreground">Access note:</strong> {destination.entryNote}</p><Link to="/destination/$slug" params={{ slug: destination.slug }} className="eyebrow mt-5 inline-block border-b border-primary pb-1 text-primary">Open WMA guide →</Link></li>)}</ul> : <div className="mt-8 border-t border-border py-10"><p className="font-display text-3xl">No WMA matches those choices.</p><p className="mt-3 text-sm text-muted-foreground">Try a broader place, county, habitat or region.</p></div>}
        ...
        <div className="mt-10 grid gap-5 border-t border-border pt-7 text-sm leading-6 text-muted-foreground sm:grid-cols-2"><p><strong className="text-foreground">Permit rules vary by area.</strong> Use each destination&apos;s current TPWD source and access note, then confirm the permit, hunt-period and registration rules that apply before arrival.</p><p><strong className="text-foreground">Need the statewide permit first?</strong> <Link to="/hunting/annual-public-hunting-permit" className="underline decoration-primary/40 underline-offset-4 hover:text-primary">Read the Annual Public Hunting Permit guide</Link> or return to <Link to="/hunting/public-hunting" className="underline decoration-primary/40 underline-offset-4 hover:text-primary">Texas public hunting</Link>.</p></div>
      </Container>
    </section>
  );
}
