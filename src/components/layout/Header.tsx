      ...
      <div onMouseLeave={() => setOpenGroup(null)} onKeyDown={(event) => { if (event.key === "Escape") { setOpenGroup(null); setOpen(false); } }}>
        ...
        <Container width="wide" className="flex h-[4.5rem] items-center justify-between lg:hidden">
          ...
          <div className="flex items-center gap-0.5">
            ...
            <button type="button" className="p-2 text-foreground/65 transition-colors hover:text-primary" aria-label={open ? brand.copy.close : brand.copy.menu} aria-expanded={open} aria-controls="mobile-main-menu" onClick={() => setOpen((value) => !value)}>{open ? <X className="size-5" aria-hidden /> : <Menu className="size-5" aria-hidden />}</button>
          </div>
        </Container>
