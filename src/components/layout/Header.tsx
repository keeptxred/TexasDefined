...
import type { BrandNavItem } from "@/brand/types";
import { Container } from "./Container";
import { cn } from "@/lib/utils";
import { useShopCart } from "@/lib/shop-cart";

function NavCardMedia({ image }: { image?: BrandNavItem["image"] }) {
  const [failed, setFailed] = useState(false);

  if (!image || failed) {
    return <div className="flex aspect-[16/9] items-end overflow-hidden bg-gradient-to-br from-surface via-muted to-primary/15 p-4 xl:aspect-[4/3]" aria-hidden="true"><span className="font-display text-xl leading-none text-foreground/55">Texas Defined</span></div>;
  }

  return <div className="overflow-hidden bg-muted"><img src={image.src} alt={image.alt} width={480} height={320} sizes="(min-width: 1280px) 16vw, 22vw" loading="lazy" decoding="async" onError={() => setFailed(true)} className="aspect-[16/9] w-full object-cover transition-transform duration-700 group-hover:scale-105 xl:aspect-[4/3]" /></div>;
}
...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border/80 bg-background/96 backdrop-blur supports-[backdrop-filter]:bg-background/92">
      <a href="#main" className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-3 focus:z-50 focus:inline-flex focus:min-h-11 focus:items-center focus:bg-primary focus:px-3 focus:py-2 focus:text-primary-foreground">
        {brand.copy.skipToContent}
      </a>

      <div onMouseLeave={() => setOpenGroup(null)} onKeyDown={(event) => { if (event.key === "Escape") { setOpenGroup(null); setOpen(false); } }}>
        ...
        <Container width="wide" className="flex h-[4.5rem] items-center justify-between lg:hidden">
          ...
          <div className="flex items-center gap-0.5">
            ...
            <button type="button" className="inline-flex min-h-11 min-w-11 items-center justify-center p-2 text-foreground/65 transition-colors hover:text-primary" aria-label={open ? brand.copy.close : brand.copy.menu} aria-expanded={open} aria-controls="mobile-main-menu" onClick={() => setOpen((value) => !value)}>{open ? <X className="size-5" aria-hidden /> : <Menu className="size-5" aria-hidden />}</button>
          </div>
        </Container>

        {brand.nav.filter((item) => item.children && openGroup === item.to).map((item) => <nav key={item.to} aria-label={`${item.label} sections`} className="absolute inset-x-0 top-full hidden animate-in fade-in slide-in-from-top-1 border-b border-border bg-background shadow-editorial duration-200 lg:block"><Container width="wide" className="py-6 xl:py-7"><div className="flex items-end justify-between border-b border-border/70 pb-4 xl:pb-5"><div><p className="eyebrow text-primary">{item.label}</p>{item.description && <p className="mt-2 max-w-2xl font-display text-2xl font-semibold leading-none xl:text-3xl">{item.description}</p>}</div><Link to={item.to} className="eyebrow group inline-flex min-h-11 items-center gap-2 border-b border-primary pb-1 text-primary" onClick={() => setOpenGroup(null)}>Explore the section <span aria-hidden="true" className="transition-transform group-hover:translate-x-1">→</span></Link></div><ul className={cn("mt-5 grid grid-cols-4 gap-5 xl:mt-6 xl:gap-6", (item.children?.length ?? 0) <= 5 ? "xl:grid-cols-5" : "xl:grid-cols-6")}>{item.children?.map((child) => <li key={child.to}><Link to={child.to} className="group block" onClick={() => setOpenGroup(null)}><NavCardMedia image={child.image} /><p className="mt-2.5 font-display text-base font-semibold leading-tight transition-colors group-hover:text-primary xl:mt-3 xl:text-lg">{child.label}</p>{child.description && <p className="mt-1.5 hidden text-sm leading-5 text-muted-foreground xl:block">{child.description}</p>}</Link></li>)}</ul></Container></nav>)}
      </div>

      <div id="mobile-main-menu" className={cn("overflow-hidden border-t border-border bg-background lg:hidden", open ? "block" : "hidden")}><Container className="max-h-[72vh] overflow-y-auto py-4"><nav aria-label="Main menu" className="flex flex-col">{brand.nav.map((item) => <div key={item.to} className="border-b border-border/70 py-2 last:border-0"><Link to={item.to} className="flex min-h-11 items-center py-2 font-display text-xl font-semibold text-foreground" activeProps={{ className: "text-primary" }} onClick={() => setOpen(false)}>{item.label}</Link>{item.children && <div className="grid grid-cols-2 gap-x-5 pb-2">{item.children.map((child) => <Link key={child.to} to={child.to} className="flex min-h-11 items-center py-2 text-sm leading-5 text-muted-foreground" activeProps={{ className: "font-semibold text-primary" }} onClick={() => setOpen(false)}>{child.label}</Link>)}</div>}</div>)}</nav></Container></div>
    </header>
  );
}
