  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border/80 bg-background/96 backdrop-blur supports-[backdrop-filter]:bg-background/90">
      ...
      <div onMouseLeave={() => setOpenGroup(null)}>
        ...
        {brand.nav.filter((item) => item.children && openGroup === item.to).map((item) => (
          <div key={item.to} className="absolute inset-x-0 top-full hidden animate-in fade-in slide-in-from-top-1 border-b border-border bg-background shadow-editorial duration-200 lg:block">
