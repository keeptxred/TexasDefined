  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border/80 bg-background/96 backdrop-blur supports-[backdrop-filter]:bg-background/92">
      ...
      <div onMouseLeave={() => setOpenGroup(null)}>
        ...
        {brand.nav.filter((item) => item.children && openGroup === item.to).map((item) => <div key={item.to} className="absolute inset-x-0 top-full hidden animate-in fade-in slide-in-from-top-1 border-b border-border bg-background shadow-editorial duration-200 lg:block"><Container width="wide" className="py-7"><div className="flex items-end justify-between border-b border-border/70 pb-5"><div><p className="eyebrow text-primary">{item.label}</p>{item.description && <p className="mt-2 max-w-2xl font-display text-3xl font-semibold leading-none">{item.description}</p>}</div><Link to={item.to} className="eyebrow group inline-flex items-center gap-2 border-b border-primary pb-1 text-primary" onClick={() => setOpenGroup(null)}>Explore the section <span aria-hidden="true" className="transition-transform group-hover:translate-x-1">→</span></Link></div><ul className={cn("mt-6 grid grid-cols-3 gap-6", (item.children?.length ?? 0) <= 5 ? "xl:grid-cols-5" : "xl:grid-cols-6")}>{item.children?.map((child) => <li key={child.to}><Link to={child.to} className="group block" onClick={() => setOpenGroup(null)}>{child.image && <div className="overflow-hidden bg-muted"><img src={child.image.src} alt={child.image.alt} width={480} height={320} loading="lazy" decoding="async" className="aspect-[4/3] w-full object-cover transition-transform duration-700 group-hover:scale-105" /></div>}<p className="mt-3 font-display text-lg font-semibold leading-tight transition-colors group-hover:text-primary">{child.label}</p>{child.description && <p className="mt-1.5 text-sm leading-5 text-muted-foreground">{child.description}</p>}</Link></li>)}</ul></Container></div>)}
      </div>

      <div className={cn("overflow-hidden border-t border-border bg-background lg:hidden", open ? "block" : "hidden")}><Container className="max-h-[72vh] overflow-y-auto py-4"><nav aria-label="Main menu" className="flex flex-col">{brand.nav.map((item) => <div key={item.to} className="border-b border-border/70 py-2 last:border-0"><Link to={item.to} className="block py-2 font-display text-xl font-semibold text-foreground" onClick={() => setOpen(false)}>{item.label}</Link>{item.children && <div className="grid grid-cols-2 gap-x-5 pb-2">{item.children.map((child) => <Link key={child.to} to={child.to} className="py-1.5 text-sm leading-5 text-muted-foreground" onClick={() => setOpen(false)}>{child.label}</Link>)}</div>}</div>)}</nav></Container></div>
    </header>
  );
}
