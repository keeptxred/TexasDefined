...
export function StayMonetizationReadiness() {
  ...
  return <section id="stay-monetization" className="mt-12">
    <p className="eyebrow text-primary">Commerce readiness</p>
    <h2 className="font-display text-3xl">Stay monetization readiness</h2>
    <p className="mt-2 text-sm text-muted-foreground">A source-of-truth view of governed Stay Nearby coverage. This panel reports registry readiness only; it does not invent traffic, booking, conversion or revenue performance.</p>

    {error ? <div className="mt-8 rounded-md border border-destructive/40 p-5"><strong>Readiness data unavailable</strong><p className="mt-2 text-sm text-muted-foreground">{error}</p></div> : null}
    {!readiness && !error ? <p className="mt-2 text-sm text-muted-foreground">Loading governed stay registries…</p> : null}

    {readiness ? <>
      <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <ReadinessMetric label="Curated contexts" value={String(readiness.totalContexts)} detail={`${readiness.venueContexts} venue · ${readiness.destinationContexts} destination (${readiness.targetedContexts} targeted)`} />
        <ReadinessMetric label="Active properties" value={String(readiness.activeProperties)} detail={`${readiness.destinationProperties} destination · ${readiness.targetedProperties} targeted properties`} />
        <ReadinessMetric label="Verified property links" value={String(readiness.verifiedAffiliateLinks)} detail={`${readiness.venueVerifiedAffiliateLinks} venue · ${readiness.destinationVerifiedAffiliateLinks} destination · ${readiness.targetedVerifiedAffiliateLinks} targeted`} />
      ...
      <div className="mt-8 rounded-md border border-border p-5">
        ...
        <p className="mt-2 text-sm text-muted-foreground">Destination pages use contextual curated cards only where this registry has a reviewed relationship. Other eligible travel pages keep the generic Expedia/Hotels.com comparison layer, with Orbitz reserved for hotel-first event/venue intent, Travelocity used for broader destination/leisure intent, and Vrbo limited to broader traveler contexts rather than inheriting unsourced local recommendations. RVshare is separately limited to high-intent camping, RV-park, state-park, outdoors and road-trip surfaces, where it replaces the lower-paying Vrbo traveler CTA and is promoted as the primary rental option.</p>
      ...
      <div className="mt-6 rounded-md border border-border p-5">
        <strong className="font-display text-2xl">Targeted trip cohort: Painted Churches · {readiness.targetedProperties} properties</strong>
        <p className="mt-2 text-sm text-muted-foreground">The Painted Churches route uses a separately governed Schulenburg lodging cohort with direct verified Hotels.com property links. It remains distinct from the standard destination registry so its trip-specific sourcing, text-only image policy and review cadence stay explicit.</p>
        <p className="mt-2 text-xs text-muted-foreground">Painted Churches registry reviewed: {data?.paintedChurches.reviewedAt || 'not recorded'} · {readiness.targetedVerifiedAffiliateLinks} verified exact-property links</p>
      </div>

      <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {readiness.destinationRows.map((row) => <article key={row.key} className="rounded-md border border-border p-5">
          <strong className="capitalize">{row.key.replaceAll('-', ' ')}</strong>
          <p className="mt-2 text-sm text-muted-foreground">{row.properties.join(' · ')}</p>
          <p className="mt-2 text-xs text-muted-foreground">{row.properties.length}/3 curated · {row.verifiedLinks} verified deeplink{row.verifiedLinks === 1 ? '' : 's'}</p>
        </article>)}
      </div>
...
function summarize({ venue, destination, paintedChurches, hotelsCom }: DashboardState) {
  ...
  const targetedProperties = (paintedChurches.properties || []).filter((property) => property.status === 'active');
  const standardProperties = [...venueProperties, ...destinationProperties];
  const all = [...standardProperties, ...targetedProperties];
  ...
  const standardDestinationContextKeys = contextKeys(destinationProperties, 'destination');
  const targetedContextKeys = contextKeys(targetedProperties, 'destination');
  const destinationContextKeys = new Set([...standardDestinationContextKeys, ...targetedContextKeys]);
  const activePropertyIds = new Set(standardProperties.flatMap((property) => property.id ? [property.id] : []));
  ...
  const targetedVerified = (property: StayProperty) => (property.bookingTargets || []).some((target) =>
  ...
  const targetedVerifiedAffiliateLinks = targetedProperties.filter(targetedVerified).length;
  const verifiedAffiliateLinks = verifiedPropertyIds.size + targetedVerifiedAffiliateLinks;
  ...
  const destinationRows = [...destinationContextKeys].sort().map((key) => {
    const matching = [...destinationProperties, ...targetedProperties].filter((property) => (property.contexts || []).some((context) => context.kind === 'destination' && context.key === key));
    return {
      key,
      properties: matching.map((property) => property.name || property.id || 'Unnamed property'),
      verifiedLinks: matching.filter((property) => property.id && (verifiedPropertyIds.has(property.id) || targetedVerified(property))).length,
    };
  });
  return {
    venueContexts: venueContextKeys.size,
    destinationContexts: destinationContextKeys.size,
    targetedContexts: targetedContextKeys.size,
    ...
    targetedProperties: targetedProperties.length,
