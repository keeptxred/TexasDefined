...
export function CountyAuthorityPage({ data }: { data: ExploreGeographyPage }) {
  ...
  const faq = [
    { question: `What can visitors find in ${label}?`, answer: `${label} currently has ${data.total.toLocaleString("en-US")} published parks, attractions, historic places, waterways, trails, and other destinations in the TexasDefined Explore Texas directory.` },
    county ? { question: `What is the ${label} property-tax rate?`, answer: `The county rate in the TexasDefined dataset is ${county.countyRate.toFixed(4)} per $100 of taxable value for tax year ${county.taxYear}. A total bill can also include city, school, college, hospital, utility, emergency-services, and other special-district rates.` } : null,
  ...
  const schema = {
    ...
    "@graph": [
      { "@type": ["WebPage", "CollectionPage"], "@id": `https://texasdefined.com${canonicalPath}#webpage`, url: `https://texasdefined.com${canonicalPath}`, name: `${label}, Texas authority guide`, description: `County guide to demographics, economy, schools, taxes, districts, representatives, cities, parks, attractions, elections, and news for ${label}, Texas.`, about: { "@id": `https://texasdefined.com${canonicalPath}#county` } },
      { "@type": "AdministrativeArea", "@id": `https://texasdefined.com${canonicalPath}#county`, name: `${label}, Texas`, url: `https://texasdefined.com${canonicalPath}`, containedInPlace: { "@type": "State", name: "Texas" } },
      { "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Explore Texas", item: "https://texasdefined.com/explore" }, { "@type": "ListItem", position: 2, name: label, item: `https://texasdefined.com${canonicalPath}` }] },
      data.items.length ? { "@type": "ItemList", name: `Parks and attractions in ${label}`, numberOfItems: data.items.length, itemListElement: data.items.map((item, index) => ({ "@type": "ListItem", position: index + 1, name: item.name, url: `https://texasdefined.com/explore/${item.slug}` })) } : null,
  ...
  return <main>
    ...
    <section className="border-b bg-muted/30"><div className="mx-auto max-w-6xl px-4 py-16 md:py-24">
      ...
      <p className="mt-5 max-w-4xl text-lg leading-8 text-muted-foreground">A single county reference for local taxes, schools, communities, government representation, elections, parks, attractions, and related TexasDefined coverage.</p>
    ...
    <div className="mx-auto max-w-6xl space-y-16 px-4 py-14">
      ...
      <Section id="economy" title={`${label} economy`}><div className="grid gap-4 md:grid-cols-2"><ResourceCard title="Employment and labor force" description="County employment, unemployment, earnings, industry, and establishment data should use the latest published reporting period." href="https://www.bls.gov/eag/"/><ResourceCard title="Regional economic context" description={`${label} is part of the ${county?.region || "Texas"} region. TexasDefined economic coverage supplies broader state context.`} href="/texas-economy"/></div></Section>
