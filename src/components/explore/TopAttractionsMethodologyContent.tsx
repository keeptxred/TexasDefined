...
const sourceHierarchy = [
  ...
  ["2. Supporting public or institutional sources", "Government agencies, universities, UNESCO, preservation bodies, conservation programs and the attraction's own history/science resources can support significance, history, geology, ecology, accessibility or institutional context."],
...
export function TopAttractionsMethodologyContent() {
  return <>
    ...
    <Section>
      <Container>
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-2">
          ...
          <article className="border-y border-border py-7"><h2 className="font-display text-3xl">Review sites are not primary evidence</h2><p className="mt-4 text-sm leading-7 text-muted-foreground">User-review platforms, generic travel blogs and social popularity can be useful for discovering questions, but they are not used as evidence for the Top-25 research process. The source list favors operators, public agencies, universities, preservation bodies, official destination organizations and other accountable institutions.</p></article>
          ...
          <article className="border-y border-border py-7"><h2 className="font-display text-3xl">Source URLs travel with the data</h2><p className="mt-4 text-sm leading-7 text-muted-foreground">The downloadable CSV and JSON distributions expose the source count and source URLs alongside the editorial comparison fields, so researchers can inspect the evidence behind a row rather than receiving a score without showing the evidence.</p></article>
        </div>
      </Container>
    </Section>
    ...
    <Section>
      <Container>
        <div className="grid gap-10 lg:grid-cols-3">
          ...
          <article><p className="eyebrow text-primary">Machine-readable</p><h2 className="mt-2 font-display text-3xl">Use the comparison datasets</h2><p className="mt-4 text-sm leading-7 text-muted-foreground">CSV and JSON distributions expose the ranked list, planning fields, canonical URLs, review dates and multi-source evidence. The human-readable Top-25 page remains the main editorial reference.</p><div className="mt-5 flex flex-wrap gap-5"><a href="/top-25-texas-attractions.csv" className="eyebrow border-b border-primary pb-1 text-primary">Download CSV →</a><a href="/top-25-texas-attractions.json" className="eyebrow border-b border-primary pb-1 text-primary">Download JSON →</a></div></article>
    ...
    <Section tone="ink">
      <Container>
        <SectionHeader eyebrow="Citation" title="How to use this collection as a reference" description="Cite the main Top-25 page for the collection and individual destination pages for attraction-specific planning context. Preserve the visible review date and official-source caveat when the underlying fact can change." />
...
export default TopAttractionsMethodologyContent;
