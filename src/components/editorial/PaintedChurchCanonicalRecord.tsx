...
const integrityLabels = {
  ...
  "uncertain": "Still being researched",
} as const;
  ...
  return (
    <section aria-labelledby="canonical-record" className="mt-14 border-t border-border pt-8">
      ...
      <dl className="mt-8 grid border-y border-border sm:grid-cols-2">
        ...
        <div className="border-b border-border py-5 sm:pl-6"><dt className="eyebrow text-muted-foreground">Documented techniques</dt><dd className="mt-2 text-base leading-7">{church.techniques.length ? church.techniques.length : "Still being researched"}</dd></div>
      </dl>
      ...
      <p className="mt-6 max-w-4xl text-sm leading-7 text-muted-foreground">The collection category describes how the church is documented, not its artistic quality. Interior-integrity labels distinguish untouched or surviving historic work from restoration, evidence-based reconstruction, later repainting and modern decorative campaigns.</p>
      <div className="mt-5 flex flex-wrap gap-x-6 gap-y-3 text-sm"><Link to="/explore/painted-churches/census" className="border-b border-primary text-primary">Open the master census</Link><Link to="/explore/painted-churches/methodology" className="border-b border-primary text-primary">How we classify churches</Link></div>
    </section>
  );
}
