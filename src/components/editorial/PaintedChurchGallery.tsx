  ...
  return (
    <section aria-labelledby="church-gallery" className="mt-14 border-t border-border pt-8">
      ...
      <div className="mt-8 grid gap-8 sm:grid-cols-2">
        {images.map((image) => (
          <figure key={image.sourceUrl} className="border-t border-border pt-4" data-runtime-image-figure>
            <img src={image.src} alt={image.alt} width={image.width} height={image.height} loading="lazy" decoding="async" className="aspect-[4/3] w-full object-cover" onError={(event) => hideFailedImageContainer(event.currentTarget, "[data-runtime-image-figure]")} />
