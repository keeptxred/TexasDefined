  ...
  return <>
    ...
    {pairedDestinations.length ? <Section tone="surface" className="py-10 sm:py-12 lg:py-14">
      <Container>
        <SectionHeader
          eyebrow="Nearby"
          title="Places worth adding to the same trip"
          description="A short set of nearby options, ordered by distance, instead of a long stack of overlapping recommendation sections."
        />
        <ul className="mt-8 grid gap-7 sm:grid-cols-2 lg:grid-cols-3">
          {pairedDestinations.map((item) => {
            ...
            return <li key={item.id}>
              <DestinationCard destination={item} regionLabel={item.region === destination.region ? regionName : undefined} />
              {miles !== null && <p className="mt-3 border-t border-border pt-3 text-[0.7rem] uppercase tracking-[0.1em] text-muted-foreground">Approx. {Math.max(1, Math.round(miles)).toLocaleString("en-US")} miles away</p>}
            </li>;
          })}
        </ul>
      </Container>
