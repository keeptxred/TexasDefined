      ...
      <Section tone="surface">
        <Container>
          <SectionHeader
            ...
            description="A few places that make sense together, whether they are just down the road, near the same town or simply worth pairing in one trip."
          />
          <nav aria-label="Ways to continue the trip" className="mt-8 flex flex-wrap gap-3">
            {groups.map((group) => (
      ...
      <Section tone="ink">
        <Container className="grid gap-6 text-ink-foreground sm:grid-cols-2 lg:grid-cols-4">
          <Link to="/explore/$category" params={{ category: destination.category }} className="border border-ink-foreground/20 p-5 hover:border-ink-foreground/60">
            ...
            <span className="mt-2 block text-sm text-ink-foreground/70">Find more places across Texas with the same kind of appeal.</span>
          </Link>
          <Link to="/explore/region/$region" params={{ region: destination.region }} className="border border-ink-foreground/20 p-5 hover:border-ink-foreground/60">
            ...
            <span className="mt-2 block text-sm text-ink-foreground/70">See what else is worth a stop in this part of Texas.</span>
          </Link>
          ...
          <Link to="/search" search={{ q: destination.nearestTown }} className="border border-ink-foreground/20 p-5 hover:border-ink-foreground/60">
            <strong className="font-display text-xl">Look nearby</strong>
