...
const labels: Record<string, string> = {
  ...
  "connected-person": "People",
  "heritage-context": "Heritage",
  "preservation-example": "Preservation",
};

export function PaintedChurchKnowledgeLinks({ slug }: { slug: string }) {
  const connections = paintedChurchKnowledgeForChurch(slug).filter((item) => item.edge.relationship !== "person-uses-technique");
  ...
  return (
    <section aria-labelledby="knowledge-graph-links" className="mt-14 border-t border-border pt-8">
      ...
      <h2 id="knowledge-graph-links" className="mt-3 font-display text-4xl">Follow the people, symbols, techniques and traditions connected to this church.</h2>
      ...
      <p className="mt-5 text-xs leading-6 text-muted-foreground">Relationships are published only when Texas Defined has a church-specific source or documented collection-level basis for the connection.</p>
    </section>
  );
}
