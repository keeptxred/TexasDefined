  ...
  return <>
    <div className="mt-8 border-y border-border py-6">
      <div className="grid gap-x-8 gap-y-5 md:grid-cols-2 lg:grid-cols-[minmax(240px,1.4fr)_1fr_1fr_1fr_auto]">
        ...
        <div className="flex items-end"><button type="button" disabled={!hasFilters} onClick={() => { setQuery(""); setRegion("all"); setActivity("all"); }} className="eyebrow h-11 whitespace-nowrap text-muted-foreground transition-colors enabled:hover:text-primary disabled:opacity-30">Reset</button></div>
      </div>
      <p className="mt-5 text-sm text-muted-foreground" aria-live="polite">{filtered.length.toLocaleString("en-US")} {filtered.length === 1 ? "place" : "places"}{hasFilters ? " match these selections" : " in this guide"}</p>
    </div>

    {visible.length > 0 ? <ul className="mt-10 grid gap-x-8 gap-y-12 sm:grid-cols-2 lg:grid-cols-3">{visible.map((destination) => <li key={destination.id}><DestinationCard destination={destination} regionLabel={regionLabel(destination.region)} /></li>)}</ul> : <div className="mt-10 border-t border-border py-12"><p className="font-display text-3xl">No places match those selections.</p><p className="mt-3 text-sm text-muted-foreground">Try another region, activity or a broader search.</p></div>}
