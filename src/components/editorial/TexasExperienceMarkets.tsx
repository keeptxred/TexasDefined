...
export function TexasExperienceMarkets() {
  ...
  const statewidePlacement = "viator-statewide-explore";
  ...
  return <section id="tours-experiences" className="scroll-mt-24 border-y border-border bg-surface py-14 sm:py-16" aria-labelledby="texas-experiences-title">
    <Container>
      ...
      <div className="mt-7 flex flex-wrap gap-3">
        <a
          href={buildViatorAffiliateUrl(viatorTexasUrl(), "texasdefined-statewide-explore")}
          target="_blank"
          rel="sponsored nofollow noopener noreferrer"
          data-affiliate-partner="viator"
          data-affiliate-placement={statewidePlacement}
          data-commercial-partner="viator"
          data-commercial-placement={statewidePlacement}
          onClick={() => trackAffiliateClick({ partner: "viator", label: "Browse current Texas experiences on Viator", placement: statewidePlacement, module: "experiences" })}
          className="inline-flex items-center bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground transition-opacity hover:opacity-90"
        >Browse current Texas experiences on Viator ↗</a>
...
function MarketCard({ market }: { market: (typeof VIATOR_RUNTIME_MARKETS)[number] }) {
  ...
  const commercialPlacement = `viator-market-${market.slug}`;
  const ctaLabel = verifiedMarketInventory ? `Current Viator options for ${market.name}` : `Browse Viator Texas inventory for ${market.name}`;
  ...
  return <article className="border border-border bg-background p-6">
    ...
    <div className="mt-5 flex flex-wrap gap-4 text-sm font-semibold">
      ...
      >{verifiedMarketInventory ? "Current Viator options ↗" : "Browse Viator Texas inventory ↗"}</a>
    </div>
  </article>;
}
