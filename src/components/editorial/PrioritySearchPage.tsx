...
export type PrioritySearchSection = { heading: string; paragraphs: string[]; links?: PrioritySearchLink[]; stayNearbySlot?: boolean };
...
export function PrioritySearchPage({ data }: { data: PrioritySearchPageData }) {
  ...
  return <main>
    ...
    <section className="py-12 md:py-16"><Container><div className="max-w-4xl divide-y divide-border">{data.sections.map((section, index) => <section key={section.heading} className="py-8 first:pt-0"><p className="eyebrow text-muted-foreground">{String(index + 1).padStart(2, "0")}</p><h2 className="mt-2 font-display text-3xl md:text-4xl">{section.heading}</h2>{section.paragraphs.map((paragraph) => <p key={paragraph} className="mt-4 leading-8 text-muted-foreground">{paragraph}</p>)}{section.stayNearbySlot ? <div data-stay-nearby-slot className="mt-6" aria-label="Places to stay for this trip" /> : null}{section.links?.length ? <div className="mt-5 flex flex-wrap gap-x-6 gap-y-3 text-sm">{section.links.map((link) => <ResourceLink key={`${link.href}-${link.label}`} link={link} />)}</div> : null}</section>)}</div></Container></section>
