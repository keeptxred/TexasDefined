...
export type PrioritySearchFact = { label: string; value: string; detail?: string };
...
export function PrioritySearchPage({ data }: { data: PrioritySearchPageData }) {
  ...
  return <main>
    <section className="border-b border-border bg-muted/30 py-14 md:py-20"><Container><p className="eyebrow text-primary">{data.eyebrow}</p><h1 className="mt-3 max-w-5xl font-display text-5xl leading-none md:text-7xl">{data.title}</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-muted-foreground">{data.intro}</p><p className="mt-5 text-xs uppercase tracking-[0.18em] text-muted-foreground">Updated {data.updated}</p></Container></section>
    ...
    {data.facts?.length ? <section className="border-b border-border py-10" aria-labelledby="priority-facts-heading"><Container><p className="eyebrow text-primary">At a glance</p><h2 id="priority-facts-heading" className="mt-2 font-display text-3xl md:text-4xl">Plan the visit</h2><div className="mt-6 grid gap-px overflow-hidden border border-border bg-border sm:grid-cols-2 lg:grid-cols-4">{data.facts.map((fact) => <div key={fact.label} className="bg-background p-5"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">{fact.label}</p><p className="mt-2 font-display text-2xl">{fact.value}</p>{fact.detail ? <p className="mt-2 text-sm leading-6 text-muted-foreground">{fact.detail}</p> : null}</div>)}</div></Container></section> : null}
    ...
    {officialSources.length ? <section className="border-t border-border py-10"><Container><div className="grid max-w-4xl gap-6 lg:grid-cols-[14rem_1fr]"><div><p className="eyebrow text-primary">Verify with the source</p><h2 className="mt-2 font-display text-3xl">Official links used in this guide</h2></div><div><p className="text-sm leading-7 text-muted-foreground">TexasDefined explains the system and helps route you to the right place. Current fees, eligibility, deadlines, appointments, rules and transactions should be confirmed with the responsible official source.</p><div className="mt-5 flex flex-wrap gap-x-6 gap-y-3 text-sm">{officialSources.map((link) => <ResourceLink key={`source-${link.href}`} link={link} />)}</div></div></div></Container></section> : null}
