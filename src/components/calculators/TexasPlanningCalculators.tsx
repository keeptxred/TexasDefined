...
export function MortgageCalculator() {
  const [state, setState] = useState(MORTGAGE_DEFAULTS);
...
export function AffordabilityCalculator() {
  const [state, setState] = useState(AFFORDABILITY_DEFAULTS);
...
export function RentVsBuyCalculator() {
  const [state, setState] = useState(RENT_BUY_DEFAULTS);
...
export function CostOfLivingCalculator() {
  const [state, setState] = useState(COST_DEFAULTS); const set = <K extends keyof CostState>(k: K, v: CostState[K]) => setState((s) => ({ ...s, [k]: v }));
...
export function SalaryCalculator() {
  const [state, setState] = useState(SALARY_DEFAULTS); const set = <K extends keyof SalaryState>(k: K, v: SalaryState[K]) => setState((s) => ({ ...s, [k]: v }));
...
export function MovingCostCalculator() {
  const [state, setState] = useState(MOVING_DEFAULTS); const set = <K extends keyof MovingState>(k: K, v: MovingState[K]) => setState((s) => ({ ...s, [k]: v }));
...
export function UtilityCalculator() {
  const [state, setState] = useState(UTILITY_DEFAULTS); const set = <K extends keyof UtilityState>(k: K, v: UtilityState[K]) => setState((s) => ({ ...s, [k]: v }));
...
export function HomeInsuranceCalculator() {
  const [state, setState] = useState(INSURANCE_DEFAULTS); const set = <K extends keyof InsuranceState>(k: K, v: InsuranceState[K]) => setState((s) => ({ ...s, [k]: v }));
