...
import { scoreReaderQuality, type ReaderQualityReferrerClass, type ReaderQualitySignals } from '@/platform/reader-quality';
...
export async function ingestReaderQualityRequest(request: Request) {
  const requestUrl = new URL(request.url);
  const origin = request.headers.get('origin');
  if (origin && origin !== requestUrl.origin) return json({ ok: false, error: 'cross_origin_rejected' }, 403);

  const fetchSite = request.headers.get('sec-fetch-site');
  if (fetchSite && !['same-origin', 'same-site', 'none'].includes(fetchSite)) return json({ ok: false, error: 'cross_site_rejected' }, 403);
  ...
  if (!text || text.length > MAX_BODY_BYTES) return json({ ok: false, error: text ? 'payload_too_large' : 'empty_payload' }, text ? 413 : 400);

  let body: ReaderQualityInput;
  try {
    body = JSON.parse(text) as ReaderQualityInput;
  } catch {
  ...
  if (sessionId.length < 8 || sessionId.length > 128) return json({ ok: false, error: 'invalid_session' }, 400);
  if (!path.startsWith('/') || path.length > 500) return json({ ok: false, error: 'invalid_path' }, 400);

  const occurredAt = new Date(occurredAtRaw);
  if (!Number.isFinite(occurredAt.getTime())) return json({ ok: false, error: 'invalid_timestamp' }, 400);
  const ageMs = Date.now() - occurredAt.getTime();
  if (ageMs < -300_000 || ageMs > 172_800_000) return json({ ok: false, error: 'timestamp_out_of_range' }, 400);
  ...
  return json({ ok: true, classification: quality.classification, qualityScore: quality.score }, 202);
}
