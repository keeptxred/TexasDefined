...
export const getReaderQualityDashboard = createServerFn({ method: 'GET' }).handler(async (): Promise<ReaderQualityDashboard> => {
  ...
  const [qualityResult, ga4Result, eventResult] = await Promise.all([
    ...
    client
      ...
      .select('metric_date,session_hash,path,quality_score,classification,visible_ms,interaction_count,max_scroll_percent,referrer_class')
      .gte('metric_date', since)
