...
export function recordGovernanceDecision(input: {
  ...
  const primaryKind = input.gate.status === 'allowed'
    ? 'publication-allowed'
  ...
  if (input.override) {
    ids.push(appendGovernanceEvent(createGovernanceEvent({
      ...common,
      kind: input.gate.overrideValid ? 'override-accepted' : 'override-rejected',
    })));
  }
  return ids;
}
