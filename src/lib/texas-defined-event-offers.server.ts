...
type OfferKind = 'event' | 'offer' | 'hotel' | 'attraction' | 'package';
type OfferCategory = 'concerts' | 'sports' | 'festivals' | 'family' | 'theater' | 'attractions' | 'tours' | 'hotels' | 'outdoors' | 'shopping';
type CommissionStatus = 'preserved' | 'reduced' | 'zero' | 'unknown';
type Network = 'impact' | 'cj' | 'expedia' | 'direct' | 'internal';
...
type SyncSourceSummary = { source: string; attempted: boolean; fetched: number; persisted: number; skipped: number; error?: string };

const DEFAULT_TICKETMASTER_IMPACT_BASE_URL = 'https://ticketmaster.evyy.net/c/7758914/264167/4272?subId1=texasdefined&partnerpropertyid=8837726&MediaPartnerPropertyId=8837726';
...
const OFFER_SOURCE_SEEDS = [
  { source_key: 'ticketmaster-impact', network: 'impact', advertiser: 'Ticketmaster', source_label: 'Ticketmaster Texas events via Impact tracking', fetch_strategy: 'api', is_active: true },
  { source_key: 'cj-link-search', network: 'cj', advertiser: 'CJ approved advertisers', source_label: 'CJ approved offer and content links', fetch_strategy: 'api', is_active: true },
  { source_key: 'impact-promotions', network: 'impact', advertiser: 'Impact programs', source_label: 'Impact promotions and promo codes', fetch_strategy: 'api', is_active: true },
] as const;

const TEXAS_CITY_HINTS = ['texas', 'tx', 'houston', 'dallas', 'austin', 'san antonio', 'fort worth', 'galveston', 'fredericksburg', 'arlington', 'corpus christi', 'el paso', 'waco', 'lubbock', 'amarillo', 'mcallen'];
...
function categoryFor(...values: unknown[]): OfferCategory {
  const text = values.map(asText).join(' ').toLowerCase();
  if (/sport|football|baseball|basketball|soccer|hockey|rodeo/.test(text)) return 'sports';
  if (/concert|music|band|singer|artist|comedy/.test(text)) return 'concerts';
  if (/festival|fair|parade|market/.test(text)) return 'festivals';
  if (/family|kid|children|disney|circus|ice/.test(text)) return 'family';
  if (/theater|theatre|broadway|opera|ballet|arts/.test(text)) return 'theater';
  if (/tour|sightseeing|guided/.test(text)) return 'tours';
  if (/hotel|stay|lodging|resort|motel/.test(text)) return 'hotels';
  if (/outdoor|camp|park|lake|hike|fish|golf/.test(text)) return 'outdoors';
  if (/shop|store|retail|gift|gear|coupon|promo|discount/.test(text)) return 'shopping';
  return 'attractions';
}
...
function supabaseHeaders(serviceRoleKey: string, extra?: HeadersInit): Headers {
  ...
  if (!serviceRoleKey.startsWith('sb_secret_') && !serviceRoleKey.startsWith('sb_publishable_')) headers.set('authorization', `Bearer ${serviceRoleKey}`);
  return headers;
}
...
async function supabaseRequest<T>(env: EnvLike, path: string, init: RequestInit = {}): Promise<T> {
  ...
  const response = await fetch(`${url}${path}`, { ...init, headers: supabaseHeaders(key, init.headers) });
  if (!response.ok) {
...
function collectRecords(value: unknown, matcher: (record: Record<string, unknown>) => boolean, output: Record<string, unknown>[] = []): Record<string, unknown>[] {
  if (Array.isArray(value)) { for (const item of value) collectRecords(item, matcher, output); return output; }
...
function xmlRecords(text: string): Record<string, unknown>[] {
  ...
  for (const block of text.match(/<link\b[\s\S]*?<\/link>/gi) ?? []) {
    const field = (name: string) => block.match(new RegExp(`<${name}[^>]*>([\\s\\S]*?)<\\/${name}>`, 'i'))?.[1]?.replace(/<!\[CDATA\[|\]\]>/g, '').replace(/&amp;/g, '&').trim() ?? '';
    records.push({ linkName: field('link-name') || field('name'), advertiserName: field('advertiser-name'), clickUrl: field('click-url') || field('clickUrl'), description: field('description'), category: field('category') });
  }
  return records;
}
...
async function fetchTicketmasterEventRows(env: EnvLike, now = new Date()): Promise<EventOfferRow[]> {
  ...
  url.search = new URLSearchParams({ apikey: apiKey, countryCode: 'US', stateCode: 'TX', locale: 'en-us', size: '200', page: '0', sort: 'date,asc', includeTBA: 'no', includeTBD: 'no', includeTest: 'no', startDateTime: start.toISOString().replace('.000', ''), endDateTime: end.toISOString().replace('.000', '') }).toString();
  ...
  return events.slice(0, MAX_SYNC_ROWS_PER_SOURCE).flatMap((event: any): EventOfferRow[] => {
    ...
    if (!event?.id || !event?.name || venue?.state?.stateCode !== 'TX' || !officialUrl || !/^\d{4}-\d{2}-\d{2}$/.test(localDate)) return [];
    ...
    return [{
      source_key: 'ticketmaster-impact', external_id: String(event.id), title: truncate(String(event.name), 180),
      description: segment && genre ? `${segment} · ${genre}` : segment || genre || 'Ticketmaster Texas event',
      kind: 'event', category: categoryFor(segment, genre, event.name), network: 'impact', advertiser: 'Ticketmaster',
      city: asText(venue?.city?.name) || null, region: 'TX', venue: asText(venue?.name) || null,
      ...
      start_date: localDate, start_at: startTime ? `${localDate}T${startTime}` : null,
      offer_label: firstPublicOnsaleDay ? 'Primary onsale — editorial only for first 24 hours' : 'Ticket availability',
      affiliate_url: firstPublicOnsaleDay ? null : buildTicketmasterAffiliateUrl(officialUrl, trackingBase), source_url: officialUrl,
      image_url: optionalUrl(image), commission_status: firstPublicOnsaleDay ? 'zero' : 'unknown', discount_preserves_commission: null,
      is_discount: false, is_editorial_only: firstPublicOnsaleDay, is_active: true, last_verified_at: verifiedAt,
...
async function fetchCjOfferRows(env: EnvLike, now = new Date()): Promise<EventOfferRow[]> {
  ...
  url.search = new URLSearchParams({ 'website-id': websiteId, 'records-per-page': '100', 'link-type': 'Content Link' }).toString();
  const payload = await fetchJsonOrText(url, { headers: { accept: 'application/json, application/xml;q=0.8, text/xml;q=0.8', authorization: `Bearer ${token}` } });
  const records = typeof payload === 'string' ? xmlRecords(payload) : collectRecords(payload, (record) => Boolean(record.clickUrl || record.clickURL || record.destination || record.linkUrl || record.linkCodeHtml));
  const verifiedAt = now.toISOString();
  const rows: EventOfferRow[] = [];
  for (const record of records.slice(0, MAX_SYNC_ROWS_PER_SOURCE)) {
    const affiliateUrl = optionalUrl(record.clickUrl ?? record.clickURL ?? record.linkUrl ?? record.destination ?? record.href);
    ...
    rows.push({
      source_key: 'cj-link-search', external_id: externalId, title, description, kind: texasRelevant ? 'attraction' : 'offer',
      category: categoryFor(record.category, record.linkType, title, description), network: 'cj', advertiser, offer_label: title,
      affiliate_url: affiliateUrl, source_url: optionalUrl(record.destinationUrl) || affiliateUrl, image_url: optionalUrl(record.imageUrl),
      commission_status: 'unknown', discount_preserves_commission: null, is_discount: /coupon|promo|discount|sale|save|%|\$\d+/i.test(`${title} ${description}`),
      is_editorial_only: false, is_active: true, last_verified_at: verifiedAt,
...
async function fetchImpactCollection(env: EnvLike, resource: 'Promotions' | 'PromoCodes'): Promise<Record<string, unknown>[]> {
  ...
  if (!sid || !token) return [];
  const payload = await fetchJsonOrText(`${IMPACT_API_BASE}/Mediapartners/${encodeURIComponent(sid)}/${resource}?PageSize=100`, { headers: { accept: 'application/json', authorization: `Basic ${btoa(`${sid}:${token}`)}` } });
...
async function fetchImpactOfferRows(env: EnvLike, now = new Date()): Promise<EventOfferRow[]> {
  const [promotions, promoCodes] = await Promise.all([fetchImpactCollection(env, 'Promotions'), fetchImpactCollection(env, 'PromoCodes')]);
  const verifiedAt = now.toISOString();
  const rows: EventOfferRow[] = [];
  for (const [resource, records] of [['Promotions', promotions], ['PromoCodes', promoCodes]] as const) {
    for (const record of records.slice(0, MAX_SYNC_ROWS_PER_SOURCE)) {
      ...
      const preservesCommission = creditedPolicy === 'ALWAYS' ? true : null;
      rows.push({
        source_key: 'impact-promotions', external_id: `${resource.toLowerCase()}-${externalId}`, title,
        description: truncate(asText(record.Description) || asText(record.Terms) || `${advertiser} ${resource.toLowerCase()}`, 260),
        kind: 'offer', category: categoryFor(record.Category, record.Type, title, advertiser), network: 'impact', advertiser,
        offer_label: code ? `${title} — code ${code}` : title, promo_code: code, affiliate_url: url, source_url: optionalUrl(record.Url) || url,
        image_url: optionalUrl(record.ImageUrl) || optionalUrl(record.LogoUrl), commission_status: preservesCommission ? 'preserved' : 'unknown',
        discount_preserves_commission: preservesCommission, is_discount: Boolean(code) || /coupon|promo|discount|sale|save|%|\$\d+/i.test(title),
        is_editorial_only: false, is_active: true, expires_at: asText(record.EndDate) || asText(record.ExpirationDate) || null,
        last_verified_at: verifiedAt, tags: compactTags(['impact', advertiser, resource, record.Category]),
...
async function ensureOfferSources(env: EnvLike): Promise<Map<string, string>> {
  await supabaseRequest(env, '/rest/v1/texasdefined_offer_sources?on_conflict=source_key', { method: 'POST', headers: { 'content-type': 'application/json', prefer: 'resolution=merge-duplicates,return=representation' }, body: JSON.stringify(OFFER_SOURCE_SEEDS) });
  const sourceKeys = OFFER_SOURCE_SEEDS.map((source) => source.source_key).join(',');
  const rows = await supabaseRequest<Array<{ id: string; source_key: string }>>(env, `/rest/v1/texasdefined_offer_sources?select=id,source_key&source_key=in.(${sourceKeys})`);
...
async function persistRows(env: EnvLike, rows: EventOfferRow[]): Promise<number> {
  ...
  const enriched = rows.map((row) => ({ ...row, source_id: sourceIds.get(row.source_key) ?? row.source_id ?? null, updated_at: new Date().toISOString() }));
  const persisted = await supabaseRequest<unknown[]>(env, '/rest/v1/texasdefined_event_offers?on_conflict=source_key,external_id', { method: 'POST', headers: { 'content-type': 'application/json', prefer: 'resolution=merge-duplicates,return=representation' }, body: JSON.stringify(enriched) });
...
async function syncOne(source: string, fetcher: () => Promise<EventOfferRow[]>, env: EnvLike): Promise<SyncSourceSummary> {
  try {
    ...
    return { source, attempted: true, fetched: 0, persisted: 0, skipped: 0, error: error instanceof Error ? error.message : 'Unknown sync error' };
  }
}
...
export async function syncTexasDefinedEventOffers(env: EnvLike, reason = 'manual') {
  ...
  return { ok: totalPersisted > 0 && errors.length < summaries.length, reason, startedAt: startedAt.toISOString(), finishedAt: new Date().toISOString(), totalPersisted, summaries };
}
...
export async function readTexasDefinedEventOffers(env: EnvLike, requestUrl: string) {
  ...
  const filtered = rows.filter((row) => {
    if (q) {
      const haystack = [row.title, row.description, row.advertiser, row.city, row.venue, row.category, ...(Array.isArray(row.tags) ? row.tags : [])].map(asText).join(' ').toLowerCase();
  ...
  return { ok: true, generatedAt: new Date().toISOString(), filters: { q, city, category, network, commissionSafeOnly, discountsOnly, limit }, count: filtered.length, results: filtered };
}
...
export async function texasDefinedEventOffersResponse(request: Request, env: EnvLike): Promise<Response | null> {
  ...
  if (url.pathname === '/api/texasdefined/event-offers' && request.method === 'GET') {
    try { return jsonResponse(await readTexasDefinedEventOffers(env, request.url)); }
    catch (error) { return jsonResponse({ ok: false, error: error instanceof Error ? error.message : 'Event offers could not be loaded.' }, 500); }
  }
  if (url.pathname === '/api/texasdefined/event-offers/sync' && request.method === 'POST') {
    if (!authorizedSyncRequest(request, env)) return jsonResponse({ ok: false, error: 'Event offer sync is not configured for manual HTTP access.' }, 403);
