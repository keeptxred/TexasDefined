...
export const paintedChurchReadiness: PaintedChurchReadinessRecord[] = expandedPaintedChurches.map((church) => {
  ...
  const dimensions: PaintedChurchReadinessDimension[] = [
    ...
    { id: "preservation-chronology", label: "Church-specific preservation chronology", complete: preservationEvents.length > 0, requiredForIndexLaunch: true, detail: preservationEvents.length ? `${preservationEvents.length} sourced intervention/disaster/restoration event${preservationEvents.length === 1 ? "" : "s"}.` : "Launch blocker: no sourced preservation/alteration/stewardship chronology has cleared the evidence standard. Silence is not treated as proof that the interior was never altered." },
